...
#define part(p) (pbuf+((p)*CSE_MAX_PATH))
	...
	getPathRootName(path, part(0));
	getPathRootDirectory(path, part(1));
	getPathRootStem(path, part(2));
	getPathExtension(path, part(3));
...
//-----------------------------------------------------------------------------
#ifdef __clang__ || __GNUC__ && __linux__
...
//-----------------------------------------------------------------------------
int memicmp(			// Compares memory ingnoring case sensitivity
	const void* str1, 	// First string
	const void* str2, 	// Second string
	unsigned int count) // Count number
// As opposed from stricmp which compares the string until the end of string character
// is reach ('\0'), memicmp compares all the memory allocated for these string. It will
//not comparing after it encounters '\0'.
{
	const unsigned char *s1 = (const unsigned char*)str1;
	const unsigned char *s2 = (const unsigned char*)str2;
	while (count-- > 0) {
		if (tolower(*s1++) != tolower(*s2++))
			return tolower(s1[-1]) < tolower(s2[-1]) ? -1 : 1;
	}
	return 0;
} 		// memicmp
#endif
