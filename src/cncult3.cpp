...
RC SFI::sf_TopSf1()
{
	...
	if (sfc==sfcSURF)
	{
		...
		if (!IsSet( SFX( GRNDREFL)))	// if sfGrndRefl not given for this surface
			CSE_V x.grndRefl = CSE_V Top.grndRefl;  	// get global value from Top object. m-h variable. default .2.
...
0			printf( "Hit\n");
		...
		CSE_V x.sfExT = CSE_V ownSf->x.sfExT;		// exterior temp for SPECT. move as NANDAT: expr handle possible.
		...
		if (sfc==sfcDOOR)				// note wnIhH now defl'd to 10000 (near-0 R) by CULT, 10-28-92. BRUCEDFL.
		{
			...
			if ( !IsSet( SFXO( ABSSLR)))	// exterior absorptivity
				CSE_V x.xs_AbsSlr( 1) = CSE_V ownSf->x.xs_AbsSlr( 1);	// default to owning surface's: drExAbs = sfExAbs
		}														// CSE_V (cnglob.h): don't move as float: (monthly-hourly) expr allowed. 4-92.
...
// returns non-RCOK only on serious error, to terminate topCkf().
{
	...
	RLUP( SgiB, sg)			// loop SGI records
	{
		...
		if (!(fs[SGI_D + SGDIST_FSC] & FsSET))		// if sd_FSC not entered by user
			CSE_V sg->d.sd_FSC = CSE_V sg->d.sd_FSO;		// copy sd_FSO.  CSE_V: *(void**)&: don't copy possible NANDLE as a float
...

{
	...
	CSE_V sg->sd_FSO = fso;		// fracs of distribution (FLOAT or expr nandle: latter errors if moved as float)
	CSE_V sg->sd_FSC = fsc;		// ... shades open and closed.  CSE_V: clglob.h: *(void**)&
}			// cnuSgDist
