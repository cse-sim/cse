...
int FC gaussjb( // Solve a system of equations using Gauss-Jordan elimination
    ...
    int invflg /*=0*/) // invert flag - if nz, a-inverse is returned in a
                       //  if 0,  a is returned garbage (not generating inverse
                       //  faster)
                               ...
                               //        false if cyclic
{
  ...
  if (dg_status[iV] == 1) { // cyclic: put offending vertex into vSorted[ 0]
    vSorted.clear();
...
//-----------------------------------------------------------------------------
bool DGRAPH::dg_CountRefsDFS(int iV, // current vertex
                             std::vector<int> &vRefCounts) {
  if (dg_status[iV] == 1) { // cyclic: put offending vertex into vRefCounts[ 0]
    vRefCounts.clear();
...
PWLFUNC<T, NI>::PWLFUNC(T (*pFunc)(T x), T xMin, T xMax)
