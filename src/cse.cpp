...
// CSE: a program to simulate a building and its HVAC systems to determine
...
extern "C"
{
	_DLLImpExp int CSEZ( // interface to CSE main function, callable by WinMain or application using DLL or subroutine
		...
		,
		HWND _hPar, char **_outPNams, struct BrHans **_hans
#endif
	)
...
x					cul( 4, NULL, NULL, NULL, NULL);	// delete input data now to make the most memory available. cul.cpp.
	...
	if (PriRep.f.fName) // insurance
	{
		...
		vrhs = PriRep.f.vrhs;
		if (VrErr)
			*vrhs++ = VrErr; // any remaining error messages, especially early fatal input errors.
		if (VrLog)
			*vrhs++ = VrLog; // any further log messages (unexpected)
		if (VrInp)
			*vrhs++ = VrInp; // any input listing not already unspooled
		if (vrTimes)
			*vrhs++ = vrTimes; // timing info spooled just above
...
{
	if (!s || !*s)
		return p; // ok NOP if nothing to append
	...
	{
		if (!pWas)
			*p = 0;	  // if new block, make it a null string
