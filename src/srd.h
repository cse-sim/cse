...
0 /*--- Small Record Descriptor Table (srfd.c:sRd)
0	This table gives information about each record type; it is accessed by searching for the record type. */
0struct SRD  	// one RT's info in Srd[]
0{   RCT rt;  		// record type plus bits (defined just above)
0    SI nFlds;		// number of fields (excluding start-record overhead)
0    SFIR * fir; 	// local ptr to array of info about fields in record: offset, name, type (sFdtab index), etc.
0};
0extern SRD sRd[];	/* null-terminated array of SRD, generated by rcdef.exe.
0			   Includes pointers to arrays of SFIR for fields of each record. */
0// note 12-12-91: sRd not referenced -- could be deleted.
#endif
