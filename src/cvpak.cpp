...
#ifdef FMTPVMASK // define in cvpak.h to restore p positive value display
                 // options, 11-91
...
#endif
    static USI
        fmt;     // cvin2s "format" argument; see cvpak.h for field definitions.
...
static SI lz; // leading 0's (right justify).  note FMTLZ not used, could code
              // out. rob 10-88
static SI wsign; // 0 or 1 columns for sign, set by cvin2s
static SI wid;   // width for sprintf: 1 for squeeze, else mfw. cvdd expects
                 // caller to set.
static SI ppos;  // precomputed sprintf precision (prcsn) for several cases for
                 // values >= 0
static SI pneg;  // precomputed sprintf precision (prcsn) for serveral cases for
                 // values < 0
...
#ifdef FMTPVMASK // define in cvpak.h to restore p positive value display
                 // options, 11-91
p p /*   Idea: left-justify format appears to ALWAYS differ by having leading -;
  p 	   could create from base format at run time ? */
    p     /*   These format definitions must match definitions of FMTPVxxx in
             cvpak.h */
        p /* ipv: if >= 0, show null,    +,	 space */
            p add NEAR's if restored. p /* SI and floating 0 formats */ p static char
                *sif[2][3] = {"%*.*d",    "%+*.*d",  "% *.*d",   /* other */
                              p "%-*.*d", "%-+*.*d", "%- *.*d"}; /* left just */
...
p       /* basic float: done in cvdd(). */
    p p /* for trim trailing 0's. ipv:
      null  +           spc */
        p static char *gf[4][3] = {
            "%-*.*g",   "%-+*.*g", "%- *.*g",   /* left j */
            p "%*.*g",  "%+*.*g",  "% *.*g",    /* rj */
            p "%0*.*g", "%0+*.*g", "%0 *.*g",   /* rj 0s */
            p "%*.*g",  "%+*.*g",  "% *.*g" p}; /* sqz */
p p   /*  IP lengths. subscript ipv: null,	 +,		  space */
...
p
#else
// sprintf formats, used in cvin2s, cvin2Ft_in.  Basic float formats are in
// cvdd.
//
//		Idea: left-justify formats appear to differ by leading -; create
// from base format at run time ?
//
...
static char *ff5 = "%*.*f";        // dfw nz, inches only
    ...
    /*----------------------- LOCAL FUNCTION DECLARATIONS
       ---------------------*/
    LOCAL SI FC
    cvttz(char *str, SI trailChar, SI minWid);
LOCAL bool FC nexK();
...
//======================================================================
char *FC cvin2sBuf(char *buf, const void *data, USI dt, SI units, USI _mfw,
                   USI _fmt)
...
// Also sets Cvnchars to strlen(result).
{
  ...
  return buf ? strcpy(
                   buf, // copy into caller's buffer
                   strtempPop(
                       p)) // free Tmpstr space (so less likely to wrap onto
                           // caller's stuff). recursive. returns p. strpak.c.
...
//======================================================================
char *FC
cvin2s( // Convert internal format data to external format string in Tmpstr

    const void
        *data, // Pointer to data in internal form, or NULL to do nothing and
               // return NULL
               //  (for DTCULSTR, is ptr to ptr to string to print, 11-91) */
    USI dt,   // Data type of internal data, or DTNA for "--" or DTUNDEF for "?"
              // from cvfddisp()
    SI units, // Units of internal data (made signed 5-89)
    USI _mfw, // Maximum field width (not including '\0').  If requested format
              // results in string longer than mfw, format will be altered if
              // possible to give max significance within mfw; when not possible
              // (never possible for inteters or if field too narrow for e or k
              // format), field of **** is returned (but see _xfw).
    USI _fmt, // Format.  See cvpak.h for definition of fields.
    USI _xfw /*=0*/) /* 0 or extra field width available: if value cannot be
                formatted in _mfw columns does fit in _mfw + _xfw or fewer cols,
                return the overlong text rather than ******.  added by rob,
                4-92. */

// Returns pointer to result in Tmpstr.
// Also sets global Cvnchars to the number of characters placed in str (not
// incl. '\0').

// This routine will in in some cases overwrite one or more chars beyond the
// specified field.  \0 always appended.

{
#ifdef DTPERCENT
  bool percent =
      false; // set true if converting a DTPERCENT; shares DTFLOAT code
#endif

  fmt = _fmt; // store format arg for use by callees
  mfw = _mfw; // .. max field width arg

  /* NULL data pointer means do NOTHING */ /* for caller convenience in
                                              supporting optional stuff,
                                              tentative 9-89. (note pgw also
                                              nops on getting null 9-89; using
                                              9-89 in crlout.c) */
  if (data == NULL
#ifdef DTUNDEF
  ...
  if (allocLen < 13)
    allocLen = 13; // always enuf for "<unset>\0" or "<expr 99999>\0" 2-27-92
  ...
  p ipv = ((USI)pv) >> FMTPVSHIFT; // .. shifted for use as subscript --
                                   // frequently used to select formats
  ...
  wid = just == FMTSQ ? 1 : mfw; // width for sprintf: 1 to squeeze (float
                                 // only). note cvdd uses file-global.
  // for several cases, preset sprintf precision (ppos, pneg, prcsn): min digits
  // for many formats other than e, f.
...
#ifdef FMTPVMASK
  p case DTSI:
    p if (*(SI *)data == 0 && pv == FMTPVPLUS) p pv = FMTPVSPACE;
    p Cvnchars = sprintf(str, sif[lj][ipv], wid, *(SI *)data < 0 ? pneg : ppos,
                         *(SI *)data);
    p break;
#else
  case DTINT:
...
#ifdef FMTPVMASK
  p case DTLI:
    p if (*(LI *)data == 0 && pv == FMTPVPLUS) p pv = FMTPVSPACE;
    p Cvnchars = sprintf(str, lif[lj][ipv], wid, *(LI *)data < 0 ? pneg : ppos,
                         *(LI *)data);
    p break;
#else
  case DTLI:
    Cvnchars =
        sprintf(str, lif[lj], wid, *(LI *)data < 0 ? pneg : ppos, *(LI *)data);
    break;
#endif
  ...
  case DTFLOAT:
  floatCase: // number-choice comes here (from default) if does not contain
             // choice
  {
...
#endif
    if (units == UNLENGTH &&
        Unsysext == UNSYSIP) // if feet-inch float/double value
      cvFtIn2s(); // convert 'val' to '*str' as feet-inches, local function,
                  // below.
    else
      cvDouble2s(); // convert 'val' to '*str', using many other variables.
                    // local function, below.
#ifdef DTPERCENT
    if (percent) // finish percent: add '%'; make 1 col wider due to wid--
                 // above.
    {
      SI i;
      SI l = strlen(str);
      ...
      while (*(str + (++i)))
        if (*(str + i) == ' ')
          break;        // find space after the nonspace, or end
      *(str + i) = '%'; // put % there
      if (i != l)
        *(str + l) = ' ';    // append space unless just put % there
      *(str + l + 1) = '\0'; // reterminate
    }
#endif
#if defined(M0BUG)
    if (Top.tp_date.month == 12 && Top.tp_date.mday == 3 && Top.iHr == 5 &&
        Top.iSubhr == 1) { // trap subhour where output mismatch occurs
  ...
  choiceCase: // float types comes here if NCHOICE found (unexpected)
    if (dt & (DTBCHOICB | DTBCHOICN)) // if a choice type
    {
      SI handle =
          (dt & DTBCHOICN)
              ? CHN(*(void **)
                        data) // extract choicn choice value (cnglob.h macro)
...
x		}
  ...
  /* if still too wide [and not variable data], fill with *******.  Note most
   * floating values have been made to fit. */

  if ((USI)Cvnchars > mfw + _xfw) // return oversize field anyway if <= _xfw
                                  // (argument) chars overwide
  {
#ifdef DTPERCENT
    if (percent) {
      mfw++;
  ...
  /* optionally add units.  Note ' " for ft-in done in cvFtIn2s.  Suppressed for
   * NCHOICE choice by clearing bits above. */
...
static SI
    amfw; // set by cvdd/cvsd: mfw adjusted for sign. used in cvDouble2s, nexK,
static SI
    ki; // 0 or number of times divided by 1000 for K format. cvDouble2s inits.
static SI nDigB4Pt; // number of digits b4 point, set in cvsd/cvdd, adj in nexK
static const char ddalpha[] =
    " kMGTPEZY?????"; // Chars for K format output field overflow handling
//======================================================================

// 4-92: needs completion of review re producing minimum-overwidth result for
// xfw -- cvsd at least done.

LOCAL void FC cvDouble2s() // float / double output conversion case for cvin2s

// converts 'val' to '*str'; uses other global statics including: fmt, mfw, val,
// str, lj, lz, ppos, .
...
// do not call for foot-inch conversion: see cvFtIn2s.
{
  ...
  {
    if (cvsd(mfw, dfw)) // convert (returns false if should use cvdd: number too
                        // small relative to space)
      return; // if converted (returns best fit if overwide; caller checks
              // Cvnchars)
    // else fall thru to basic conversion to get 0.000 for small numbers, rather
    // than e- format nor overwide .000000n
  } // if FMTRTZ

  // basic floating conversion (not ft-in, not 0, not FMTRTZ)

  if (cvdd(mfw, dfw)) // drifting decimal conversion, nz ok.  Uses str, val,
                      // etc; sets amfw; below.
  ...
  if ((fmt & FMTOVFMASK) ==
      FMTOVFK) // if overlow is to be handled with k format
  {

    // float overflow, K format option (default).  Could recode with new fcn
    // nexK()... 4-92

    // 4-92: needs review re producing minimum-overwidth result for xfw.

    // static char ddalpha[]=" kMGTPE???????"; 	// Chars for K format output  is
    // above
    double maxfit;
    ...
    maxfit =
        Pten[amfw] - 0.5; // protection for amfw > sizeof Pten needed ??? 10-88
  ...
  } else // cvdd fail and not K format
  {
    ...
    while (1)        // until fits or fails
    {
      ...
      if (fabs(val) <
          Pten[oamfw - ew]) // if could fit cols
...
{
  bool biglen =
      (aval > 178000000.); // true to show feet only: set if > max 32-bit int
                           // inches, also set below if too wide.
  if (!biglen) // if feet not too big to express inches in int (in sepFtInch):
               // ie normally
  {
    aval = fabs(val); // absolute val
    bool sq =
        (just == FMTSQ); // squeeze (minimum columns) flag (also 'wid' is 1)
    int inch;            // inches
    ...
    x // true to omit feet; never happens if quinch is off -- prevents ambiguous
      // single numbers.
    ...
    bool justInches = (ft == 0 && inch != 0); // true to omit feet
        ...
        int indfw = fmt & FMTDFWMASK; // init decimal places for inches (same
                                      // value as former 'dfw')
    ...
    int inw = justInches ? 0 // for inches-only fw is used, else
                         : 2 // 2 for integral inches (space b4 0-9; exact inw
                             // needed for rj) ..
                               - sq // but only 1 col if squeezing (no space b4
                                    // squeezed 0-9; inw < actual ok when sq on)
    ...
    if (indfw) // if non-0 # dec places for inches
    {
      ...
      if (fmt & FMTRTZ) // trim trailing zeros option
      {
        ...
        Cvnchars = cvttz(
            ...
            lj ? 1 : wid); /* if lj or FMTSQ (wid=1), do fully (lj padded
                              below). If rj, don't shorten < wid: wd need to pad
                              at front, defer doing that code til need found. */
      }
    } else // indfw 0: show inches as integer only
    {
#ifdef FMTPVMASK // define in cvpak.h to restore p positive value display
                 // options, 11-91
      p p if (justInches) // if showing inches only
          p Cvnchars = sprintf(str, ff3[ipv], p fw, prcsn, // inches wid, digits
                               p inch);
...
#else
      if (justInches)                           // if showing inches only
        Cvnchars = sprintf(str, ff3, fw, prcsn, // inches wid, digits
                           inch);
      else                            // feet and inches
...
#ifdef FMTNOQUINCH // define in cvpak.h to restore feature, 11-91
    x x // omit zero inches with feet if ALL 3 of these options on (rob 10-88
        // for text files):
        x x if (fmt & (FMTSQ | FMTRTZ | FMTNOQUINCH) ==
                          (FMTSQ | FMTRTZ | FMTNOQUINCH)) x {
      x if (!justInches // if feet shown
                x &&
            *(str + Cvnchars - 1) == '0' // 0 last
                x &&
            !isdigitW(*(str + Cvnchars - 2))) // nondigit b4 0
          x {
        x *(str + --Cvnchars) = '\0'; // truncate the 0
        x // quinch = 0;  				* add no " *  quinch is
          // 0 cuz FMTNOQUINCH on if here
            x
      }
      x
    }
    x x // add " for inches unless suppressed
        x x if (quinch /* && Cvnchars < mfw * to omit if won't fit */) x {
      *(str + Cvnchars++) = '"';
      x *(str + Cvnchars) = 0;
      x
    }
    ...
    *(str + Cvnchars) = 0;
    ...
    biglen = (USI)Cvnchars >
             mfw; // set biglen true iff feet-inches too wide for field

  } // if (!biglen)

  // if feet-inch value too big, show feet only

  if (biglen) // if value too big for int inches at entry, OR feet-inches format
              // exceeded field width
  {
...
//======================================================================
LOCAL bool FC nexK() // K format incrementer: call each time need to make string
                     // shorter.  false if cannot.

// "K format" means 1000 shortened to 1k, 1234000 shortened to 1.234M or 1.23M
// or 1.2M, etc.  see ddalpha[] for chars.

// alters val (value), ki (assumed init 0), aval, wid, amfw, nDigB4Pt.  Note mfw
// is not changed. after completion of formatting, caller must append
// ddalpha[ki] to string if ki > 0.
...
// if true is returned, converted string may not be shorter due to round-up.
{
  ...
  {
    if (amfw <= 2)  // no way without a digit and k after (note: need 2 digits +
                    // k to handle all values)
      return false; // not enough room, period.
    if (fabs(val) < 500.) // 500-->1k: smaller (too misleading?);
                          // 499.9999-->.5k: no smaller than 499.
      return false;       // too small to gain from shrinking
    amfw--; // first time, reduce field width (after sign) by 1 for the k, M,
            // etc
    if (wid > 1)
      wid--; // unless FMTSQ, reduce sprintf width for the ditto.
  } else     // already in K format
  {
    if (fabs(val) < 99.5) // 99.5k -->.1M: smaller than 100k.  99.49999k-->.09M,
                          // larger than 99k
      return false;       // too small to shrink more
    if (ddalpha[ki + 1] == '?') // if we have used up all the suffix characters
                                // (ie have divided by about 1e18)
...
//======================================================================
LOCAL bool FC
cvsd( // Significant-digits (trim trailing 0's, g format) output conversion
...
/* returns false if number too small for method used here:
                 use cvdd to get eg 0.00 for .0003, .001 for 0.0006 in 4 cols,
   not .000n (too wide) nor e- format (too wide). returns true  if converted. If
   overwide, mininum width form returned. Also sets:  Cvnchars:    Number of
   characters output: caller checks for excess width */
{
  // ki = 0;					// K format index, init by
  // caller
  aval = fabs(val);    // absolute val
  amfw = _mfw - wsign; // width less sign (public static).  wsign is !(val >= 0.
                       // [&& pv==FMTPVNULL])
  setToMin(_dfw, amfw); // fix turkey calls; reduce for sign.  Note may be
                        // increased to nDigB4Pt below.

// return false if value too small for width: don't duplicate 0 exception and
// cvdd's small-value formatting.
//	eg for 3 cols, need at least .095 to print here as .01; let cvdd do less
//(cvdd prints .005 as .01, .00499 as 0.0).
#define MINHERE                                                                \
  1e-4 // min abs value ever done here, per preferences & sprintf %g limits, was
       // hard-coded .001.
  ...
  if (aval < 1)        // 1 or greater always done here
    if (aval < MINHERE // less than this never handled here
        ||
        aval <
            9.5 * NPten[min(amfw, NPTENSIZE)]) // if too small for g format in
                                               // width even when only 1 sig dig
  ...
  for (nDigB4Pt = 0; // count digits before point (b4 rounding)
       nDigB4Pt < PTENSIZE &&
       Pten[nDigB4Pt] <= aval; //   Pten: powers of 10, [0]..[15]. cons.c.
       nDigB4Pt++)
    ;

#if 0 // idea not used -- leave user the control of seeing eg 99.9k not 99990 by
      // spec'ing 4 digits in 6 columns. 4-9-92.
...
x       _dfw = nDigB4Pt;   				// use the digits, not e or k format
  ...
  // initial handling of k format overflow format: make value fit if can, if
  // does not round up.
  if ((fmt & FMTOVFMASK) ==
      FMTOVFK) // if overlow is to be handled with "k format"
  {
    if (_dfw < 3) // needs 3 digits or 2 + k to work in general: 499, 1k, 99k,
                  // .1M, etc.
      _dfw = min(
          SI(3),
          amfw); // so take them if avail; loop below reduces dfw if it helps.
    while (nDigB4Pt > _dfw && nexK())
      ; // while too many digits b4 ., while possible, divide by 1000 & bump ki.
  }

  // for numbers still too big for small field, use enough digits to get the
  // least excess width, to fit best in mfw+xfw.
  SI idigB4Pt =
      nDigB4Pt +
      (Pten[nDigB4Pt] <= aval + 0.5); // digits in integer format: may round up
  if (idigB4Pt <= 5) // if integer form not wider than e format form "1e+01"
    setToMax(_dfw, idigB4Pt); // do 1st convert with at least enuf digits to not
                              // get e format

  // format number as text, retry if comes out wider than field width

  for (;;) // loop to reduce significant digits and/or do k thing to fit into
           // field
  {
    setToMin(_dfw, max(amfw, idigB4Pt)); // saves an iteration if nexK reduced
                                         // amfW and number clearly fits

    /* pre-round when field width is exactly 1 more than digits b4 point, to fit
       values that sprintf itself cannot. example: 999.9, mfw = 4: there is no
       dfw that will make sprintf do it for us (dfw=4 -->999.9: 5 cols; dfw=3
       overflows) but sprintf of 1000 with dfw==4 works. */
    double _val =
        nDigB4Pt + 1 == amfw // if have 1 column + digits b4 pt (less or more we
                             // can't fix)
                &&
                nDigB4Pt + 1 ==
                    _dfw // and are printing that # sig digits (less-->e format)
                && Pten[nDigB4Pt] <= aval + 0.5 // and rounding adds digit b4
                                                // point (--dfw loop can't fix)
            ? val + 0.5
            : val; // then pre-round value to sprintf, else don't meddle.
    ...
    if (_dfw == amfw    // if enuf digits to fill field specified
        ...
        (nDigAfPt != 1 // and value like 999.9
         || Pten[nDigB4Pt] >
                aval + 0.5)) //  is not going to round to 1000, dropping point
      _dfw--;                // then drop a digit now, save time of a sprintf

#ifdef FMTPVMASK
    p Cvnchars = sprintf(str, gf[ijust][ipv], wid, _dfw,
                         _val); // convert number to string (c library)
#else
    Cvnchars = sprintf(str, gf[ijust], wid, _dfw,
                       _val); // convert number to string (c library)
#endif
    ...
    if (Cvnchars <= amfw + wsign) // note mfw not reduced for k char if any
      if ((fmt & FMTOVFMASK) != FMTOVFK ||
          !strchr(str, 'e')) // if not 'k' fmt overflow or sprinf used no 'e'
        break;               // normal termination of loop

    // text wider than field (expected for roundup cases) or contains 'e' when k
    // format desired

    if (!memcmp(str + wsign, "0.",
                2)) // if begins with "0." (implies following digits)
    {
      strcpy(str + wsign,
             str + wsign + 1);        // take out the leading 0 before the .
    ...
    {
      if (nDigAfPt > 0 &&
          _dfw > 3) // first trim digits after point: 123.4k-->.1234M is no gain
      {
        _dfw--;
        continue; // (after can't nexK, final digits after . trimmed below)
      }
      if (nexK())
        continue; // do a(nother) k format /1000  /  if could, format again
    }

    // for overwide field, produce mininimum possible width for max chance of
    // fitting mfw+xfw columns

    /* don't reduce dfw if further reduction would go to e format if this would
       widen field. Shortest string is "1e+01", or dfw==nDigB4Pt for val <
       9999.5 Stop at dfw==nDigB4Pt if nDigB4Pt < 5, or at dfw==nDigB4Pt+1 if
       value rounds up. */

    if (_dfw <= 1)
      break; // never go to 0 digits
    ...
    if (nDigAfPt > 0       // if there are digits AFTER point
        ...
        || _dfw > nDigB4Pt // or number is not yet as narrow as it can get
                           // (integer format)
                      + (Pten[nDigB4Pt] <=
                         aval + 0.5)) //   this term is 1 if number will round
                                      //   up to added digit
  ...
  return true; // true: we did format the number (see small aval false return
               // above)
} // cvsd
//======================================================================
...
LOCAL bool FC cvdd( // Drifting decimal output conversion
    ...
    SI _dfw) /* Decimal field width: # digits after point,
        also, if non-0, # sig digits for numbers < .1 (digits after .
...
/* Returns 1 if conversion was successful, 0 otherwise.  Also sets:
        ...
        amfw (static SI):     _mfw adjusted for sign, used by caller re field
   overflow handling */
{
#ifdef FMTPVMASK // define in cvpak.h to restore p positive value display
                 // options, 11-91
  p p // These format definitions must match definitions of FMTPVxxx in cvpak.h
      p // null        +	      ' ' if positive
          p static char *ff[4][3] = {
              "%-*.*f",   "%-+*.*f", "%- *.*f",   // left just
              p "%*.*f",  "%+*.*f",  "% *.*f",    // rt
  ...
  aval = fabs(val); // absolute val
  amfw = _mfw -
         wsign; // width less sign.  wsign is !(val >= 0. [&& pv==FMTPVNULL])

  // compute precision (digits after point) to fit field, in range 0 to
  // (possibly increased) _dfw

  int _nDigB4Pt = 0; // local
  while (_nDigB4Pt < PTENSIZE &&
         Pten[_nDigB4Pt] <= aval)   // Pten: powers of 10, [0]..[15]. cons.c.
  ...
  {
    /* attempt to maintain dfw significant digits for numbers smaller than .1:
       conditionally add a few digits after point, 1-92. goals: show more of
       small numbers in typical report colums (dfw 1-3), but not as much as for
       large numbers. numbers too small for significance to show: don't crowd
       page too much with 0.0000's. don't add lots of digits to small export
       numbers (FMTRTZ gets here when < .001, dfw 6 already so) */

    if (_dfw >
        0) /* don't go into decimal places if none requested (take dfw==0 as 0
sig dig here)
(??? or, do "if (!_dfw) _dfw++;" at foot of loop to get a sig dig to show) */
    {
      int maxDfw = min(prcsn, _dfw + 3); // don't exceed space avail; don't add
                                         // > 3 places here (shd that be 2?)
      maxDfw = min(maxDfw, 7); // increase only to 7 dec places: avoid garbage
                               // detail; don't exceed NPten[].
      if (aval < NPten[maxDfw] * .5) // if only 0's would show anyway even after
                                     // round, don't crowd:
        maxDfw =
            min(maxDfw - 1, /* add 1 less digit here (re 7 or +3 limits), and */
                prcsn + wsign - 2); // leave space for sign even if > 0, here
                                    // only.  CAUTION: may make maxDfw < 0.

      for (int n0afPt = 1; _dfw < maxDfw;
           _dfw++, n0afPt++) // increase _dfw til dfw sig digits show or to
                             // maxDfw limit
      {
        // test if enuf significant digits will (now) show
        if (aval >= NPten[n0afPt]) // if aval > .1 at entry, > .01 next
                                   // iteration, then .001, .0001, etc.
          break; // stop if have added a digit for each leading 0 after point

        /*if (!_dfw) _dfw++   		 	if get here with 0 _dfw, do
           extra ++ for 1 sig dig. else dfw==0 means "0 sig dig" and only
           leading 0's after . wd show. */
      }
    }
    if (prcsn > _dfw)
  ...
  while (1) {
    ...
    // too wide.  If number < 1.0, sprintf output "0.xxx".  ndig did not allow
    // for 0 b4 point.  Remove it.
    char *zp;
    if (_nDigB4Pt == 0 &&
        *(zp = (str + Cvnchars - prcsn - 2)) == '0') // if of format 0.xxx
    {
...
// returns new length
// called from cvin2s to trim decimal inches 10-88; might be useful re other
// floats types if reimplementing.
{
  char *p, *pend, *q;
  SI len;
...
RC
    FC
    cvLmCk( // Check input data for being within limits for data and limit type
...
double FC cvExtoIn( // Convert value from external units to internal
    ...
    int units) // unit type: UNxxx defines from units.h generated by rcdef.exe
               // from data\units.def
...
// Returns converted value
{
  if (units != UNNONE) {
    ...
    if (units == UNTEMP && Unsysext == UNSYSSI) // if temperature
      f +=
          32.0; // complete C to F adjusment -- ONLY units where 0 point changes
  }
  return f;
} // cvExtoIn
//======================================================================
...
/* DO NOT use in high precision situations without consideration, cuz if ang >
   2PI-.001, it is changed to 0. Other such "snaps" may be added in the future.
 */
...
// Returns equivalent angle that falls in range 0 - 2*PI
{
  ...
  while (ang < 0.0) // until positive
    ang += k2Pi;    // add 2 * Pi
  if (ang > (k2Pi - .001))
    ang = 0.; // If angle is close to 2PI, change it to 0 so it won't display as
              // 360.
  return ang;
} // cvstdangle
...
// If d < 0, feet are returned negative; inches are returned positive unless
// feet are 0. Since d is converted to int inches, the largest distance which
// can be converted is around 178,956,970 ft (about 33,893 miles; I know you're
// disappointed).
{
  int nd = d < 0.0 ? -12 : 12;
...
const char *getChoiTxI( // text for choice of choice data type
    ...
    SI chan, // choicb index (use CHN(float), cnglob.h, to extract hi word of
             // choicn)
             //   (1 based)
...

{
  if (dt & (DTBCHOICB | DTBCHOICN)) // verify that is a choice type -- debug aid
  {
    ...
    if (dt & DTBCHOICN) // for choicn, chan may be hi word of float, including
                        // bits to make a NAN
      if ((chan & 0xff80) ==
          NCNAN) // if proper NAN bits (NCNAN: 7f80, cnglob.h), not just garbage
        chan &= ~NCNAN; // remove them for check. But leave improper bits to
                        // evoke error msg.
    if (chan <= 0 ||
        chan > GetDttab(dt).nchoices) // check that choice is in range 1 to #
                                      // choices for dt. GetDttab: srd.h.
    {
    ...
    // access text
    const char *chtx =
        GetChoiceText(dt, chan); // access text for fcn. srd.h fcn.

    // return info
    if (!(options & 1)) { // retrieve 1st piece to TmpStr
      const char *p = chtx;
...
//======================================================================
RC FC cvS2Choi(    // convert string to choice value for given data type else
                   // format (do not display) error msg
    const char *s, // string to convert
    USI dt,   // choicb or choicn data type (dtypes.h) to convert: specifies
              // choice strings in Dttab[].
    void *pv, // NULL or receives choice value: 2 bytes for choicb, 4 bytes (hi
              // 2 significant) for choicn.
    USI *pSz, // NULL or receives size: 2 or 4
    MSGORHANDLE
        *pms) // if non-NULL
              //   string not found: receives ptr to Tmpstr message insert: "%s
              //   not one of xxx yyy zzz ..." string=alias: receives ptr to
              //   deprecation Tmpstr message else receives NULL
...
//    RCBAD2 with *pms set on info/warning (use of alias,)
{
  ...
  if (dt & (DTBCHOICB | DTBCHOICN)) // if a choice type
  {
    // search this choice data type's strings for a match, using getChoiTxI
    // (just above).
    int v;
    for (v = 1; v <= GetDttab(dt).nchoices;
         v++) // loop data type's choices (GetDttab: srd.h)
    {
      int tyX;
    ...
    // not found.  generate error message insert showing given string and
    // choices
    ...
    if (pms) // if message return pointer given
    {
      ...
      const char *ms =
          strtprintf(MH_V0039, s); // start assembling string "'%s' is not one
                                   // of choice1 choice2 ..."
      for (v = 1; v <= GetDttab(dt).nchoices;
           v++) // loop data type's choices, concatenate each to ms
      {
        int tyX;
        const char *chtx = getChoiTxI(dt, v, &tyX); // get vth choice
        if (tyX != chtyHIDDEN)                      // if not hidden
        {
          if (strlen(ms) + strlen(chtx) >
              200) // if there's an unexpectedly large # long choices
          {
  ...
  if (pms)
    *pms = strtprintf(
        MH_V0038,
        dt);    // "cvpak:cvS2Choi(): given data type 0x%x not a choice type"
...
t t #include<gvpak.h> /* test input routines, in lib\ but not in library.*/
    t t static float testf = 123.345;
t t main() t {
  t char buf[200], buf2[200], dbuf[10];
  t #define NV 11 t static SI vallist[NV] = {-10000, -1000, -100, -10,  -1,   0,
                                             1,      10,    100,  1000, 10000};
  t #define NFV 12 t static float fvlist[NFV] =
      t{-13945789, -999.99, -100., -99.99, -1., 0.,
        .006,      .999,    1.0,   9.999,  10., t 978437852312.};
  ...
  t f = FMTRJ + 2;
  t while (1) t {
    cvin2sBuf(buf, (char *)&val, DTFLOAT, UNLENGTH, 10, f);
    t
  }
  t t /* #define PERCENTTEST */
      t #ifdef PERCENTTEST t dt = DTPERCENT;
  t val = .30;
  ...
  t printf("\n'%s'", buf);
  t #endif /* PERCENTTEST */
      t t  /* #define CHOICETEST */
          t #ifdef CHOICETEST t dt = DTYESNO;
  t un = UNNONE;
  t lm = LMNONE;
  t dfw = 0;
  ...
  t #endif /* CHOICETEST */
      t t  /* #define INTEST */
          t #ifdef INTEST t dt = DTPERCENT;
  t dfw = 2;
  t while (strlen(getstr(">>> ", buf2)) > 0) t {
    *(long *)dbuf = 0L;
    t rc = cvs2in(buf2, dt, un, lm, dbuf, WRN);
    t if (dbuf != Pgb) t ? ? ? t cvin2sBuf(buf, Pgb, dt, un, 7, FMTSQ + dfw);
    t if (rc == RCOK)
        t printf("Cvnchars=%d  Gbsize=%d  %s\n", Cvnchars, Gbsize, buf);
    t
  }
  t #endif  /* INTEST */
      t t t /* #define TESTINT */
          t #ifdef TESTINT t f = FMTLZ;
  t dt = DTUSI;
  t mfw = 6;
  t for (iv = 0; iv < NV; iv++) t {
    i = vallist[iv];
...
t		*/
  ...
  t printf("[%s]   %d\n", buf, Cvnchars);
  t #endif                                    /* TESTSTR */
      t t #undef TESTDATE t #ifdef TESTDATE t /*
                                            t        dt = DTDOY;
                                            t        mfw = 10;
                                            t        i = 1;
                                            t        p = (char *)&i;
  ...
  t i = 4;
  t p = (char *)&i;
  t t cvin2sBuf(buf, p, dt, UNNONE, mfw, FMTLJ);
  t printf("[%s]   %d\n", buf, Cvnchars);
  ...
  t #endif /* TESTDATE */
      t t  /* #define TESTFL  */
          t #ifdef TESTFL t dt = DTFLOAT;
  t for (iv = 0; iv < NFV; iv++) t {
    val = fvlist[iv];
    t mfw = 10;
    t dfw = 2;
    t f = FMTLJ;
  ...
  t #endif /* TESTFL */
      t t #ifdef TESTFTIN t val = 100.9;
  t mfw = 6;
  t dt = DTFLOAT;
  t *buf = '\0';
  t for (iv = 0; iv < 2; iv++) t {
    for (ifx = 0; ifx < 4; ifx++)
      t {
        f = fmts[ifx];
        t printf("\n%s ... ", fmtnames[ifx]);
  ...
  t #endif /* TESTFTIN */
      t t  /* #define TESTCF */
          t #ifdef TESTCF t #define GETFDDT(r, f)
              DTFLOAT t /* #define GETFDP(r,f) (&testf) */
                  t #define GETFDUN(r, f)
                      UNENERGY t #define GETUNTAG(u)("kBtuh")
                          t #define RCRT(r)(0) t m = 30;
  t for (mfw = 5; mfw < 9; mfw++) t {
    cvcf2init(m, mfw, m + 20, mfw + 1);
    t for (dfw = 0; dfw < 3; dfw++) t {
      p = cvcf2(buf, "Dog", &testf, 1, FMTRJ + FMTUNITS + dfw, t & testf, 1,
                FMTRJ + FMTUNITS + FMTNODATA + dfw);
      t printf("[%s] Len=%d\n", p, strlen(p));
      t
    }
    t
...
t t main() t {
  ...
  t static struct /* test driver table */
      t {
    char *s;   /* string to convert, NULL to term table */
...
t 			   3: cvatof, percent = 1 */
    t
              ...
              t NULL t};
  t t loopLimit = 1;
  t for (loop = 0; ++loop <= loopLimit;) t {
    t printFlg = loop == loopLimit;
    t i = -1;
    t while ((s = tab[++i].s) != NULL) t {
      test = tab[i].test;
      t if (printFlg) t printf("s=|%s|  ", s);
      t if (test <= 1) t {
        t li = 0L;
        t rc = cvatol(s, &li, test);
        t if (printFlg) t printf("li = %ld", li);
        t
      }
      t else t {
        t d = 0.;
        t rc = cvatof(s, &d, test - 2);
        t if (printFlg) t printf("d = %f", d);
        t
      }
      t if (printFlg) t printf("   rc = %d\n", rc);
      t
    }
    t
  }
...
#define ERR 0xF0  /* syntax error, return "Invalid number" */
  static UCH actState[9][7] = {
      ...
      /*				------  ------	------	------	------
         ------	------ */
      /* lws: leading ws   */ {SK + lws, GSN + ld, GD + gd, SK + gdd, ERR, ERR,
                               ERR},
      /* ld:  leading dgt  */ {SK + ld, ERR, GD + gd, SK + gdd, ERR, ERR, ERR},
      /* gd:  get digits   */
      {SK + tc, ERR, GD + gd, SK + gdd, SK + xsn, SK + tws, DONE},
      /* gdd: get dec dgts */
      {SK + tc, ERR, GDD + gdd, ERR, SK + xsn, SK + tws, DONE},
  ...
  for (;;) /* loop over char string */
  {
    ...
    if (isdigitW(c))
      ...
      ct = 5; /* cond'l recog of '%' */
    else
      return MH_V0010; // "Invalid number": unrec char
    //   or bad format
