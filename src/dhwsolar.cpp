...
///////////////////////////////////////////////////////////////////////////////
/*virtual*/ DHWSOLARSYS::~DHWSOLARSYS() {} // DHWSOLARSYS::~DHWSOLARSYS
//---------------------------------------------------------------------------
/*virtual*/ void DHWSOLARSYS::Copy(const record *pSrc, int options /*=0*/) {
    ...
    new(&sw_tank.hw_pNodePowerExtra_W) std::vector<double>(
            ((const DHWSOLARSYS *) pSrc)->sw_tank.hw_pNodePowerExtra_W);
} // DHWSOLARSYS::Copy
//----------------------------------------------------------------------------
/*virtual*/ void DHWSOLARSYS::ReceiveRuntimeMessage(const char *msg)
// callback from HPWH for reporting error messages
{
    pInfo("%s: HPWH message (%s):\n  %s", objIdTx(), Top.When(C_IVLCH_S), msg);
...
// hour init
{
    ...
    DHWSOLARCOLLECTOR *pSC;
    RLUPC(ScR, pSC, pSC->ownTi == ss) { rc |= pSC->sc_DoHour(); }
    return rc;
} // DHWSOLARSYS::sw_DoHour
//-----------------------------------------------------------------------------
RC DHWSOLARSYS::sw_DoSubhrStart(
        [[maybe_unused]] int
        iTk0) // subhr starting tick within hr (0 .. Top.tp_nHrTicks()-1)
{
    RC rc = RCOK;
        ...
        IVLCH ivl) // C_IVLCH_Y, _M, _D, _H (do not call for _S)
{
    ...
    DHWSOLARCOLLECTOR *pSC;
    RLUPC(ScR, pSC, pSC->ownTi == ss)rc |= pSC->sc_DoHourEnd();
    ...
    if (ivl == C_IVLCH_Y) {
        ...
        sw_SSFAnnual = SSFDen > 0. ? min(1.f, float(SSFNum / SSFDen)) : 0.f;

        double scTotQFluidAll = 0.;
        RLUPC(ScR, pSC, pSC->ownTi == ss) { scTotQFluidAll += pSC->sc_totQFluid; }
        ...
        int iTk) // tick within hour, 0 - Top.tp_nHrTicks()-1
{
    ...
    if (sw_tickTankTOutlet >=
        sw_tankTHxLimit) // if tank temp >= max allow temp
    ...
    {                 // collector flow > 0
        ...
        sw_scTInlet = sw_scTOutlet -
                      sw_tankHXEff * deltaT; // collector inlet temp for next tick
#endif
    } else {
    ...
    rc |= sw_tank.hw_DoSubhrTick(iTk, sw_tickVol, scQGain, sw_tankTInlet,
                                 sw_tickTankTOutlet);
...
RC DHWSOLARCOLLECTOR::sc_Init() {
    ...
    if (!IsSet(DHWSOLARCOLLECTOR_OPRMASSFLOW)) {
        ...
        sc_flowCorrection =
                sc_FlowCorrection(); // operating and test mass flow rates given
    ...
    sc_oprVolFlow =
            sc_areaTot * sc_oprMassFlow / pSW->sw_scFluidDens * galPerFt3 / 60.f;
    ...
    sc_tickPumpQ =
            sc_pumpPwr * Btu_per_Wh * Top.tp_tickDurHr; // pump energy per tick, Btu
    sc_pumpDT = sc_pumpLiqHeatF * sc_pumpPwr * Btu_per_Wh /
                sc_oprMCp; // temp rise through pump
...
//--------------------------------------------------------------------------------------
/*static*/ float
DHWSOLARCOLLECTOR::sc_Deriveb0(float KtaX, // Kta for given incidence angle
        //   0 < KtaX < 1
...
//--------------------------------------------------------------------------------------
float DHWSOLARCOLLECTOR::sc_Kta(float incA) const // incident angle, rad
{
    static const float incALim = RAD(89.f);
    float kta = incA > incALim ? 0.f
...
RC DHWSOLARCOLLECTOR::sc_InitPiping() // init collector loop piping params
{
    ...
    //   approx same as CSI-Thermal assumption (=50 ft + 5 ft per collector
    //   module)
    if (!IsSet(SCPIPE(LEN)))
...
RC DHWSOLARCOLLECTOR::sc_DoHour() {
    ...
    rc |= slPerezSkyModel(sc_tilt, sc_azm, Top.iHrST, Wthr.d.wd_DNI,
                          Wthr.d.wd_DHI, Top.grndRefl, sc_incA, sc_poaRadDB,
                          sc_poaRadDS, sc_poaRadDG);

    // total incident w/o incidence angle modifier
    float poaRadTotNoIAM = sc_poaRadDB + sc_poaRadDS + sc_poaRadDG;

    if (sc_b0 > 0.f && poaRadTotNoIAM > 0.f) { // apply IAM factors
...
//-----------------------------------------------------------------------------
float DHWSOLARCOLLECTOR::sc_TempOutlet( // col+piping outlet temp for current
        // conditions
...
// returns return temp, 0 if no sun
{
    ...
    if (sc_poaRadTot > 0.f) {
        ...
        float heat_gain =
                sc_areaTot * sc_poaRadTot * colEff; // heat collection power, Btuh
...
RC DHWSOLARCOLLECTOR::sc_DoSubhrTick() {
    ...
    sc_tInlet =
            (sc_tOutletB + pSW->sw_tankHXEff * (pSW->sw_tankTHx - sc_tOutletB)) /
