...
/* #define TEST */
#ifdef TEST
t unmaintained 9 - 12 - 89 t main() t {
  t SI i;
  t float x;
  t LDATETIME ldt;
  t IDATETIME idt;
  t t #ifdef KITEST t enkiinit();
  ...
  t if (enkichk()) t printf("Still set\n");
  t else t printf("Clear\n");
  t t hello(NULL);
  t enkimode(KICLEAR + KIBEEP);
  t printf("\nlooping...");
  t for (i = 0; i < 10000; i++) t {
    x = 2.;
    t x = x * x * x * x * x;
    t if (enkichk()) t {
      printf("\n\nInterrupt....");
...
// returns info as string or "?" if error
{
...
#if CSE_OS == CSE_OS_WINDOWS
  HANDLE hFile = CreateFile(exePath.c_str(), GENERIC_READ, FILE_SHARE_READ,
  ...
  if (hFile == INVALID_HANDLE_VALUE)
    ...
    if (hFileMapping == 0)
      ...
      PIMAGE_DOS_HEADER pDosHeader = (PIMAGE_DOS_HEADER)MapViewOfFile(
      ...
      if (pDosHeader == 0)
        ...
        if (pDosHeader->e_magic != IMAGE_DOS_SIGNATURE)
          ...
          try { // use try/catch re possible bad e_lfanew, EOF, etc.
            ...
            if (pNTHeader->Signature != IMAGE_NT_SIGNATURE)
              ...
              linkerVersion = WStrPrintf(
                  "%d.%d", pNTHeader->OptionalHeader.MajorLinkerVersion,
  ...
  if (timeDateStamp == 0)
    ...
    exeInfo =
        WStrPrintf("%s   (VS %s    %d bytes)", tdldts(timeDateStamp, NULL),
                   linkerVersion.c_str(), codeSize);
  return exeInfo;
} // enExeInfo
//=====================================================================
...
#define KISEEN                                                                 \
  2 // KI has been noticed by prog and is being processed.
...
//=====================================================================
void FC
enkimode( // Set keyboard interrupt mode and conditionally reset interrupt flag
    ...
    SI mode) // Keyboard interrupt mode.  defines in envpak.h:
             ...
             //  KILEAVEHIT:  do not clear hit not yet seen by program (any real
             //  uses? dospak:pause() 11-89... */
{
...
// Returns non-0 if ^C has occurred and has not yet been cleared by calling
// enkimode().
{
#if 1
  return CheckAbort();
...
//										 actual
// decl in CRT (formerly math.h), now (12/23) not known
// void CDEC fpeErr( INT, INT);		// intercepts floating point errors, and
// integer errors under Borland
...
t t main() /* Test routines */
  ...
  t #else t i = j;
  t i = 1 / i;
  t #endif t t return 0;
  t
} /* test main */
#endif /* TEST */

//=====================================================================
void FC hello() // initializes envpak, including re library code error exits and
                // user exits.
...
signal( SIGFPE, 			// floating point errors, and integer errors (changes 0:0) under Borland C
  ...
  // _dos_setvect( 0, (void (interrupt *)())iDiv0Err );	// 0: integer divide by
  // 0
  //    to ease reporting calling location (in which case aborts, no return). */

  //--- save current drive/dir to restore at exit
  strcpy(cwdSave, xgetdir()); // save current dir (including drive) in static
                              // for byebye to use at exit
} // hello
//=====================================================================
...
// error reporting out of library.
{
  ...
  errCrit(PWRN, // display critical msg
          ...
          x->arg1, x->arg2);      // POSSIBLE arg values; not reliable since
                                  // intrinsic fcns do not set these members
...
#if 0 && !defined(__BORLANDC__) // used under microsoft C; borland generates
