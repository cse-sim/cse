...
// returns RCOK iff success
{
  ...
  if (volX > 0.f && hw_pHPWH->setTankSize(volX, HPWH::V_UNITS::GAL, true) != 0)
    rc |= RCBAD;
...
// returns RCOK iff success
{
  ...
  if (hw_pHPWH->HPWHinit_presets(preset) != 0)
    rc |= RCBAD;
  if (hw_pHPWH->setTankSize(vol, HPWH::V_UNITS::GAL) != 0)
    rc |= RCBAD;
  return rc;
...
// returns RCOK iff success
{
  ...
  int hpwhRet = 0;
  double minT = hw_pHPWH->getMinOperatingT(HPWH::T_UNITS::F);
  if (minT == double(HPWH::HPWH_ABORT))
    ...
    hpwhRet = hw_pHPWH->setCompressorOutputCapacity(
        ...
        HPWH::UNITS_BTUperHr, HPWH::T_UNITS::F);
...
{
  RC rc = RCOK;
  vol = hw_pHPWH->getTankSize(HPWH::V_UNITS::GAL);
  double UAd;
...
//   (accounting for possible multiple tanks)
{
  ...
  double surfAPerTank =
      HPWH::getTankSurfaceArea(volPerTank, HPWH::V_UNITS::GAL, HPWH::UNITS_FT2);
...
// returns RCOK iff success
{
  ...
  float runHrMin = hw_pHPWH->getCompressorMinRuntime(
      HPWH::UNITS_HR); // minimum compressor run time, hr
...
{
  return hw_pHPWH->getTankSize(HPWH::V_UNITS::GAL);
...
// returns average tank temp, F for node range
{
  ...
  for (int iN = iNode0; iN != iNodeN; iN += incr)
    T += hw_pHPWH->getTankNodeT(iN, HPWH::T_UNITS::C);
...
//   = current top node temp (no consideration of draw etc.)
{
  ...
  return hw_pHPWH->getTankNodeT(iNodeTop, HPWH::T_UNITS::F);
...
// returns RCOK iff success
{
  ...
  if (!hw_pHPWH->isSetpointFixed()) {
    ...
    hw_pHPWH->setSetpointT(tSetpointX, HPWH::T_UNITS::F);
  }

  // retrieve resulting setpoint after HPWH restrictions
  tSetpoint = hw_pHPWH->getSetpointT(HPWH::T_UNITS::F);
  if (hw_tHWOut == 0.f)
  ...
  if (!hw_tankTempSet) { // initialize tank temp on 1st call
    ...
    if (tankTInit != nullptr) {
      ...
      if (hw_pHPWH->setTankTs(vTankTInit, HPWH::T_UNITS::F))
        rc |= RCBAD;
    } else {
...
// returns RCOK iff success
{
  ...
  hw_tankHCBeg = hw_tankHCEnd > 0. ? hw_tankHCEnd
                                   : KJ_TO_KWH(hw_pHPWH->getHeatContent_kJ());
    ...
    int drStatus /*=0*/)     // demand response control signal
{
  ...
  if (hw_bWriteCSV) {
    ...
    CSVItem CI[] = {
        ...
        "qTX",      tk.wtk_qTX,
        UNENERGY3,  5,
        "qEnv",     KJ_TO_BTU(hw_pHPWH->getEnergyRemovedFromEnvironment_kJ()),
        UNENERGY3,  5,
        "qLoss",    KJ_TO_BTU(hw_pHPWH->getStandbyLosses_kJ()),
        UNENERGY3,  5,
        "XBU",      HPWHxBU,
        UNENERGY3,  5,
        NULL};
    ...
    ZNR *pZnASHPSrc)        // ASHP heat source zone, NULL if none
{
  ...
  hw_tankHCEnd = KJ_TO_KWH(
      hw_pHPWH->getHeatContent_kJ()); // end-of-step heat content
...
// returns RCOK on success
{
    ...
    if (what) {
      ...
      if (wh_HPWH.hw_pHPWH->isTankSizeFixed())
        oInfo("%s is ignored when whASHPType=%s (tank volume is fixed at %0.0f "
              ...
              wh_HPWH.hw_pHPWH->getTankSize(HPWH::V_UNITS::GAL));
      ...
      if (vol > 0.f) {
        wh_vol = vol;
        wh_HPWH.hw_pHPWH->setTankSize_adjustUA(vol, HPWH::V_UNITS::GAL);
      }
    }
  }
  ...
  if (!rc && !pWS->ws_configChecked) {
    if (wh_HPWH.hw_IsSetpointFixed()) {
      ...
      float tspFixed = wh_HPWH.hw_pHPWH->getSetpointT(HPWH::T_UNITS::F);
...
RC DHWHEATER::wh_DoSubhrStart() {
  ...
  if (wh_IsHPWHModel())
    ...
    if (wh_effSh <= 0.f)
      rc |= err("%s, %s: Invalid water heater efficiency %0.3f", objIdTx(),
