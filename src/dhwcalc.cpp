...
{
    RC rc = RCOK;
    if (ivl == C_IVLCH_Y) { // beg of warmup or main sim: zero all DHWSYSRESs
        DHWSYSRES *pWS;
        RLUP(WsResR, pWS)pWS->wsr_Init();
    }
    DHWSYS *pWS;
    RLUPC(WsR, pWS,
          !pWS->ws_HasCentralDHWSYS()) { // loop central systems (child systems
            // handled within ws_DoHour)
            rc |= pWS->ws_DoHour(ivl);
        }

    // solar water heating systems
    DHWSOLARSYS *pSW;
    RLUP(SwhR, pSW)rc |= pSW->sw_DoHour();
...
RC DHWSubhr() // DHW (including solar DHW) subhr calcs
{
    ...
    DHWSYS *pWS;
    RLUP(WsR, pWS)rc |= pWS->ws_DoSubhrStart(iTk0);

    DHWSOLARSYS *pSW;
    RLUP(SwhR, pSW)rc |= pSW->sw_DoSubhrStart(iTk0);

    for (int iTk = iTk0; !rc && iTk < iTkL; iTk++) {
        // solar water heating systems: init
        RLUP(SwhR, pSW)pSW->sw_TickStart();

        // water heating systems
        //   models all children (DHWHEATER, ...)
        RLUP(WsR, pWS)rc |= pWS->ws_DoSubhrTick(iTk);

        // solar water heating systems
        //   draw and inlet temp are accum'd during DHWSYS calcs
        RLUP(SwhR, pSW)rc |= pSW->sw_TickCalc(iTk);
    }

    RLUP(WsR, pWS)rc |= pWS->ws_DoSubhrEnd();

    RLUP(SwhR, pSW)rc |= pSW->sw_DoSubhrEnd();
...
// called at end of each hour
{
    ...
    // solar water heating systems
    DHWSOLARSYS *pSW;
    RLUP(SwhR, pSW)rc |= pSW->sw_EndIvl(ivl);

    DHWSYS *pWS;
    RLUP(WsR, pWS)rc |= pWS->ws_EndIvl(ivl);
...
// utility functions
///////////////////////////////////////////////////////////////////////////////
static int DHWMix(float tMix,  // target mixed water temp
...
//-----------------------------------------------------------------------------
void DHWMTR_IVL::wmt_Copy(const DHWMTR_IVL *s, float mult /*=1.f*/) {
    if (mult == 1.f)
...
void DHWMTR::wmt_Accum(
        IVLCH ivl,    // destination interval: day/month/year.  Accumulates from
        // hour/day/month.  Not Top.ivl!
        int firstflg) // iff TRUE, destination will be initialized before values are
// accumulated into it
{
    DHWMTR_IVL *dIvl =
            &curr.Y + (ivl - C_IVLCH_Y); // point destination substruct for interval
...
{
    BYTE *d = ((BYTE *) this + (offsetof(DHWMTR, prior) - offsetof(DHWMTR, curr)));
...
struct DHWFX // info about a fixture
{
    DHWFX(DHWEUCH hwEndUse = 0, int drainCnx = 0, int coldCnx = 0)
            : fx_hwEndUse(hwEndUse), fx_drainCnx(drainCnx), fx_coldCnx(coldCnx),
              fx_hitCount(0) {}
...
{
    DWHRUSE()
            : wdw_iFx(-1), wdw_coldCnx(0), wdw_vol(0.f), wdw_volHR(0.f),
              wdw_temp(0.f) {}

    DWHRUSE(int iFx, int coldCnx, float vol, float volHR, float temp)
            : wdw_iFx(iFx), wdw_coldCnx(coldCnx), wdw_vol(vol), wdw_volHR(volHR),
              wdw_temp(temp) {}
...
struct DHWTICK // per tick info for DHWSYS
{
    ...
    float wtk_tInletX;  // post-DWHR / post-SSF cold water temperature for this
    // tick, F
    ...
    float wtk_qLossNoRL; // additional non-loop losses (e.g. branch), Btu (+ = out
    // of DHWSYS)
    double wtk_volIn;    // total tick inlet vol, gal (not including wtk_volRL and
    // wtk_volCHDHW)
    ...
    void wtk_Init(float startMin = 0.f, double whUseTick = 0.,
                  float tInlet = 50.f) {
        ...
        double mult)         // multiplier
{
    ...
    if (tWhUse > 0.) {
        wtk_tInletX = (wtk_whUse * wtk_tInletX + sWhUse * s.wtk_tInletX) / tWhUse;
        wtk_whUse = tWhUse;
    }
...
//=============================================================================
float DHWTICK::wtk_DrawTot( // tick draw for non-HPWH (not called for HPWH)
...
// see HPWHLINK::DoSubhrTick() for comparable calcs for HPWH
...
// returns total WH draw volume for tick (including loop and CHDHW flow), gal
{
    ...
    //  Note: CHDHW not supported for non-HPWH 12/2022
    //        covered here re possible extension
    float volX = wtk_volRL + wtk_volCHDHW;
    if (volX == 0.f)
        return wtk_volIn; // no add'l flows, tInletMix OK

    float drawTot = wtk_volIn + volX;
    tInletMix = (wtk_volIn * tInletMix + wtk_volRL * wtk_tRL +
                 wtk_volCHDHW * wtk_tRCHDHW) /
                drawTot;
    return drawTot;
...
struct DHWHRTICK // per tick info for DHWHEATREC
{
    ...
    void wrtk_Init() { wrtk_draws.resize(0); }
...
private:
    ...
    std::unique_ptr<DHWSIZEDAY[]>
            wz_topNDays; // top wz_nSizeDays DHWSIZEDAYs *not sorted*
    ...
    std::priority_queue<DHWSIZEDAY *, std::vector<DHWSIZEDAY *>,
            auto (*)(const DHWSIZEDAY *, const DHWSIZEDAY *)->bool>
            wz_sizeDays{[](const DHWSIZEDAY *l, const DHWSIZEDAY *r) -> bool {
        return l->Sum() > r->Sum();
    }};
...
// returns RCOK iff success
{
    ...
    for (iX = 0; iX < min(nSizeDaysActual, static_cast<UINT>(NDHWSIZEDAYS));
         iX++) {
    ...
    for (DHWSIZEDAY *pSzD: topN) {
        for (UINT iH = 0; iH < 24; ++iH) {
            ...
            for (UINT iL = 0; iL < 24;
                 ++iL) { // net heat for current hour = cap - use
                float qSupHr = heatingCapDes -
                               pSzD->wzd_loadHrs[(iH + iL) % 24]; // + = excess capacity
    ...
    float volRunning =
            max(10.f, -qRunning / (waterRhoCp_Btu_per_galF *
...
// called at end of each DHWSYS input
{
    ...
    if (IsSet(DHWSYS_CENTRALDHWSYSI)) { // if served by central DHWSYS, msg
        // disallowed inputs
        // can't use ws_HasCentral(), ref may not be resolved yet
        rc |= disallowN("when wsCentralDHWSYS is given", DHWSYS_SSF,
                        DHWSYS_CALCMODE, DHWSYS_TSETPOINT, DHWSYS_TSETPOINTLH,
                        DHWSYS_TUSE, DHWSYS_TINLET, DHWSYS_LOADSHAREDHWSYSI, 0);
    } else if (IsSet(DHWSYS_LOADSHAREDHWSYSI)) { // if DHWSYS shares load, msg
        // disallowed inputs
        rc |= disallowN("when wsLoadShareDHWSYS is given", DHWSYS_CENTRALDHWSYSI,
                        DHWSYS_DAYUSENAME, DHWSYS_HWUSE, 0);
    }
...
{
    RC rc = RCOK;
    if (!IsSet(DHWSYS_LOADSHAREDHWSYSI)) { // don't report error if both
        // wsLoadShareDHWSYS and
        // wsCentralDHWSYS are given.
...
//-----------------------------------------------------------------------------
DHWSYS *DHWSYS::ws_GetCentralDHWSYS() const { // note: dicey for input records
    //   ws_centralDHWSYS may not be resolved
    DHWSYS *pWS = (DHWSYS *) b->GetAtSafe(ws_centralDHWSYSi);
    return pWS;
} // DHWSYS::ws_GetCentralDHWSYS
//-----------------------------------------------------------------------------
int DHWSYS::ws_IsCentralDHWSYS()
const { // note: dicey for input records
    //   ws_childSYSCount is derived in ws_Init() pass 1
#if defined(_DEBUG)
                                                                                                                            if (b == &WSiB)
    err(PERR, "ZNRES::ws_IsCentralDHWSYS '%s': called on input record", Name());
#endif
...
// called *last* from topDHW
{
    ...
    if (pass == 0) { // pass 0: init things that have no inter-DHWSYS effect
        ...
        ws_hasTestInput = IsSet(DHWSYS_TUSETEST) || IsSet(DHWSYS_TINLETTEST) ||
                          IsSet(DHWSYS_HWUSETEST) || IsSet(DHWSYS_TRLTEST) ||
                          IsSet(DHWSYS_VOLRLTEST);
        ...
        // note DHWHEATER::wh_HPWHInit() can override ws_tSetpointDes for
        // fix-setpoint HPWHs
        ...
        if (ws_wtCount > 0) {
            DHWTANK *pWT;
            RLUPC(WtR, pWT, pWT->ownTi == ss)rc |= pWT->wt_Init();
        }
        ...
        if (pWSCentral) {
            for (int iEU = 0; iEU < C_DHWEUCH_COUNT; iEU++) {
                ...
                pWSCentral->ws_fxCount[iEU] =
                        0; // counts derived from child DHWSYSs (see below)
            }
    ...
    if (pass == 2) { // final pass: all mbrs of loadShare group get identical
        // ws_loadShareCount[]
            ...
            {        // ensure that at least one end use target is included in group.
                ...
                for (int iEU = 0; iEU < C_DHWEUCH_COUNT; iEU++) {
                    if (pWS->ws_loadShareCount[iEU] < 1) {
                        if (iEU > 0)
                            pWS->oInfo("1 %s has been added for load sharing purposes -- "
                                       "none found in input.",
        ...
        ws_wrFeedWHCount =
                0; // count of child DHWHEATRECs feeding DHWHEATER cold inlet
        ...
        RLUPC(WrR, pWR, pWR->ownTi == ss) {
                ...
                if (pWR->wr_mult > 0) {
                    ...
                    ws_wrFxDrainCount +=
                            pWR->wr_nFXDrain * pWR->wr_mult; // # of fixture drains
        ...
        if (ws_wrFxDrainCount > ws_ShowerCount())
            rc |= oer("Invalid heat recovery arrangement: more DHWHEATREC drain "
                      "connections (%d) than showers (%d)",
        ...
        if (!rc && ws_ShowerCount() > 0) {
            ...
            int iFx = 0;
            RLUPC(WrR, pWR, pWR->ownTi == ss)rc |= pWR->wr_SetFXConnections(this, iFx);
            // any remaining showers are linked to "no DHWHEATREC"
        }

        // additional config checks
        if (!ws_configChecked) { // don't bother with ignore msgs if not RCOK (no
            // run)
#if 0
...
0			}
    ...
    // pass 1
    if (ws_HasCentralDHWSYS()) { // check that central DHWSYS tree is only one
        // deep
        ...
        VAccum(pWSCentral->ws_fxCount, C_DHWEUCH_COUNT, ws_fxCount,
               ws_mult); // total # of fixtures served
        ...
        if (!pWS)
            ...
            rc |= oer("DHWSYS '%s' (given by wsLoadShareDHWSYS) also specifies "
                      "wsLoadShareDHWSYS.",
                      pWS->Name());
        else { // note ws_fxCount[ 0] is 1
    ...
    if (!ws_HasDHWDAYUSEDraws()) { // no DHWDAYUSE (on this or any child): info
        // msgs re draw-related input
...

{
    ...
    if (ws_wlCount > 0)
        rc |= disallowN("when DHWSYS has DHWLOOP(s)", DHWSYS_TRLTEST,
                        DHWSYS_VOLRLTEST, 0);
...
// returns RCOK iff valid calc
{
    ...
    {
        if (Top.isBegRun || Top.tp_isBegMainSim) { // intialize run totals for all
            // child water heaters
            //  also sets up DHWHEATER -> DHWSYSRES linkage
            DHWHEATER *pWH;
            RLUPC(WhR, pWH, pWH->ownTi == ss)pWH->wh_InitRunTotals();
            RLUPC(WlhR, pWH, pWH->ownTi == ss)pWH->wh_InitRunTotals();
        }

        if (Top.tp_isBegMainSim) { // Note: DHWSYSRES 0'd in DHWBegIvl
        ...
        if (IsSet(DHWSYS_DAYUSENAME)) { // beg of day: locate DHWDAYUSE, set
            // ws_dayUsei
            if (WduR.findRecByNm1(ws_dayUseName, &ws_dayUsei, NULL))
                return orMsg(ERRRT + SHOFNLN, "DHWDAYUSE '%s' not found.",
                             ws_dayUseName.CStr());
        }
    ...
    if (IsSet(DHWSYS_TINLETTEST))
        ws_tInlet =
                ws_tInletTest; // ws_tInlet also set subhourly if ws_tInletTest provided
    ...
    static_assert(sizeof(ws_drawDurF) / sizeof(ws_drawDurF[0]) == NDHWENDUSES,
                  "ws_DrawDurF array size error");
    static_assert(sizeof(ws_drawWaste) / sizeof(ws_drawWaste[0]) == NDHWENDUSES,
                  "ws_drawWaste array size error");

    // temperature-dependent end uses
    //   losses modeled by extending draw
    float drawDurFDflt = ws_WF * ws_DLM; // can vary hourly
    static const int dhwEUList[] = {0, C_DHWEUCH_SHOWER, C_DHWEUCH_FAUCET,
                                    C_DHWEUCH_BATH, -1};
    ...
    // here we combine these to derive consistent hourly total and tick-level (1
    // min) bins
    ...
    DHWDAYUSE *pWDU =
            WduR.GetAtSafe(ws_dayUsei); // ref'd DHWDAYUSE can vary daily
    if (pWDU) { // accumulation DHWDAYUSE input to tick bins and total use
    ...
    DHWTANK *pWT;
    if (ws_wtCount > 0)
        RLUPC(WtR, pWT, pWT->ownTi == ss)rc |= pWT->wt_DoHour();
    ...
    if (ws_wlCount > 0) // if any loops
    {
        ...
        RLUPC(WlR, pWL, pWL->ownTi == ss) {
                rc |=
                        pWL->wl_DoHour(mult); // also calcs child DHWLOOPSEGs and DHWLOOPPUMPs
                HRLL += pWL->wl_HRLLnet;  // loop loss
                ws_HRBL += pWL->wl_HRBL;  // branch loss, Btu
                ws_t24WL +=
                        pWL->wl_t24WL; // T24 model branch waste loss volume, gal (info only)
    ...
    {                   // DHWPUMPs consume electricity but have no other effect
        ...
        DHWPUMP *pWP;
        RLUPC(WpR, pWP, pWP->ownTi == ss)pWP->wp_DoHour(mult);
    }
    if (ws_whCount > 0.f) {
        DHWHEATER *pWH;
        RLUPC(WhR, pWH, pWH->ownTi == ss)rc |= pWH->wh_DoHour();

        // loop heaters
        if (ws_wlhCount > 0)
            RLUPC(WlhR, pWH, pWH->ownTi == ss)rc |= pWH->wh_DoHour();
    }
...
#undef ALTDRAWCSV // define to enable alternative draw export format
    ...
    if (ws_pSizer) { // water heating requirement, Btu
        //   based on design temps (ignore solar, DWHR, )
        float loadDHW =
                ws_whUse.total * (ws_tUse - ws_tInletDes) * waterRhoCp_Btu_per_galF;
    ...
    if (ws_calcMode == C_WSCALCMODECH_PRERUN) {
        [[maybe_unused]] float drawSum =
                ws_drawMaxMS.vm_Sum(ws_whUse.total, &ws_drawMax);
        float whLoad =
                ws_whUse.total * (ws_tUse - ws_tInletX) * waterRhoCp_Btu_per_galF;
    ...
    if (ws_drawCSV == C_NOYESCH_YES && !Top.isWarmup) {
      if (ws_pFDrawCSV == NULL) { // dump file name = <cseFile>_draws_ddll.csv
        const char *nameNoWS = strDeWS(strtmp(name));
        const char *fName = strsave(
            strffix2(strtprintf("%s_%s_draws_%2.2d%2.2d", InputFilePathNoExt,
                                nameNoWS, ws_drawMaxDur, ws_loadMaxDur),
                     ".csv", 1));
      ...
      fprintf(ws_pFDrawCSV, "%d,%d,%d,%0.1f,%0.1f\n", Top.tp_date.month,
              Top.tp_date.mday, Top.iHrST + 1, drawSum, loadSum);
    }
#endif
    }
...
//   supports non-DHWUSE draws
{
    ...
    DHWHEATREC *pWR;
    RLUPC(WrR, pWR, pWR->ownTi == ss)pWR->wr_InitTicks();
...
// returns all-tick average of wtk_tInletX
{
    ...
    return whUseTot > 0.f ? tUseSum / whUseTot
...
// called iff there are DWHRs and/or events that might recover heat
{
...
#if defined(_DEBUG)
                                                                                                                                // tick energy balance
    float qXNoHR = (whUseNoHR + whUseOther) * waterRhoCp_Btu_per_galF *
                   (ws_tUse - ws_tInlet);
    float qX =
        tk.wtk_whUse * waterRhoCp_Btu_per_galF * (ws_tUse - tk.wtk_tInletX);
    float qXHR = qX + qR;
    if (frDiff(qXHR, qXNoHR, 1.f) > .001f) {
      printf("\nDHWSYS '%s': ws_DoHourDWHR tick balance error (md=%d)", Name(),
             multiDraw);
      if (nReDo++ < 2)
  ...
  float tInletX =
      ws_tInlet + ws_qDWHRWH / (waterRhoCp_Btu_per_galF * ws_whUse.total);
  ...
  if (frDiff(qXHR, qXNoHR, 1.f) > .001f)
    printf("\nDHWSYS '%s': ws_DoHourDWHR balance error (md=%d)", Name(),
           multiDraw);
#endif
...
// Done at beg of hour (after draws known) re cross refs (e.g. GAIN
// gnCtrlDHWMETER)
{
    // accumulate water use to DHWMTRs if defined
    //   include DHWSYS.ws_mult multiplier
...
//----------------------------------------------------------------------------
void DHWSYS::ws_AccumUseSingle( // accumulate single use to meter and annual
        // total
        ...
        int iTk0)               // initial tick idx for subhr
{
    ...
    DHWHEATER *pWH;
    RLUPC(WhR, pWH, pWH->ownTi == ss)rc |= pWH->wh_DoSubhrStart();

    RLUPC(WlhR, pWH, pWH->ownTi == ss)rc |= pWH->wh_DoSubhrStart();
...
RC DHWSYS::ws_DoSubhrTick(int iTk) {
    ...
    DHWTICK &tk = ws_ticks[iTk];
    tk.wtk_volIn = 0.; // inlet volume due to draws, gal
    //  (not including loop and CHDHW flow)
    //  derived during calcs when mixdown known
    ...
    float tCHDHWSupply =
            ws_GetCHDHWTSupply(); // CHDHW supply temp, 0 if not CHDHW
    DHWSYSRES *pWSR = ws_GetDHWSYSRES();
    pWSR->S.wsr_AccumTick(tk, ws_tUse, tCHDHWSupply);
...
RC DHWSYS::ws_WriteDrawCSV() // write this hour draw info to CSV
{
    if (!ws_pFDrawCSV) {
        ...
        const char *nameNoWS = strDeWS(strtmp(name));
        const char *fName = strsave(strffix2(
                strtprintf("%s_%s_draws", InputFilePathNoExt, nameNoWS), ".csv", 1));
        ws_pFDrawCSV = fopen(fName, "wt");
        if (!ws_pFDrawCSV) {
            ws_drawCSV = C_NOYESCH_NO; // don't try again
            ...
            fprintf(ws_pFDrawCSV, "Mon,Day,DOW,Hr,MinHr,MinDay,tIn (F),tInHR "
                                  "(F),tHot (F),vHot (gal)\n");
        }
    }
    ...
    int nTk = Top.tp_NHrTicks();
    for (int iTk = 0; iTk < nTk; iTk++) { // standard time?
        int iHr = Top.iHrST;                // report in standard time
        fprintf(ws_pFDrawCSV, "%d,%d,%d,%d,", Top.tp_date.month, Top.tp_date.mday,
                Top.dowh + 1, iHr + 1);
        if (nTk == 60)
...
RC DHWSYS::ws_DoSubhrEnd() {
    ...
    if (ws_CHDHWCount > 0) { // Problem: electricity use is not in phase with load
        // due to tank storage
        //   Allocate electricity use by ratio (heating output) / (DHW + heating
        //   output)
    ...
    // water heaters
    DHWHEATER *pWH;
    RLUPC(WhR, pWH, pWH->ownTi == ss)rc |= pWH->wh_DoSubhrEnd(false);

    // loop heaters
    RLUPC(WlhR, pWH, pWH->ownTi == ss)rc |= pWH->wh_DoSubhrEnd(true);
...
// called at end of hour
{
    ...
    if (ws_wlhCount > 0.f)
        RLUPC(WlhR, pWH, pWH->ownTi == ss)rc |= pWH->wh_EndIvl(ivl, 0.f, ws_mult);
    ...
    if (ivl <= C_IVLCH_D) {
        ...
        if (ivl == C_IVLCH_Y) {
            ...
            double totHARLCk = 0.;
            if (ws_whCount > 0.f)
                RLUPC(WhR, pWH, pWH->ownTi == ss)totHARLCk = pWH->wh_totHARL;
...
// call after last hour of PRERUN
{
    ...
    DHWHEATER *pWH;
    RLUPC(WhR, pWH, pWH->ownTi == ss)rc |= pWH->wh_DoEndPreRun();
    ...
    if (!ws_HasCentralDHWSYS()) // if central or stand-alone
    {
        ...
        DHWSYS *pWSChild;
        RLUPC(WsR, pWSChild, pWSChild->ws_centralDHWSYSi == ss)VAccum(ws_drawCount, NDHWENDUSES,
                                                                      pWSChild->ws_drawCount);
        ...
        for (int iEU = 1; iEU < NDHWENDUSES; iEU++) {
            if (ws_dayWasteDrawF[iEU] >
                0.f) { // average daily waste = average # of draws * waste per draw
        ...
        // draw scaling factor: cause draw waste to (approx) equal target waste per
        // day
        ...
        //   available for use in DHWHEATER sizing expressions at start of
        //   C_WSCALCMODECH_SIM
...
//   6, 12, and 24 produce similar-ish result changes; 12 seems reasonable,
//   6-Feb-2023
...

{
    ...
    if (ws_tUse < 120.f)
        rc |= oer("wsTUse (%0.1f F) must be >= 120 F when DHWSYS is used for space "
                  "heating.",
                  ws_tUse);
    ...
    RLUPC(WhR, pWH,
          pWH->ownTi ==
          ss) { // check/setup all DHWHEATERs altho they must identical
            rc |= pWH->wh_SetupAsCHDHWSource();
            if (ws_pCHDHWDHWHEATER)
                rc |= pWH->oer("Unexpected DHWHEATER.  To represent multiple DHWHEATERs "
                               "in a DHWSYS\n"
                               "    used for space heating, specify a single DHWHEATER "
                               "with whMult > 1.");
            else {
...
// return space heating hot water supply temp, F
{
    float tSupply = 0.f;
    if (ws_pCHDHWDHWHEATER)
        tSupply = max(ws_tUse, ws_pCHDHWDHWHEATER->wh_HPWH.hw_GetCHDHWTSupply());
    return tSupply;
...
//-----------------------------------------------------------------------------
void DHWSYSRES_IVL::wsr_Copy(const DHWSYSRES_IVL *s, float mult /*=1.f*/) {
    if (mult == 1.f)
...
void DHWSYSRES_IVL::wsr_AccumTick( // accum tick values
        ...
        float tCHDHWSupply /*=0.f*/)   // combined heat/DHW supply temp, F
...
//       Here tick values are accumed to subhr
{
    ...
    qOutHtg += tk.wtk_volCHDHW * waterRhoCp_Btu_per_galF *
               (tCHDHWSupply - tk.wtk_tRCHDHW);
    qLossMisc -= tk.wtk_qLossNoRL;
    qDWHR += tk.wtk_qDWHR;
    qSSF += tk.wtk_qSSF;

#if 0
                                                                                                                            printf("\n%d accum  qCHDHW=%0.4f  qWH=%0.4f  qXBU=%0.4f",
		Top.iSubhr, qCHDHW, qWH, qXBU);
#endif
...
// done once at run start
{
    ...
    if (pass == 0) {
        ...
        RLUPC(WuR, pWU,
              pWU->ownTi == ss) { // DHWUSE subscript range within this DHWDAYUSE
    ...
    for (int iWU = wdu_wuSsBeg; iWU < wdu_wuSsEnd; iWU++) {
        ...
        if (pWU->wu_eventID >
            eventIDmax[pWU->wu_hwEndUse]) { // as yet unseen eventID
            eventIDmax[pWU->wu_hwEndUse] = pWU->wu_eventID;
            pWU->wu_drawSeqN = drawSeqNNext[pWU->wu_hwEndUse]++;
        } else { // DHWUSE may be part of previously seen draw
            ...
            for (iWX = pWU->ss - 1; iWX > 0; iWX--) {
                ...
                if (pWUX && pWUX->r_status > 0 && pWUX->ownTi == ss &&
                    ...
                    pWU->wu_drawSeqN =
                            pWUX->wu_drawSeqN; // part of previous event, use same seq #
                    break;
                }
            }
            if (iWX == 0)
...
// called at end of each DHWUSE input
{
    ...
    if (IsSet(DHWUSE_HEATRECEF)) {
        ...
        rc |=
                disallowN("when wuHotF is specified", DHWUSE_TEMP, DHWUSE_HEATRECEF, 0);
...
//         else error (stop run)
{
    ...
    if (pWS->ws_loadShareCount[0] >
        1) { // if load is being shared by more than 1 DHWSYS
    ...
    if (durX > minPerDay) { // duration longer than 1 day
        // warn and limit
        rc |= orMsg(WRNRT,
                    "adjusted draw duration %0.1f min changed to maximum allowed "
                    "1440 min (24 hr)",
                    durX);
        durX = minPerDay;
    }
    ...
    if (endM > minPerDay) {    // period wraps over midnight
        //   treat as 2 non-wrapping segments
        rc |= wu_DoHour1(pWS, mult, iH, begM, min(begMHot, minPerDay), minPerDay);
        rc |= wu_DoHour1(pWS, mult, iH, 0., max(0., begMHot - minPerDay),
                         endM - minPerDay);
    } else
...
//         else error (stop run)
{
    ...
    //     1. draws that span midnight are counted twice (these are rare in
    //     typical input)
    ...
    if (iFx >= 0) {
        ...
        if (pWR) { // draw is for fixture draining via DHWHEATREC
            ...
            for (iTk = iTk0; tickBeg < endX; iTk++) {
                ...
                float fxHotTick = iTk > iTk0Hot ? fxMixTick
                                                : iTk == iTk0Hot ? fxFlow * (endXTick - begHotX)
                                                                 : 0.f;
                pWR->wr_ticks[iTk].wrtk_draws.push_back(
                        DWHRUSE(iFx, coldCnx, fxMixTick, fxHotTick, wu_temp));
    ...
    if (!IsSet(DHWUSE_TEMP))
    ...
    else {                       // use temperature is specified
        ...
        else {             // local legacy-model heat recovery available and legal
            if (wu_heatRecEF > 0.9f) { // warn and limit
...
//         errors are msg'd, fHot returned 0 or 1
{
    ...
    if (mixRet) {
        if (mixRet == -2)
            rc |= orMsg(WRNRT,
                        "Cold water temp (%0.1f F) >= hot water temp (%0.1f F).  "
                        "Cannot mix to wuTemp (%0.1f F).",
                        tCold, tHot, wu_temp);
        else if (mixRet == -1)
            rc |= orMsg(WRNRT,
                        "wuTemp (%0.1f F) < cold water temp (%0.1f F).  "
                        "Cannot mix to wuTemp.",
                        wu_temp, tCold);
        else if (mixRet == 1)
            rc |= orMsg(WRNRT,
                        "wuTemp (%0.1f F) > hot water temp (%0.1f F).  "
                        "Cannot mix to wuTemp.",
                        wu_temp, tHot);
    }
    return rc;
...
#endif
//-----------------------------------------------------------------------------
/*static*/ void HPWHLINK::hw_HPWHMessageCallback(const std::string message,
                                                 void *contextPtr) {
...
RC HPWHLINK::hw_InitResistance( // set up HPWH has EF-rated resistance heater
        [[maybe_unused]] WHRESTYCH
        resTy, // resistance heater type (currently unused)
        float vol, // tank volume, gal
        float EF,  // rated EF
...
//-----------------------------------------------------------------------------
/*static*/ int HPWHLINK::hw_HPWHInfo(WHASHPTYCH ashpTy,
...
// return HPWH preset, -1 if ashpTy not found
{
    ...
    static const WWTABLE /* { SI key, value; } */ presetTbl[] = {
            ...
            {C_WHASHPTYCH_AOSMITHSHPT50,
                                           hwatSMALL | HPWH::MODELS_GE2012}, // AO Smith SHPT models: base on GE2012
            {C_WHASHPTYCH_AOSMITHSHPT66,
                                           hwatSMALL | HPWH::MODELS_GE2012}, //  caller must modify UA and vol
            ...
            {C_WHASHPTYCH_GE2014STDMODE_80,
                                           hwatSMALL | HPWH::MODELS_GE2014STDMode_80},
            ...
            {C_WHASHPTYCH_RHEEM2020BUILD40,
                                           hwatSMALL | HPWH::MODELS_Rheem2020Build40},
            {C_WHASHPTYCH_RHEEM2020BUILD50,
                                           hwatSMALL | HPWH::MODELS_Rheem2020Build50},
            {C_WHASHPTYCH_RHEEM2020BUILD65,
                                           hwatSMALL | HPWH::MODELS_Rheem2020Build65},
            {C_WHASHPTYCH_RHEEMHBDRBUILD80,
                                           hwatSMALL | HPWH::MODELS_Rheem2020Build80},

            {C_WHASHPTYCH_RHEEMPLUGINSHARED40,
                                           hwatSMALL | HPWH::MODELS_RheemPlugInShared40},
            {C_WHASHPTYCH_RHEEMPLUGINSHARED50,
                                           hwatSMALL | HPWH::MODELS_RheemPlugInShared50},
            {C_WHASHPTYCH_RHEEMPLUGINSHARED65,
                                           hwatSMALL | HPWH::MODELS_RheemPlugInShared65},
            {C_WHASHPTYCH_RHEEMPLUGINSHARED80,
                                           hwatSMALL | HPWH::MODELS_RheemPlugInShared80},

            {C_WHASHPTYCH_RHEEMPLUGINDEDICATED40,
                                           hwatSMALL | HPWH::MODELS_RheemPlugInDedicated40},
            {C_WHASHPTYCH_RHEEMPLUGINDEDICATED50,
                                           hwatSMALL | HPWH::MODELS_RheemPlugInDedicated50},
            ...
            {C_WHASHPTYCH_WORSTCASEMEDIUM,
                                           hwatSMALL | HPWH::MODELS_UEF2generic}, // alias (testing aid)
            {C_WHASHPTYCH_AWHSTIER3GENERIC40,
                                           hwatSMALL | HPWH::MODELS_AWHSTier3Generic40},
            {C_WHASHPTYCH_AWHSTIER3GENERIC50,
                                           hwatSMALL | HPWH::MODELS_AWHSTier3Generic50},
            {C_WHASHPTYCH_AWHSTIER3GENERIC65,
                                           hwatSMALL | HPWH::MODELS_AWHSTier3Generic65},
            {C_WHASHPTYCH_AWHSTIER3GENERIC80,
                                           hwatSMALL | HPWH::MODELS_AWHSTier3Generic80},

            {C_WHASHPTYCH_AQUATHERMAIRE,   hwatSMALL | HPWH::MODELS_AquaThermAire},

            // large
            {C_WHASHPTYCH_SANCO2_GS3,
                                           hwatLARGE | HPWH::MODELS_SANCO2_GS3_45HPA_US_SP},
            ...
            {C_WHASHPTYCH_MITSU_QAHVN136TAUHPB_SP,
                                           hwatLARGE | HPWH::MODELS_MITSUBISHI_QAHV_N136TAU_HPB_SP},
            ...
            {C_WHASHPTYCH_RHEEM_HPHD60HNU_201_MP,
                                           hwatLARGE | HPWH::MODELS_RHEEM_HPHD60HNU_201_MP},
            {C_WHASHPTYCH_RHEEM_HPHD60VNU_201_MP,
                                           hwatLARGE | HPWH::MODELS_RHEEM_HPHD60VNU_201_MP},
            {C_WHASHPTYCH_RHEEM_HPHD135HNU_483_MP,
                                           hwatLARGE | HPWH::MODELS_RHEEM_HPHD135HNU_483_MP},
            {C_WHASHPTYCH_RHEEM_HPHD135VNU_483_MP,
                                           hwatLARGE | HPWH::MODELS_RHEEM_HPHD135VNU_483_MP},
            ...
            {C_WHASHPTYCH_AWHSTIER4GENERIC40,
                                           hwatSMALL | HPWH::MODELS_AWHSTier4Generic40},
            {C_WHASHPTYCH_AWHSTIER4GENERIC50,
                                           hwatSMALL | HPWH::MODELS_AWHSTier4Generic50},
            {C_WHASHPTYCH_AWHSTIER4GENERIC65,
                                           hwatSMALL | HPWH::MODELS_AWHSTier4Generic65},
            {C_WHASHPTYCH_AWHSTIER4GENERIC80,
                                           hwatSMALL | HPWH::MODELS_AWHSTier4Generic80},
...
// returns RCOK iff success
{
    ...
    int attrs;
    HPWH::MODELS preset =
            HPWH::MODELS(hw_HPWHInfo(ashpTy, attrs)); // HPWH "preset"
    ...
    if (ashpTy ==
        ...
        UAX = 2.9f;
               ...
               C_WHASHPTYCH_AOSMITHSHPT66) { // preset = HPWH::MODELS_GE2012;
        volX = 63.9f;
        UAX = 3.4f;
    } else if (ashpTy ==
               C_WHASHPTYCH_AOSMITHSHPT80) { // preset = HPWH::MODELS_GE2012;
        volX = 80.7f;
        UAX = 4.7f;
    }
        ...
        float inHtLoopRet)        // loop return fractional height, -1 = don't set
{
    ...
    try {
        ...
        hw_tankHCNominal = HPWH::Energy_t(40. * HPWH::DENSITYWATER_kg_per_L *
    ...
    try {
        if (hw_pHPWH->hasACompressor()) {
            ...
            for (int iRE = 0; iRE < nRE; iRE++) {
                double capx = hw_pHPWH->getResistanceCapacity(iRE)(Units::Btu_per_h);
                if (capx > cap)
                    cap = capx;
            }
        }
...
//-----------------------------------------------------------------------------
RC HPWHLINK::hw_DeriveVolFromVolRunning( // calc required volume from running
        // vol
        float volRunning,                    // required running volume, gal
...
{
    return hw_pHPWH->getTankSize()(Units::gal);
...
// returns RCOK iff success
{
    ...
    try {
        if (!hw_pHPWH->isSetpointFixed()) {
            ...
            bool bSPP = hw_pHPWH->isNewSetpointPossible({tSetpoint, Units::F},
                                                        tSetpointMax, whyNot);
...
// returns RCOK iff success
{
    ...
    // hw_fMixUse, hw_fMixRL: initialized in wh_InitRunTotals(); value retained
    // hour-to-hour
    ...
    hw_tankHCBeg = hw_tankHCEnd > 0. ? hw_tankHCEnd
        ...
        int drStatus /*=0*/) // demand response control signal
{
    ...
    //    DHW = drawUse (water replaced from mains with possible solar and/or
    //    DWHR) DHW recirc loop CHDHW (combined heat and DHW) Losses (extra draw
    //    to compensate for distribution losses)
    ...
    // combined recirc
    double drawRC =
            drawRL + drawCHDHW; // recirc(loop + CHDHW) total flow vol for tick, gal
    ...
    double drawLoss{
            0.}; // pseudo-draw (gal) to represent e.g. central system branch losses
#if 0
                                                                                                                            if (Top.tp_isBegMainSim)
		printf("\nBeg");
#endif
    if (whfcn & DHWHEATER::whfcnSUPPLIESLOAD) { // mixdown: DHW and loop draws are
        // reduced based
        ...
        drawLoss = tk.wtk_qLossNoRL * scaleX /
                   (waterRhoCp_Btu_per_galF * max(1., tMix - tMains));
    ...
    HPWH::PowerVect_t powerV;
    HPWH::PowerVect_t *powerV_p = NULL;
    if (tk.wtk_qTX > 0.f) // ignore tank "cooling"
    {
        ...
        powerV = {hw_pNodePowerExtra_W, Units::W};
        powerV_p = &powerV;
    }

    try {
        hw_pHPWH->runOneStep(
                {tInlet, Units::F},        // inlet temp
                {drawForTick, Units::gal}, // draw volume
                {hw_tEx, Units::F},        // ambient T (=tank surround)
                {hw_tASHPSrc, Units::F},   // heat source T
        ...
        hw_tOut = hw_pHPWH->getOutletT()(Units::C); // output temp, (0 if no draw)
...
#endif
    if (hw_tOut < .01) { // no draw / outlet temp estimated
        hw_tOutCHDHW = 0.;
        ...
        if (whfcn & DHWHEATER::whfcnSUPPLIESLOAD) { // output goes to load
            if (tOutF <
                tMix) { // load not met, add additional (unlimited) resistance heat
                hw_fMixUse = hw_fMixRL = 1.f;
        ...
        double qHWTick = HPWH::Energy_t(
                (drawForTick * hw_tOut - (drawForTick - drawRC) * DegFtoC(tInlet) -
                 drawRC * DegFtoC(tRC)) /
                gal_per_L * HPWH::DENSITYWATER_kg_per_L * HPWH::CPWATER_kJ_per_kgC,
                Units::kJ)(Units::kWh);
    ...
    // accumulate by primary (= compressor or all resistance) [ 0] vs backup
    // resistance [ 1]
    for (int iHS = 0; iHS < hw_HSCount; iHS++) {
        hw_inElec[hw_HSMap[iHS]] +=
                hw_pHPWH->getNthHeatSourceEnergyInput(iHS)(Units::kWh);
        hw_heatAdded[hw_HSMap[iHS]] +=
                hw_pHPWH->getNthHeatSourceEnergyOutput(iHS)(Units::kWh);
#if 0 && defined(_DEBUG)
                                                                                                                                // debug aid
    ...
    int hpwhOptions =
            dumpUx == UNSYSIP ? HPWH::CSVOPT_IPUNITS : HPWH::CSVOPT_NONE;
    ...
    if (hw_bWriteCSV) {
        ...
        CSVItem CI[] = {
                ...
                UNTEMP, 5,
                "tDWHR", tk.wtk_tInletX,
                UNTEMP, 5,
                "tRL", drawRL > 0. ? tRL : CSVItem::ci_UNSET,
                UNTEMP, 5,
                "tRC", drawRC > 0. ? tRC : CSVItem::ci_UNSET,
                UNTEMP, 5,
                "tIn", tInlet > 0. ? tInlet : CSVItem::ci_UNSET,
                UNTEMP, 5,
                "tSP", hw_pHPWH->getSetpointT()(Units::F),
                UNTEMP, 5,
                ...
                "qTX", tk.wtk_qTX,
                UNENERGY3, 5,
                "qEnv", hw_pHPWH->getEnergyRemovedFromEnvironment()(Units::Btu),
                UNENERGY3, 5,
                "qLoss", hw_pHPWH->getStandbyLosses()(Units::Btu),
                UNENERGY3, 5,
                "XBU", HPWHxBU,
                UNENERGY3, 5,
                NULL};
        ...
        if (!hw_pFCSV->is_open()) {
            ...
            const char *fName = strffix2(
                    strtprintf("%s_%s_hpwh", InputFilePathNoExt, nameNoWS), ".csv", 1);
            hw_pFCSV->open(fName, std::ifstream::out); // implies truncation
            if (!hw_pFCSV->is_open())
                err(PWRN, "HPWH report failure for '%s'", fName);
            else { // headings
                *hw_pFCSV << strtprintf("%s,%s,%s\n", hw_pOwner->GetDescription(),
                                        Top.repHdrL.CStr(), Top.runDateTime.CStr());
                *hw_pFCSV << strtprintf("%s%s %s %s HPWH %s\n", Top.tp_RepTestPfx(),
                                        ProgName, ProgVersion, ProgVariant,
                ...
                *hw_pFCSV << strtprintf("minYear,draw( L)\n");
                ...
                // hw_pHPWH->WriteCSVHeading(hw_pFCSV, s.c_str(), nTCouples,
                // hpwhOptions);
#endif
            }
        }
...
#else
            WStr s = strtprintf("%d,%d,%d,", Top.tp_date.month, Top.tp_date.mday,
                                Top.iHr + 1);
        ...
        ZNR *pZnASHPSrc)        // ASHP heat source zone, NULL if none
{
    ...
    if (hw_nzDrawCount)
        hw_tHWOut =
                hw_tHWOutF / hw_nzDrawCount; // note: >= ws_tUse due to unlimited XBU
    ...
    if (pZnASHPSrc && hw_qEnv > 0.) { // heat extracted from zone
    ...
    hw_tankHCEnd =
            hw_pHPWH->getTankHeatContent()(Units::kWh); // end-of-step heat content
        ...
        { // energy balance error
            ...
            if (hw_balErrCount <= HWBALERRCOUNTMAX || fBal > 0.01) {
                hw_pOwner->orWarn("HPWH energy balance error (%1.6f kWh  f=%1.6f)%s",
                                  ...
                                  ? "\n    Skipping further messages for minor "
...
/*virtual*/ void DHWHEATER::Copy(const record *pSrc, int options /*=0*/) {
    ...
    new(&wh_HPWH.hw_pNodePowerExtra_W) std::vector<double>(
            ((const DHWHEATER *) pSrc)->wh_HPWH.hw_pNodePowerExtra_W);
} // DHWHEATER::Copy
//---------------------------------------------------------------------------
...
// called at end of each DHWHEATER input
{
    ...
    const char *whenTyHs =
            strtprintf("when whType=%s and whHeatSrc=%s", whTyTx, whHsTx);
    ...
    if (whfcn == whfcnLOOPHEATER && !wh_CanHaveLoopReturn())
        rc |=
                oer("DHWLOOPHEATER must have whHeatSrc=ASPX or whHeatSrc=ResistanceX.");

    if (wh_type != C_WHTYPECH_STRGSML &&
        wh_type != C_WHTYPECH_BUILTUP) { // wh_type: _STRGLRG, _INSTSML, _INSTLRG,
        // _INSTUEF
        if (wh_heatSrc == C_WHHEATSRCCH_ASHP || wh_heatSrc == C_WHHEATSRCCH_ASHPX ||
            wh_heatSrc == C_WHHEATSRCCH_ELRESX)
            rc |= ooer(DHWHEATER_HEATSRC, "whHeatSrc=%s is not allowed %s", whHsTx,
                       whenTy);

        ignoreN(whenTy, DHWHEATER_LDEF, DHWHEATER_ASHPRESUSE, DHWHEATER_RESHTPWR,
                DHWHEATER_RESHTPWR2, DHWHEATER_TANKCOUNT, 0);

        if (wh_type == C_WHTYPECH_INSTUEF) {
            if (IsSet(DHWHEATER_HEATSRC) && wh_heatSrc != C_WHHEATSRCCH_FUEL)
                rc |=
                        ooer(DHWHEATER_HEATSRC,
                             "whHeatSrc=%s is not allowed %s (use whHeatSrc=Fuel or omit)",
                             whHsTx, whenTy);
            wh_heatSrc = C_WHHEATSRCCH_FUEL;
            rc |= requireN(whenTy, DHWHEATER_UEF, DHWHEATER_RATEDFLOW,
                           DHWHEATER_ANNUALFUEL, DHWHEATER_ANNUALELEC, DHWHEATER_EFF,
                           0);
            ignoreN(whenTy, DHWHEATER_EF, 0);
            if (wh_UEF > wh_eff)
                rc |= oer("whEff (%0.3f) must be >= whUEF (%0.3f)", wh_eff, wh_UEF);
            ...
            rc |= require(whenTy, DHWHEATER_EF);
               ...
               C_WHHEATSRCCH_ASHPX) { // STRGSML or BUILTUP HPWH model
        // TODO: more specific checking for ASHPX
        ignoreN(whenHs, DHWHEATER_LDEF, DHWHEATER_RESHTPWR, DHWHEATER_RESHTPWR2,
                DHWHEATER_RESTY, 0);
        RC rc1 = requireN(whenHs, DHWHEATER_ASHPTY, 0);
        rc |= rc1;
        if (!rc1) {
            int reqdAttr = wh_type == C_WHTYPECH_BUILTUP ? HPWHLINK::hwatLARGE
                                                         : HPWHLINK::hwatSMALL;
            const char *whAshpTyTx = getChoiTx(DHWHEATER_ASHPTY, 1);
            if (!wh_HPWH.hw_IsAttr(wh_ashpTy, reqdAttr))
                rc |= ooer(DHWHEATER_ASHPTY, "whASHPType=%s not supported %s",
                           whAshpTyTx, whenTy);
            else if (wh_ashpTy == C_WHASHPTYCH_GENERIC)
                rc |=
                        requireN("when whASHPType=Generic", DHWHEATER_EF, DHWHEATER_VOL, 0);
            else
        ...
        if (IsSet(DHWHEATER_ASHPTSRC))
            rc |=
                    disallowN("when 'whASHPSrcT' is specified", DHWHEATER_ASHPSRCZNTI, 0);
        else { // default ASHP src from heater location zone
            //   (heat source is typically heater location zone)
            // wh_ashpSrcZnTi = wh_znTi done in wh_Init() (after deferred ref
            // resolution)
            CSE_V wh_ashpTSrc = CSE_V wh_tEx; // default ashpTSrc to tEx
            //   VD handles NANDLES
        }
    } else if (wh_heatSrc == C_WHHEATSRCCH_ELRESX) { // small storage electric
        // resistance (HPWH model)
        ignoreN(whenHs, DHWHEATER_LDEF, DHWHEATER_ASHPTY, DHWHEATER_ASHPTSRC,
                DHWHEATER_ASHPSRCZNTI, DHWHEATER_ASHPRESUSE, 0);
        ...
        if (wh_EF == 1.f) { // special case: "ideal" behavior (no losses)
            ignore(strtprintf("%s and whEF=1", whenTy), DHWHEATER_LDEF);
            wh_LDEF = 1.;
        } else { // either LDEF required or EF + prerun
            if (!bIsPreRun)
                rc |= require(strtprintf("%s and DHWSYS is not PreRun", whenTy),
                              DHWHEATER_LDEF);
            if (IsSet(DHWHEATER_LDEF))
                ignore(strtprintf("%s and whLDEF is given", whenTy), DHWHEATER_EF);
            else if (bIsPreRun)
                rc |= require(strtprintf("%s and whLDEF is not given", whenTy),
                              DHWHEATER_EF);
        }
    }

    if (wh_IsHPWHModel()) { // need sufficient info to determine tank UA
    ...
    if (wh_IsStorage()) { // note wh_vol is required in some cases
        //   see above
...
//        -1: maybe (re HPWH pending HPWHLINK setup)
{
    int ret = 0;
    if (wh_IsHPWHModel()) {
        ret = wh_HPWH.hw_pHPWH ? wh_HPWH.hw_pHPWH->isHPWHScalable() : -1;
    }
    // else
    //	ret = 0;
...
//-----------------------------------------------------------------------------
int DHWHEATER::wh_CanSetVolFromVolRunning()
const // can volume be derived from volRunning
...
//-----------------------------------------------------------------------------
RC DHWHEATER::wh_SetupAsCHDHWSource() // check / setup as CHDHW space heating
// source
...
RC DHWHEATER::wh_Init() // init for run
{
    ...
    if (wh_CanHaveLoopReturn() &&
        pWS->ws_calcMode ==
        C_WSCALCMODECH_SIM) { // no info msgs on PRERUN -- else duplicates
        if (pWS->ws_wlCount == 0) {
            ignore("when DHWSYS includes no DHWLOOP(s).", DHWHEATER_INHTLOOPRET);
            if (wh_IsLoopHeater())
                oInfo("modeled as a series heater because DHWSYS includes no "
                      "DHWLOOP(s).");
    ...
    if (!IsSet(DHWHEATER_ASHPTSRC) && !IsSet(DHWHEATER_ASHPSRCZNTI))
        wh_ashpSrcZnTi =
                wh_znTi; // default ASHP source zone = DHWHEATER location zone
...
// returns whfcnXXXX
{
    wh_fcn = b == &WhR || b == &WHiB ? whfcnPRIMARY
...
//           -1.f iff error (inapplicable wh_type, )
{
    ...
    static const float LDtab[][6] = {
            //       a         b          c          d        e    max
            {-0.098311f, 0.240182f, 1.356491f, -0.872446f, 0.946f,
                                                                   .90f},                                                   // storage gas
...
RC DHWHEATER::wh_DoHour() // DHWHEATER hour calcs
{
    ...
    if (pWS->ws_tOutPrimLT == 0.f)
        pWS->ws_tOutPrimLT =
                tSetpoint; // initial guess for primary heater outlet temp
    //   meaningful for HPWH only?

    if (wh_IsHPWHModel()) {
        rc |=
                wh_HPWH.hw_DoHour(tSetpoint,         // set point, F
...
// DHWHEATER subhour models accum to wh_inElec, wh_inElecBu, wh_inElecXBU, and
// wh_inFuel
...

{
    ...
    if (ivl == C_IVLCH_Y) { // definition of "unmet" depends on heater specifics
        //   generally unexpected due to XBU
        if (wh_unMetHrs > 0)
            warn("%s: Output temperature too low during %d hours of run.", objIdTx(),
                 wh_unMetHrs);
    }
...
/*virtual*/ void DHWHEATER::ReceiveRuntimeMessage(const char *msg) {
    const char *whenMsg =
            Top.When(C_IVLCH_S); // date/time or "" if not simulating
    pInfo("%s: HPWH message%s:\n  %s", objIdTx(),
          whenMsg[0] ? strtprintf(" (%s)", whenMsg) : "", msg);
...
// returns RCOK on success
{
    ...
    if (wh_heatSrc == C_WHHEATSRCCH_ELRESX) { // resistance tank (no preset)
        ...
        rc |= wh_HPWH.hw_InitResistance(wh_resTy, wh_vol, wh_EF, insulR,
                                        wh_resHtPwr, wh_resHtPwr2);
        // bVolMaybeModifiable = true;
    } else if (wh_ashpTy == C_WHASHPTYCH_GENERIC) { // generic HPWH (no preset)
    ...
    if (!rc && IsSet(DHWHEATER_HEATINGCAP)) { // check whether heating capacity
        // can be adjusted
    ...
    if (!rc && bVolMaybeModifiable) {
        ...
        if (what) {
            ...
            if (wh_HPWH.hw_pHPWH->isTankSizeFixed())
                oInfo("%s is ignored when whASHPType=%s (tank volume is fixed at %0.0f "
                      "gal)",
                      what, getChoiTx(DHWHEATER_ASHPTY, 1),
                      wh_HPWH.hw_pHPWH->getTankSize()(Units::gal));
            else if (IsSet(DHWHEATER_VOLRUNNING)) { // semi-redundant check
                if (!wh_CanSetVolFromVolRunning())
                    oInfo("%s is ignored (tank volume is fixed at %0.0f gal)", what,
                          wh_HPWH.hw_pHPWH->getTankSize()(Units::gal));
                else {
                    RC rc2 = wh_HPWH.hw_DeriveVolFromVolRunning(
                            wh_volRunning, wh_heatingCap,
                            pWS->ws_tSetpointDes - pWS->ws_tInletDes, vol);
                    if (rc2)
                        rc |= err(PERR, "DHWHEATER::wh_HPWHInit: "
                                        "hw_CanSetVolFromVolRunning() inconsistency.");
                }
    ...
    if (!rc) { // tank inlet fractional heights
    ...
    if (!rc && pWS->ws_drMethod == C_DHWDRMETH_SOC &&
        wh_IsPrimary()) { // "State of Charge" controls
        //   compressor operation controlled by tank heat content
        //      compared to scheduled target
        if (!wh_HPWH.hw_pHPWH->canUseSoCControls()) {
            rc |= oer("'%s' does not support StateOfCharge controls", wh_desc);
        } else {
            double tMin = 110.f;
            try {
    ...
    if (!rc && !pWS->ws_configChecked) {
        if (wh_HPWH.hw_IsSetpointFixed()) {
            ...
            if (fn)
                pWS->ignore(strtprintf("-- HPWH '%s' has a fixed setpoint.", Name()),
                            fn);

            // force consistent ws_tSetpointDes
            float tspFixed = wh_HPWH.hw_pHPWH->getSetpointT()(Units::F);
...
static const DRMAP drMap[] = {C_DHWDRSIG_ON,
                              HPWH::DR_ALLOW,
                              C_DHWDRSIG_TOO,
                              HPWH::DR_TOO,
                              C_DHWDRSIG_TOOLOR,
                              HPWH::DR_TOO | HPWH::DR_LOR,
                              C_DHWDRSIG_TOOLOC,
                              HPWH::DR_TOO | HPWH::DR_LOC,
                              C_DHWDRSIG_TOT,
                              HPWH::DR_TOT,
                              C_DHWDRSIG_TOTLOR,
                              HPWH::DR_TOT | HPWH::DR_LOR,
                              C_DHWDRSIG_TOTLOC,
                              HPWH::DR_TOT | HPWH::DR_LOC,
                              C_DHWDRSIG_LOC,
                              HPWH::DR_LOC,
                              C_DHWDRSIG_LOR,
                              HPWH::DR_LOR,
                              C_DHWDRSIG_LOCLOR,
                              HPWH::DR_LOC | HPWH::DR_LOR,
                              -1,
                              -1};
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
/* static*/ int
DHWHEATER::wh_DRMapSigToDRStatus(DHWDRSIG drSig) // CSE DR choice value
//   -1: validate table
...
RC DHWHEATER::wh_DoSubhrStart() {
    ...
    else if (wh_IsInstUEFModel()) { // carry-forward: instantaneous heaters
        // throttle flow to maintain temp.
    ...
    } else {                  // T24DHW efficiency-based models
        wh_effSh = wh_type == C_WHTYPECH_INSTSML ? wh_EF * 0.92f
                                                 : wh_type == C_WHTYPECH_INSTLRG ? wh_eff * 0.92f
                                                                                 : wh_type == C_WHTYPECH_STRGSML
                                                                                   ? wh_LDEF
                                                                                   : wh_type == C_WHTYPECH_STRGLRG
                                                                                     ? wh_eff
                                                                                     : -1.f;

        if (wh_effSh <= 0.f)
            rc |= err(ERR, "%s, %s: Invalid water heater efficiency %0.3f", objIdTx(),
                      Top.When(C_IVLCH_S), wh_effSh);
    }
...
//   other: = ws_tUse
{
    ...
    if (wh_IsHPWHModel()) { // demand response (DR)
        ...
        rc |= wh_HPWH.hw_DoSubhrTick(tk, wh_fcn, tInletWH, wh_tHWOutNoMix,
                                     drawForTick, scaleWH, pWS->ws_tUse,
                                     pWS->ws_tInlet, drStatus);
        ...
        drawForTick =
                tk.wtk_DrawTot(pWS->ws_tUse, tInletWH, pWS->ws_tInlet, tInletMix) *
                scaleWH;
        ...
        if (wh_IsInstUEFModel())
            ...
            float HARL = drawForTick * waterRhoCp_Btu_per_galF *
                         deltaT; // load on this heater, Btu

            wh_qHW += HARL; // output = load

            float WHEU = HARL / wh_effSh +
                         wh_SBL * Top.tp_tickDurHr; // current tick energy use, Btu

            // electricity / fuel consumption for this DHWHEATER (no multipliers)
            wh_inElecSh +=
                    wh_parElec * Top.tp_tickDurHr * BtuperWh; // electric parasitic
            ...
            if (wh_IsElec())
                ...
                wh_inFuelSh +=
                        WHEU +
                        wh_pilotPwr * Top.tp_tickDurHr; // pilot included in all fuel types
            //   (but not elec WH)

            // Note: adjustment factors wh_fAdjElec and wh_fAdjFuel are applied in
            // wh_DoSubhrEnd
        }
    }
    ...
    if (wh_IsPrimary()) {
        ...
        float dhwLoadTk1 =
                drawWH * waterRhoCp_Btu_per_galF * (pWS->ws_tUse - pWS->ws_tInlet);
        pWSR->S.qOutDHW += dhwLoadTk1;

        float dhwLoadTk2 = drawWH * pWS->ws_mult * waterRhoCp_Btu_per_galF *
                           (pWS->ws_tUse - tk.wtk_tInletX);
        pWS->ws_SSFAnnualReq += dhwLoadTk2;

        if (pWS->ws_pDHWSOLARSYS) { // accumulate solar draws for next tick
            float drawSolarSys =
                    tk.wtk_volIn; // draw from solar: does not include loop flow
            if (drawSolarSys > 0.f)
                rc |= pWS->ws_pDHWSOLARSYS->sw_TickAccumDraw(
                        pWS, drawSolarSys * scaleWH * wh_mult * pWS->ws_mult,
...
// returns RCOK iff simulation should continue
{
    ...
    if (wh_IsHPWHModel()) {
        ...
        wh_pResSh->qStorage[iLH] +=
                wh_mult * (wh_HPWH.hw_tankHCBeg - wh_HPWH.hw_tankHCEnd) * Btu_per_kWh;
        wh_pResSh->qError[iLH] -= wh_mult * wh_HPWH.hw_qBal * Btu_per_kWh;

        if (bIsLH)
            wh_pResSh->qXBUDHW +=
                    wh_qXBU * wh_mult; // loop heater never serves heating
            //   all XBU assigned to DHW
        else { // if primary heater, allocate XBU per htg/dhw ratio
        ...
        // electricity use
        wh_inElecSh =
                (wh_HPWH.hw_inElec[0] + wh_parElec * Top.tp_subhrDur) * Btu_per_kWh;
        ...
        if (wh_IsInstUEFModel()) {
            ...
            double rcovElec =
                    wh_operElec * wh_nzDrawCount *
                    Top.tp_tickDurHr; // assume operation for entire tick with any draw
            // double startElec = wh_cycLossElec * nTickStart;	// unused in
            // revised model standby in ticks w/o draw
            double stbyElec = wh_stbyElec * (Top.tp_nSubhrTicks - wh_nzDrawCount) *
                              Top.tp_tickDurHr;
            ...
            wh_inElecSh += rcovElec /*+ startElec*/ +
                           (stbyElec + wh_parElec * Top.tp_tickDurHr) * BtuperWh;
    ...
    if (wh_pMtrElec) {
        ...
        float multDHW = mult;
        float multBU = mult;
        if (wh_fcn & whfcnSUPPLIESCHDHW) { // Problem: electricity use is not in
            // phase with load due to tank storage
            //   Allocate electricity use by ratio (CHDHW heat output) / (total heat
            //   output)
            //      Primary: per recent ws_CHDHWHistoryHours (currently 12)
            //      Backup: per current current subhour
            ...
            mtrH.htg +=
                    mult * (pWS->ws_CHDHWHtgFractAvg * wh_inElecSh +
...
//--------------------------------------------------------------------------------------
RC DHWHEATER::wh_InstUEFInit() // one-time setup for UEF-based instantaneous
// model
// returns RCOK on success
//    else other -- do not run
{
    ...
    static const UEFPARAMS UEFParams[] = {
            // size	       flowMax  flowRE	      A         B      qOutUEF    qOutRE
            // durRE  honUEF
            {/*very small*/ 0.f,  1.f,  10.f,     3.62150f, 5452.f,  1090.f,  2.000f,
                                                                                      0.1667f},
            {/*low*/        1.7f, 1.7f, 22.3529f, 5.78215f, 20718.f, 8178.f,  8.824f,
                                                                                      0.4275f},
            {/*medium*/     2.8f, 1.7f, 32.3529f, 6.30610f, 29987.f, 8178.f,  8.824f,
                                                                                      0.5941f},
    ...
    double Pe = (wh_annualElec * 1000. / 365. - wh_stbyElec * (24. - UP.honUEF)) /
                UP.honUEF;
    if (Pe < 0)
        rc |= oer("Inconsistent input for whAnnualElec (=%.2f kWh) and whStbyElec "
                  "(=%.2f W).\n"
                  "    Derived operating electrical power is < 0.",
                  wh_annualElec, wh_stbyElec);

    float P = (UP.qOutUEF / wh_UEF - UP.B * UP.qOutRE / wh_eff) /
              (UP.A - UP.B * UP.durRE);
    ...
    wh_loadCFwdMax =
            wh_loadCFwdF * wh_ratedFlow * waterRhoCp_Btu_per_galF * 67. * 60.;
...

{
    ...
    if (draw > 0.) {
        ...
        if (deltaT > 0.f) {
            float fLoad;
            double drawFullLoad =
                    wh_maxFlowX / deltaT; // max vol that can be heated in this tick
            if (draw > drawFullLoad) {
                ...
                if (wh_loadCFwd > wh_loadCFwdMax) {
                    wh_qXBU += wh_loadCFwd -
                               wh_loadCFwdMax; // excess heating required to meet ws_tUse
...
RC DHWTANK::RunDup( // copy input to run record; check and initialize
        const record *pSrc, int options /*=0*/) {
    RC rc = record::RunDup(pSrc, options);
    DHWSYS *pWS = wt_GetDHWSYS();
...
// provides default iff wt_tTank not set
{
    ...
    float tTank = IsSet(DHWTANK_TTANK) ? wt_tTank : tUse;
...
// called at end of each DHWHEATREC input
{
    ...
    if (wr_hwEndUse != C_DHWEUCH_SHOWER)
        rc |= ooer(DHWHEATREC_HWENDUSE,
                   "wrHWEndUse=%s not supported (must be Shower)",
                   getChoiTx(DHWHEATREC_HWENDUSE));

    if (wr_nFXDrain <= 0) { // no drain connections -- treat as not present
        wr_nFXCold = 0;
    ...
    if (!IsSet(DHWHEATREC_NFXCOLD))
        ...
        rc |= oer("Invalid configuration: wrCountFXCold (%d) must be <= "
                  "wrCountFXDrain (%d)",
                  wr_nFXCold, wr_nFXDrain);
...
//----------------------------------------------------------------------------
RC DHWHEATREC::wr_SetFXConnections(DHWSYS *pWS, // parent DHWSYS
                                   int &iFx)
// returns RCOK iff success
{
    RC rc = RCOK;
...
float DHWHEATREC::wr_CalcTick( // calculate performance for 1 tick
        ...
        float &qRWH)     // returned updated: tick recovered heat added to WH inlet
// water, Btu
...
//     (not including vHotOther)
{
    ...
    //    >> DHWHEATER inlet flow for other draws assumed to flow equally via
    //    parallel paths
    //   this-DHWHEATREC fixture flows are assigned to this-DHWHEATREC potable
    //   flow
    ...
    for (iD = 0; iD < nD; iD++) {
        ...
        tdI += hru.wdw_volHR *
               (hru.wdw_temp - wr_tdInDiff) // hot vol (apply tdInDrain)
               + (hru.wdw_vol - hru.wdw_volHR) *
                 wr_tdInWarmup; // warmup vol (no tdInDiff)

        // hot water use at fixture if mixed with mains cold
        float vHotNoHR1 = hru.wdw_vol * DHWMixF(hru.wdw_temp, tHotFX, tpI);
    ...
    if (vMixFXHR > 0.f) // if any current draw feeds a fixture
    {
        ...
        int iL;
        for (iL = 0; iL < 10; iL++) { // cold water temp at wdw_coldCnx fixture(s)
    ...
    } else {             // no current fixture draw uses tempered cold-side water
        //   all flows known
        vHotFX = vHotFX0;   // hot water needed
        if (wr_FeedsWH()) { // potable side feeds water heater
            //    recovered heat boosts tInlet
            vp = vHotFX + vHotOther;
            wr_EffAdjusted(vp, tpI, vd, tdI); // derive wr_eff
            tpO = wr_HX(vp, tpI, vd, tdI);
        } else { // does not feed WH
            //   no heat recovered
...
// sets and returns wr_eff
{
    if (wr_type == C_DWHRTYCH_SETEF)
        ...
        double fV = -6.98484455e-4 * v4 + 1.28561447e-2 * v3 - 7.02399803e-2 * v2 +
                    1.33657748e-2 * v + 1.23339312;
...
//-----------------------------------------------------------------------------
  ...
  static float vdX[] = {0.f, 0.1f, 0.3f, 0.5f, 1.f,  1.5f, 2.f,  2.5f,
                        3.f, 4.f,  7.f,  10.f, 13.f, 16.f, 20.f, -1.f};
  static float tdIX[] = {100.f, -1.f};
  static float vpX[] = {0.f, 0.1f, 0.3f, 0.5f, 1.f,  1.5f, 2.f,  2.5f,
                        3.f, 4.f,  7.f,  10.f, 13.f, 16.f, 20.f, -1.f};
    ...
    for (int iTpI = 0; tpIX[iTpI] >= 0.f; iTpI++)
      for (int iVd = 0; vdX[iVd] >= 0.f; iVd++)
        for (int iVp = 0; vpX[iVp] >= 0.f; iVp++) {
          ...
          // float fX = effAdjustedX(vpX[iVp], tpIX[iTpI], vdX[iVd],
          // tdIX[iTdI]);
...
RC DHWLOOP::wl_Init() // DHWLOOP init for run
{
    ...
    RLUPC(WgR, pWG, pWG->ownTi == ss) {
            ...
            DHWLOOPBRANCH *pWB;
            RLUPC(WbR, pWB, pWB->ownTi == pWG->ss)wl_branchTotals.st_Accum(pWB->ps_totals, pWB->wb_mult);
        }
...
//    else results unusable
{
    ...
    DHWLOOPSEG *pWG;
    RLUPC(WgR, pWG,
          pWG->ownTi == ss) { // note: segment chain relies on input order
    ...
    if (wl_wlpCount > 0 &&
        wl_flow * wl_runF > 0.f) // if any loop pumps and any flow
    {
        DHWLOOPPUMP *pWLP;
    ...
    if (wl_HRLLnet > 0.f) { // meter hookup
    ...
    wl_tRL =
            volHr > 0.f // return temp
            ? tIn1 - wl_HRLLnet /
                     (volHr *
                      waterRhoCp_Btu_per_galF) // not wl_tIn1! (see above)
            : 0.f;
    ...
    wl_HRLLnet *= wl_mult;
    wl_HRBL *= wl_mult;
    wl_volRL = (1.f - fMakeUp) * volHr *
               wl_mult; // flow returning to primary DHWHEATER(s)
...
///////////////////////////////////////////////////////////////////////////////
SEGTOTS::SEGTOTS() { st_Init(); }

//-----------------------------------------------------------------------------
void SEGTOTS::st_Init() { st_count = st_len = st_vol = st_exArea = st_UA = 0.; }

//-----------------------------------------------------------------------------
void SEGTOTS::st_Accum(const SEGTOTS &src, double count /*=1.*/) {
...
//----------------------------------------------------------------------------
float PIPERUN::pr_DeriveSizeFromFlow(float flow,   // flow rate, gpm
...
// returns UA, Btuh/F
{
    ...
    if (pr_insulThk < .001f)
        ...
        double rIns = log(diaX / diaO) /
                      (2. * pr_insulK); // insulation resistance (per ft thk)
...
//----------------------------------------------------------------------------
float PIPERUN::pr_SetBeta(float mCp,          // heat capacity rate, Btuh/F
                          float fUA /*=1.f*/) // UA adjustment factor
// beta = (1 - approach to surround) for pipe loss
// sets pr_beta (and returns it)
{
    pr_beta = mCp > .1f ? exp(max(-80.f, -pr_totals.st_UA * fUA / mCp))
...
//=============================================================================
void PBC::sb_Init(PIPESEG *pPS) { sb_pPS = pPS; } // PBC::sb_Init
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
/*virtual*/ int PBC::sb_Class() const { return sfcPIPE; } // PBC::sb_Class
//=============================================================================
...
//----------------------------------------------------------------------------
float PIPESEG::ps_CalcUA(float fUA /*=1.f*/) {
...
//    else results unusable
{
    ...
    if (fNoFlow < 1.f) {
        ...
        {
            float drawFlow =
                    pWS->ws_whUse.total / (pWS->ws_wlCount * 60.f); // draw, gpm
            if (drawFlow > 0.f)
                ps_fvf += drawFlow;
        }
    } else
