                  ...
                  // accumulated into it
{
  DHWMTR_IVL *dIvl =
      &curr.Y +
      (ivl -
       C_IVLCH_Y); // point destination substruct for interval
...
// called iff there are DWHRs and/or events that might recover heat
{
  ...
  reDo:
    ...
    // tick energy balance
    float qXNoHR =
        (whUseNoHR + whUseOther) * waterRhoCp * (ws_tUse - ws_tInlet);
...
//         else error (stop run)
{
  ...
  if (!IsSet(DHWUSE_TEMP))
    ...
    float tCold =
        pWS->ws_tInlet;        // cold water temp at fixture, F
...
// returns RCOK iff success
{
  ...
  int attrs;
  HPWH::MODELS preset = HPWH::MODELS(hw_HPWHInfo(
      ashpTy, attrs)); // HPWH "preset"
  ...
  if (volX > 0.f &&
      hw_pHPWH->setTankSize(volX, HPWH::Units::Volume::gal, true) != 0)
    rc |= RCBAD;
  if (UAX >= 0.f && hw_pHPWH->setUA(UAX, HPWH::Units::UA::Btu_per_hF) != 0)
    rc |= RCBAD;
...
// returns RCOK iff success
{
  ...
  if (hw_pHPWH->initPreset(preset) != 0)
    rc |= RCBAD;
  if (hw_pHPWH->setTankSize(vol, HPWH::Units::Volume::gal) != 0)
    rc |= RCBAD;
  return rc;
...
// returns RCOK iff success
{
  ...
  if (UA >= 0.f) {
    if (hw_pHPWH->setUA(UA, HPWH::Units::UA::Btu_per_hF) != 0)
      rc |= RCBAD;
  }
  return rc;
    ...
    float inHtLoopRet)        // loop return fractional height, -1 = don't set
{
  ...
  hw_tankHCNominal =
      KJ_TO_KWH(40. * HPWH::DENSITYWATER_kgperL * HPWH::CPWATER_kJperkgC *
...
// returns RCOK iff success
{
  ...
  int hpwhRet = 0;
  double minT = hw_pHPWH->getMinOperatingT(HPWH::Units::Temp::F);
  if (minT == double(HPWH::HPWH_ABORT))
    ...
    hpwhRet = hw_pHPWH->setCompressorOutputCapacity(
        ...
        HPWH::Units::Power::Btu_per_h, HPWH::Units::Temp::F);

    if (!hpwhRet)
      // set capacity of all reistance elements to design cap
      //   (handles e.g. possible low-temp lockout)
      hpwhRet = hw_pHPWH->setResistanceCapacity(heatingCap, 0,
...
//    else RCBAD (heatingCap = 0)
{
  ...
  if (hw_pHPWH->hasACompressor()) {
    ...
    for (int iRE = 0; iRE < nRE; iRE++) {
      double capx =
          hw_pHPWH->getResistanceCapacity(iRE, HPWH::Units::Power::Btu_per_h);
...
{
  RC rc = RCOK;
  vol = hw_pHPWH->getTankSize(HPWH::Units::Volume::gal);
  double UAd;
  if (hw_pHPWH->getUA(UAd, HPWH::Units::UA::Btu_per_hF) != 0)
    rc |= RCBAD;
  UA = float(UAd);
...
//   (accounting for possible multiple tanks)
{
  ...
  double surfAPerTank = HPWH::getTankSurfaceArea(
      volPerTank, HPWH::Units::Volume::gal, HPWH::Units::Area::ft2);
...
// returns RCOK iff success
{
  ...
  float runHrMin = hw_pHPWH->getCompressorMinimumRuntime(
      HPWH::Units::Time::h); // minimum compressor run time, hr
...
{
  return hw_pHPWH->getTankSize(HPWH::Units::Volume::gal);
...
// returns average tank temp, F for node range
{
  ...
  for (int iN = iNode0; iN != iNodeN; iN += incr)
    T += hw_pHPWH->getTankNodeT(iN, HPWH::Units::Temp::C);
...
//   = current top node temp (no consideration of draw etc.)
{
  ...
  return hw_pHPWH->getTankNodeT(iNodeTop, HPWH::Units::Temp::F);
...
RC HPWHLINK::hw_DoHour( // hourly HPWH calcs
    float &tSetpoint,   // setpoint for current hour, F
                        ...
                        //    restrictions if any
    float
        targetSoC, // state of charge (SOC) target, 0 - 1
...
// returns RCOK iff success
{
  ...
  if (!hw_pHPWH->isSetpointFixed()) {
    ...
    hw_pHPWH->setSetpointT(tSetpointX, HPWH::Units::Temp::F);
  }

  // retrieve resulting setpoint after HPWH restrictions
  tSetpoint = hw_pHPWH->getSetpointT(HPWH::Units::Temp::F);
  if (hw_tHWOut == 0.f)
  ...
  if (!hw_tankTempSet) { // initialize tank temp on 1st call
    ...
    if (tankTInit != nullptr) {
      ...
      if (hw_pHPWH->setTankTs(vTankTInit, HPWH::Units::Temp::F))
        rc |= RCBAD;
    } else {
    ...
    int drStatus /*=0*/)     // demand response control signal
{
  ...
  if (whfcn & DHWHEATER::whfcnSUPPLIESLOAD) { // mixdown: DHW and loop draws are
    ...
    drawLoss =
        tk.wtk_qLossNoRL * scaleX / (waterRhoCp * max(1., tMix - tMains));
  ...
  if (hw_tOut < .01) { // no draw / outlet temp estimated
    ...
    double qHWTick =
        KJ_TO_KWH((GAL_TO_L(drawForTick) * hw_tOut -
                   GAL_TO_L(drawForTick - drawRC) * DegFtoC(tInlet) -
                   GAL_TO_L(drawRC) * DegFtoC(tRC)) *
                  HPWH::DENSITYWATER_kgperL * HPWH::CPWATER_kJperkgC);
  ...
  if (hw_bWriteCSV) {
    ...
    CSVItem CI[] = {
        ...
        UNTEMP,     5,
        "tSP",      DegCtoF(hw_pHPWH->getSetpointT_C()),
        UNTEMP,     5,
    ...
    ZNR *pZnASHPSrc)        // ASHP heat source zone, NULL if none
{
  ...
  if (hw_nzDrawCount)
    hw_tHWOut =
        hw_tHWOutF /
        hw_nzDrawCount; // note: >= ws_tUse due to unlimited XBU
...
// returns RCOK on success
{
    ...
    if (what) {
      ...
      if (wh_HPWH.hw_pHPWH->isTankSizeFixed())
        oInfo("%s is ignored when whASHPType=%s (tank volume is fixed at %0.0f "
              ...
              wh_HPWH.hw_pHPWH->getTankSize(HPWH::Units::Volume::gal));
      ...
      if (vol > 0.f) {
        wh_vol = vol;
        wh_HPWH.hw_pHPWH->setTankSizeWithSameU(vol, HPWH::Units::Volume::gal);
      }
    }
  }
  ...
  if (!rc && !pWS->ws_configChecked) {
    if (wh_HPWH.hw_IsSetpointFixed()) {
      ...
      float tspFixed = wh_HPWH.hw_pHPWH->getSetpointT(HPWH::Units::Temp::F);
...
RC DHWHEATER::wh_DoSubhrTick( // DHWHEATER energy use for 1 tick
    ...
    float scaleWH,  // draw scale factor = 1/ws_whCount or 1/ws_wlhCount
                    //   re DHWSYSs with >1 DHWHEATER
                    //   *not* including any mixdown factors (e.g. hw_fMixUse or
                    //   hw_fMixRL)
    float tInletWH, // water heater inlet temp, F (NOT mains temp)
    float &tOutlet) // returned: unmixed outlet temp, F
                    //   HPWH: from last draw or estimated from tank top if no
                    //   draw other: = ws_tUse
{
  RC rc = RCOK;
  ...
  if (wh_IsHPWHModel()) { // demand response (DR)
    ...
    if (wh_IsInstUEFModel())
      ...
      wh_inElecSh += wh_parElec * Top.tp_tickDurHr *
                     BtuperWh; // electric parasitic
  ...
  if (wh_IsPrimary()) {
    ...
    float dhwLoadTk2 =
        drawWH * pWS->ws_mult * waterRhoCp * (pWS->ws_tUse - tk.wtk_tInletX);
...
// returns RCOK iff simulation should continue
{
  ...
  if (wh_IsHPWHModel()) {
    ...
    wh_pResSh->qStorage[iLH] +=
        wh_mult * BtuperkWh * (wh_HPWH.hw_tankHCBeg - wh_HPWH.hw_tankHCEnd);
    ...
    // electricity use
    wh_inElecSh = wh_HPWH.hw_inElec[0] * BtuperkWh +
                  wh_parElec * BtuperWh * Top.tp_subhrDur;
...
float PIPESEG::ps_CalcUA(float fUA /*=1.f*/) {
  ...
  if (ps_insulThk < .001f)
    ...
    double rIns =
        log(diaX / diaO) / (2. * ps_insulK); // insulation resistance
