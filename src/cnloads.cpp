...
// cnloads.cpp -- Hourly simulation loads modelling calculations for a single
// hour for CSE
...
Zone temperatures are computed in cnloads.cpp/cnhvac.cpp, using various
subexpressions (members of the ZNR struct) precomputed in cnguts.cpp.  They are
computed several times an hour; t is Top.tp_subhrDur.  For "hc" the CAIR is used
...
// initialization common to main simulation run and each autosize design day
{
  ...
  znXLGain = znXLGainLs =
      0; // no condensation heat leftover from prior iteration, rob 6-11-97
...
// returns RCOK iff all OK
{
  ...
  if (zn_pz0WarnCount[0] > 0 || zn_pz0WarnCount[1] > 0) {
    warn("Zone '%s': %s unreasonable pressure warning counts --"
         "\n    mode 0 = %d / mode 1 = %d",
         Name(),
         isAusz ? strtprintf("%s autosizing", Top.tp_AuszDoing())
                : "Simulation",
...
//====================================================================
RC FC loadsHourBeg() // start of hour loads stuff: solar gains, hourly masses,
                     // zones init, .

// non-RCOK return means terminate run.  Message already issued.
{
  /* CALCULATE HOURLY SOLAR GAINS (most are subhourly, done elsewhere)
    Set "targets" in [zones and] masses to their solar gains for hour,
    calculated by combining hour's weather data and control variable values
    (window shade positions) with the precalculated (cgsolar.cpp) factors in the
    entries in the (currently selected) Solar Gain Rat. The factors already
    reflect the size, orientation, and absorptivity of each XSURF and the sun's
    position this month.   cnrecs.def/rccn.h structures. */
  SGRAT *sge;
  RLUP(SgR, sge) {
    if (!sge->sg_isSubhrly)
...
// can be called hourly or subhourly, args must appropriately correspond to time
{
  ...
  {
    sg_pTarg->st_bm =
        0.; // just zero all targets.  Unnecessary to test sge->addIt.
    sg_pTarg->st_df = 0.;
  } else {
    ...
    {
      bmGain *=
          *sg_pControl; // multiply gain by it. eg zone shades-closed fraction.
      dfGain *= *sg_pControl;
    }
...
void SGRAT::sg_DbDump() const {
  DbPrintf("\nSGDIST '%s': isSubhrly=%d  addIt=%d\n%s   targ=%p  control=%p",
           Name(), sg_isSubhrly, sg_addIt, Top.tp_RepTestPfx(), sg_pTarg,
           sg_pControl);
  for (int iH = 0; iH < 24; iH++) {
    if (iH % 4 == 0)
      DbPrintf("\n");
    DbPrintf("%8.2d  %6.2f  %6.2f  %6.2f", iH, sg_dfXDfF[iH], sg_dfXBmF[iH],
             sg_bmXBmF[iH]);
  }
} // sg_DbDump
//-----------------------------------------------------------------------------
...
// done *after* expressions, before dependent inits
{
  ...
  if (Top.isBegRun) { // prior hour setpoints
    zn_tzspHlh = i.znTH;
    zn_tzspDlh = i.znTD;
    zn_tzspClh = i.znTC;
  }
...
{
  zn_SetAirRadXArea(); // update air temp/relHum dependent radiant exchange
                       // values
  ...
  // Non-hvac hour-constant portions of MAIN EQUATION "Tz = (a + q)/b" (story at
  // start file)

  // sum ua*t for specified-temp-exposure surfaces
  XSRAT *xs /*= NULL*/;
  zn_uaXSpecT = 0.;
  for (TI xi = xsSpecT1; xi != 0;
       xi = xs->nxXsSpecT) // these xsurfs have own chain.  loop over them.
  {
    xs = XsB.p + xi; // access XSRAT record xi
    zn_uaXSpecT += xs->x.xs_uval * xs->x.xs_area *
                   xs->x.sfExT; // u * a * outside temp (hourly vbl)
    /*>>>> there are 2 hourly and 1 subhourly use of uval*area --> otta
      precompute it, maybe also use in cnguts re ua -SpecT.  FLOAT gud enuf for
      the latter? rob 3-91. */
  }

  // zn_aqLdHr: hour-constant non-hvac a's and q's for numerator.  a =
  // sigma(UAi*Ti) + Tzold*hc/t.
  zn_aqLdHr =
  ...
  // zn_bLdHr: hour-constant non-hvac b's for denominator.   b = sigma(UA) +
  // hc/t.
  zn_bLdHr = zn_bcon; // just the run-const UAs

  /* zn_xqHr: q = b * t - a  using only hourly components, for change tests
     (just below) only: Intended to be appropriately sensitive to changes when
     b*a almost equal to a, eg when aMassHr settling & all else constant, cuz
     terminal loads are of form  b * t - a  (with added subhrly components).
          Fixed problems with bug39a.inp run (1 mass surf, const temps).
     12-5-94. */
  zn_xqHr =
      zn_bLdHr * tz - zn_aqLdHr; // member only cuz saved to -Pr in loadsSubhr.
  ...
  /* hourly-only load change checks:
      ...
      znLGain: latent gain rate (set per GAINs by cnguts.cpp:doHourgains, used
     in cnZtu.cpp:ZNR::znW). Note: for subsequent subhrs, wz change sets ztuCf
     in loadsSubhr. -Pr's used here are updated in loadsSubhr, every time ztuCf
     is on. */
  // zn_aqLdHr and zn_bLdHr are covered by zn_xqHr check & no longer checked
  // separately 12-94.
  //   Removing aq- and zn_bLdHr checks made NO difference in test suites 12-94
  if (
      // .02 found better than .05,.1,.2 in bug39a.inp run (1 mass surf, const
      // temps) (12-94):
      RELCHANGE(zn_xqHr, zn_xqHrPr) >
          .02 * Top.relTol *
              MARG1 // if hourly part of difference b*t-aq (for loads) changed
      || ABSCHANGE(znLGain,
                   znLGainPr) // if zone moisture GAIN rate (Btuh) changed:
             > PsyHCondWtr *
                   Top.absHumTol) //  test abs lb/hr change after conversion
  {
...
{
  memset(&ZnresB.p[ss].curr.S, 0,
         sizeof(ZNRES_IVL_SUB)); // access subhr results struct in ZNRES via
                                 // subscr in ZrB & 0 it.
  ZnresB.p[ss].curr.S.nSubhr =
      1L; // count subhours by setting to 1 at subhr level & accumulating
...
// call *after* lagged values used
{
  ...
  zn_hcFrc = Top.tp_hConvF * i.zn_hcFrcF *
             pow(max(zn_hcAirXls + zn_hpwhAirX, 0.f), 0.8f);
  ...
  qMsSg = 0.f; // zone's mass solar and radiant internal gains.  Accum'd during
               // mass calcs
  qrIgMs = 0.f;
...
//============================================================================
RC FC loadsSubhr() // loads stuff for all zones for subhour.  Call BEFORE hvac
                   // code.
...
// Assumes zone subhour results pre-0'd: done in zn_BegSubhr
{
  ...
  /* CALCULATE SUBHOURLY SOLAR GAINS
    Set subhourly "targets" in zones and masses to their solar gains for
    subhour, calculated by combining subhour's weather data and control variable
    values (window shade positions) with the precalculated (cgsolar.cpp) factors
    in the entries in the (currently selected) Solar Gain Rat. The factors
    already reflect the size, orientation, and absorptivity of each XSURF and
    the sun's position this month.   cnrecs.def/rccn.h structures. */
  SGRAT *sge;
#if 1
...
#else
  x SI sunup = Top.radDiffShAv > 0.F ||
               Top.radBeamShAv > 0.F; // 0 if no sun this subhour
      ...
      x #ifdef SOLAVNEND   // undef in cndefns.h (via cnglob.h) 1-18-94: only if
          ...
          xo sge->isEndIvl // TRUE to use end-subhr not subhr-avg, 1-95
              ...
              ? Top.radDiffSh * sge->diff[Top.iHrST] // combine weather data
                                xo +
                    Top.radBeamSh *
                        sge->beam[Top.iHrST] // with this sge's gain factors
                        xo
              : Top.radDiffShAv * sge->diff[Top.iHrST] xo +
                    Top.radBeamShAv * sge->beam[Top.iHrST];
      x #else x #if 1 x float gain =
          Top.radDiffShAv *
              sge->sg_diff[Top.iHrST] // gain consists of subhour average
                                      // weather data values
              x +
          Top.radBeamShAv * sge->sg_beam[Top.iHrST]; // ... combined with hour's
                                                     // gain factors in sge
      x #else // temp backtrack 2-95, undone
          xx float gain =
              Top.radDiffShAv *
                  sge->diff[Top.iHr] // gain consists of subhour average weather
                                     // data values
                  xx +
              Top.radBeamShAv * sge->beam[Top.iHr]; // ... combined with hour's
                                                    // gain factors in sge
      x #endif x #endif x // (gain fixed for diff[24], standard time 2-95)
          x if (sge->sg_control) // if sge has pointer to control factor
          x gain *=
          *sge->sg_control; // apply control. eg fraction zone shades closed.
  ...
  // Must follow (any future) subhouly solar gain determination, precede zone
  // loop. Uses Top.tp_subhrDur.
  ...
  if (!Top.tp_bAllCR) { // all CNE zones (all CR or all CNE enforced)
#if !defined(CRTERMAH)
    RLUP(ZrB, zp)
  ...
  if (ventAvail == C_VENTAVAILVC_WHOLEBLDG ||
      ventAvail == C_VENTAVAILVC_ZONAL) {
    int nVentUseful =
        0; // ventilation usefulness
    ...
    } else {              // vent might be useful for at least 1 zone
      loadsIzxSh2();      // AirNet "vents open"
      Top.tp_fVent = 1.f; // try full vent and limit
      RLUP(ZrB, zp) {
        if (!zp->zn_IsUZ()) {
          int vRet = zp->zn_FVentCR();
          if (ventAvail == C_VENTAVAILVC_WHOLEBLDG) { // whole building control
          ...
          } else { // zonal control: each zone gets preferred
  ...
  RSYS *rs;
  if (ventAvail == C_VENTAVAILVC_RSYSOAV) { // attempt OAV for each system
    // if successful, rs_mode = (zones)->zn_hcMode = rsmOAV (prevents other RSYS
    // modeling) fVent = 0 if here
    RLUP(RsR, rs)
  ...
  RLUP(ZrB, zp)
  rc |= zp->zn_CondixCR(
      ventAvail); // duct leakage can add cross-zone air
  ...
  if (DbDo(dbdAIRNET | dbdIZ) && ZrB.GetCount() > 0) {
    DbPrintf("\nZone airnet results\n"
             "Zone                  pz0W[0]  pz0W[1]      pz0      qIzSh    "
             "AmfCp     AmfCpT\n");
    RLUP(ZrB, zp)
    DbPrintf("%-20.20s %8.5f %8.5f %8.5f %10.2f %8.2f %10.1f\n", zp->Name(),
             zp->zn_pz0W[0], zp->zn_pz0W[1], zp->zn_pz0, zp->zn_qIzSh,
...
RC ZNR::zn_InitSubhr() {
  ...
  if (Top.tp_autoSizing) { // avoid setpoint step changes when autosizing
  ...
  } else { // not autosizing: use setpoints as input
    zn_tzspH = i.znTH;
    zn_tzspD = i.znTD;
    zn_tzspC = i.znTC;
  }
  ...
  if (zn_UsesZoneSetpoints() && zn_tzspH >= zn_tzspC)
    orer("Impossible setpoints -- znTH (%0.2f) >= znTC (%.2f)", zn_tzspH,
         zn_tzspC);
...
#else
  zn_uaInfil =
      Top.tp_airxOSh // Btuh/cfm-F air flow heat xfer at current outdoor temp
      *
      (i.znVol             // air changes/hr part: zone volume (ft3), times...
           * i.infAC / 60. // user input air changes per hour, hourly variable,
                           // /60 for per minute.
       +
       i.infELA    // ELA part: user input effective leakage area (ft2), hourly
                   // variable
           * sqrt( // flow is proportional to square root of
                 zn_stackc *
                     fabs(tzlh - Top.tDbOSh) + //  stack coeff (see
                                               //  ZNR.zn_InfilSetup) * delta t
                 zn_windc * Top.windSpeedSquaredSh) //  wind coeff (ditto) *
                                                    //  wind speed ^ 2
      );
#endif

  // for hvac code, compute a+q, b contributions this subhour for everything but
  // hvac, for use in "Tz = (a + q)/b" a+q =  sigma(UAi*Ti) + other q's +
  // Told*hc/t
  zn_qSgAir = zn_sgAirTarg.st_tot; // solar gain to air
  zn_aqLdSh =
      zn_aqLdHr // hour-constant part: intl gain, solar gain, cond to ambient &
                // specT's, etc.
  ...
  /* zn_xqSh: q = b * t - a  using components avail here, for change tests only:
          Intended to be appropriately sensitive to changes when b*a almost
     equal to a, since terminal loads are of form  b * t - a. Insurance 12-94 --
     need was not proven at subhour level when all masses hourly (b4 aMassSh
     added). But it did something: eliminated undercooling in hour 13 in S1LOH
     test run, 12-5-94, so keep. */
  ...
  double nAirSh =
      ...
      + zn_AnAmfCpT(0) + zn_qIzXAnSh // non-airnet interzone
      + znXLGainLs // excess latent gain last subhr === heat of condensation
      + zn_uaInfil * Top.tDbOSh; // non-airnet infiltration
  zn_nAirSh += nAirSh;

  double dAirSh = // associated denominator terms
      i.znCAirSh + zn_AnAmfCp(0) + zn_uaInfil;
  zn_dAirSh += dAirSh;

  double nRadSh = qrIgTot; // radiant internal gains
...
#if defined(CRTERMAH)
  if (zn_IsConvRad()) { // convective/radiant: set up CNE-style zone factors
    zn_aqLdSh = zn_balC1 / zn_dRpCx;
    zn_bLdSh = zn_balC2 / zn_dRpCx;
  }

  // zn_xqSh: q = b * t - a  using components avail here, for change tests only:
  //  Intended to be appropriately sensitive to changes when b*a almost equal to
  //  a, since terminal loads are of form  b * t - a. Insurance 12-94 -- need
  //  was not proven at subhour level when all masses hourly (b4 aMassSh added).
  //  But it did something: eliminated undercooling in hour 13 in S1LOH test
  //  run, 12-5-94, so keep. */
  zn_xqSh = zn_bLdSh * tzls - zn_aqLdSh;
#endif
  ...
  if (DbDo(dbdRCM))
    DbPrintf("%s air  qsIgHr=%0.1f  qrIgTot=%0.1f  uaInfil=%0.1f\n"
             "      nAir=%0.2f  dAir=%0.2f  nRad=%0.2f  dRad=%0.2f  CxF=%0.6g  "
             "CX=%0.2f\n",
             Name(), qsIgHr, qrIgTot, zn_uaInfil, nAirSh, dAirSh, nRadSh,
             dRadSh, zn_airCxF, zn_airCx);
#endif
  return RCOK;
} // ZNR::zn_InitSubhr
//-----------------------------------------------------------------------------
    ...
    BOO azCf) // TRUE iff "all-zones change" e.g. outdoor w changed
{
  ...
  // set change flag for terminal module if zone loads changed.  Cleared in
  // ZNR::ztuCompute.

  // monitor subhourly-changing loads inputs to zone heat and moisture balances
  // except subhrDur:
  //  heat balance:     aqLdSh: tzls, zn_qIzSh; zn_aqLdHr is hourly; znCair is
  //  constant.  zn_bLdSh: changes only per subhrDur.
  //                    zn_xqSh
  //  moisture balance: wzls.  znLGain is hourly.
  //  other:            md

  if (ztuCf   // if a flag already set, skip tests, update priors to avoid unnec
              // flagging later.
      || azCf // if all-zones change detected (outdoor w, above)
      || spCf // spCf implies ztuCf (and more), 6-92.
      || RELCHANGE(zn_xqSh, zn_xqShPr) >
             .02 * Top.relTol *
                 MARG2 // b*t - a as known here: difference sensitive.
                       // .02 copied from hour case. Probably new dominant term;
                       // may make some of the other checks unnec 12-5-94.
      ||
      ABSCHANGE(tzls, tzlsPr) >
          Top.relTol * 2. *
              MARG2 // Dominant term.  MATCH above. use relTol as small absTol.
      ...
      || ABSCHANGE(wzls, wzlsPr) >
             .2 * Top.absHumTol // MATCH tolerance above. dflt .000006, or as
                                // changed. .00002 (vs what??) adds iterations;
                                // .00001 adds even more; otherwise unexplored.
      || zn_md !=
             zn_mdPr // if zn hvac mode changed last subhr, don't skip tu's now:
                     // eg be sure tz right on change into a setpoint mode.
      || RELCHANGE(zn_qIzSh, qIzShPr) > .00005) {
    ztuCf++;            // say zone's load changed.
    qIzShPr = zn_qIzSh; // reset comparators (Prior values), whenever (and only
                        // when) flag set.
    tzlsPr = tzls;
    wzlsPr = wzls;
    zn_mdPr = zn_md;
  ...
  } else                 // load not changed
  {
    ...
    // other items, such as zn_qsHvac, remain approx same for same tz rate of
    // change. 4-92
    ...
    // Tried removing in 6-92, restoring seemed to reduce # subhours & #
    // iterations in test runs.
    aWz =                // ah working copy
...
void ZNR::zn_DbDump() const {
  ...
  DbPrintf("%s %s: anMCp/T[ 0]=%.2f/%.1f  anMCp/T[ 1]=%.2f/%.1f  ventUt=%d\n",
           Name(), zn_IsUZ() ? "UZ" : "CZ", zn_AnAmfCp(0), zn_AnAmfCpT(0),
           zn_AnAmfCp(1), zn_AnAmfCpT(1), zn_ventUt);
  DbPrintf(
      "     Nair=%.2f  Dair=%.2f  Nrad=%.2f  Drad=%.2f  CX=%.2f  airX=%.3f\n",
      zn_nAirSh, zn_dAirSh, zn_nRadSh, zn_dRadSh, zn_cxSh, i.zn_hcAirX);
  if (!zn_IsUZ()) {
    DbPrintf("     TH=%.2f  TD=%.2f  TC=%.2f", zn_tzspH, zn_tzspD, zn_tzspC);
    if (!zn_HasRSYS())
      DbPrintf("  qhCap=%.f  qcCap=%.f\n", i.znQMxH, i.znQMxC);
    else
      DbPrintf("  tSP=%.2f  md=%d  amfReq=%.f  amfSup=%.f   tSup=%.2f\n",
               zn_tzsp, zn_hcMode, zn_rsAmfSysReq[0], zn_sysAirI.af_amf,
               zn_sysAirI.as_tdb);
  }
  DbPrintf(
      "     ta=%.2f  tr=%.2f  qIzSh=%.f  fvent=%.3f  pz0=%.4f  qsHvac=%.f\n",
...
//          1: might be helpful
{
  ...
  if (!zn_IsUZ() && zn_anVentEffect > 0) {
    ...
    else if (tz > zn_tzspD) { // zone temp is above TD
...
x	}
  ...
  if (bUZ || znfVent > 0.) { // float temp
    ...
    if (znfVent > 0.f) {
      ...
      zn_pz0 =
          znfVent * zn_pz0W[1] + (1.f - znfVent) * zn_pz0W[0]; // zone pressure

      tz = zn_TAirCR(zn_anAmfCpTVent, zn_anAmfCpVent);

      if (!bUZ && znfVent < 1.f &&
          znfVent == zn_fVentPrf) { // equal fVents: current zone is "control"
...
#if defined(_DEBUG)
    if (!bUZ && zn_anVentEffect > 0) { // check conditioned zone outcome
  ...
  else { // HVAC may be required

    zn_fVent = 0.f; // no vent
    zn_pz0 = zn_pz0W[0];

    RSYS *rs = zn_GetRSYS();
    if (rs) {                        // this zone is served by an RSYS
...
x						printf( "1B\n");
          ...
          if (rsAvail >= 2) { // mode is possible
            rc |= zn_AirRequest(rs,
                                1);  // determine air requirement given rs_asSup
                                     //   1: msg if bad supply temp
                                     // zone temp calc'd in zn_CondixCR2()
          } else if (rsAvail == 1) { // autosizing but not requested mode
    ...
    } else { // non-RSYS zone ("magic" heating and cooling)
      zn_IdealHVAC();
    }
  }
...
// returns RCOK iff success
{
  ...
  if ((options & 1) && !Top.isWarmup && rs->rs_speedF > 0.99f &&
      ((zn_hcMode == RSYS::rsmCOOL && tSup0 >= zn_tzsp) ||
       (zn_hcMode == RSYS::rsmHEAT && tSup0 <= zn_tzsp &&
        rs->rs_effHt > 0.f))) {
    orWarn("Flipped tSup  RSYS='%s', tPln=%0.3f, tSup=%0.3f, tSP=%0.1f, "
           "tZn=%0.3f\n",
  ...
  double amfSup0 =
      zn_AmfHvacCR(zn_tzsp, tSup0); // dry air mass flow rate required to hold
#if 0 && defined(_DEBUG)
  ...
  zn_rsAmfSysReq[0] =
      rs->rs_ZoneAirRequest(amfSup0, 0); // notify system of requirement
  CHECKFP(zn_rsAmfSysReq[0]);            // check for NaN etc (debug only)

  if (zn_hcMode == RSYS::rsmHEAT && rs->rs_CanHaveAuxHeat() &&
      rs->rs_speedF ==
          1.f) { // HP heating full speed: repeat calc with full aux
...
// call ONLY when RSYS capacity sufficient
{
  if (zn_hcMode != RSYS::rsmOFF) {
    ...
    zn_rsAmfRet = zn_rsAmfSup * rs->rs_ducts[iHC].ductLkXF[1] /
                  rs->rs_ducts[iHC].ductLkXF[0];
    zn_rsFSize = 1.f;
  }
  // else zn_rsAmfxxx = 0
...
// applies "magic" heating or cooling
{
  if (tz > zn_tzspC && i.znQMxC < 0.f) {
  ...
  } else if (tz < zn_tzspH && i.znQMxH > 0.f) { // tz < TH: air needs heating
    zn_hcMode = RSYS::rsmHEAT;
    tz = zn_tzsp = zn_tzspH;
#if defined(ZNHVACFCONV)
    zn_fConv = zn_fConvH;
    ...
    zn_qsHvac = zn_QAirCR(tz);
#endif
    if (zn_qsHvac > i.znQMxH) { // heat at heating capacity, find resulting tz.
      zn_qsHvac = i.znQMxH;
#if defined(ZNHVACFCONV)
      tz = zn_TAirCR(zn_qsHvac * zn_fConv, 0., zn_qsHvac * (1.f - zn_fConv));
#else
...
//-----------------------------------------------------------------------------
void ZNR::zn_MapTerminalResults() // transfer terminal model outcomes to working
                                  // mbrs
{
  ...
  for (TU *tu = nullptr; nxTu(tu);) {
    ...
    if (!ah) { // no terminal air flow w/o AH
      continue;
    }
    ...
    if (frDiff(tu->cz, aCz) > 0.001)
      printf("\nFlow mismatch -- tu->cz=%.2f   tu->aCz()=%.2f", tu->cz, aCz);
#endif

    if (tu->cz > 0.) { // accumulate supply flow and state
      cSup += tu->cz;  // supply flow (at zone), Btuh/F
      cRet += tu->cz *
              (1. - ah->oaZoneLeakF * ah->po); // return flow (at zone), Btuh/F
  ...
  if (cSup > 0.) {
    ...
    zn_sysAirO.af_AccumDry(-zn_rsAmfRet, tz,
                           wzls); // w finalized in zn_AirXMoistureBal
  }
...
RC ZNR::zn_CondixCR2() // zone conditions, part 2
{
  ...
  if (rs) {
    if (zn_rsAmfSup >
        0.) { // this zone is served by RSYS and receives air
  ...
  zn_qIzSh =
      zn_anAmfCpTVent + zn_AnAmfCpT(0) + zn_sysDepAirIls.af_AmfCpT() -
...
RC ZNR::zn_AirFlowVsTsup() {
  ...
  for (int iAf = 0; iAf < 10; iAf++) {
    ...
    fprintf(f, "%d,%d,%d,%d,%.1f,%.0f,%0.1f,%.0f\n", Top.tp_date.month,
            Top.tp_date.mday, Top.iHrST + 1, Top.iSubhr, fAmf, amf,
            asSup.as_tdb, amfNeed);
  }

  return rc;
} // ZNR::zn_AirFlowVsTsup
//-----------------------------------------------------------------------------
float RSYS::rs_TSupVarFlow(float fAmf, AIRSTATE &asSup) {
...
// returns zone air temp, F
{
  double tza = (mCpT * zn_dRpCx + zn_balC1) / (mCp * zn_dRpCx + zn_balC2);
  return tza;
} // ZNR::zn_TAirCR
//----------------------------------------------------------------------------
...
//------------------------------------------------------------------------------
double
ZNR::zn_AmfHvacCR(     // sensible hvac air requirements w/ add'l radiant heat
    double tza,        // zone air temp, F
...
// returns dry air mass flow rate into zone (lbm/hr) due to AirNet exchanges,
// lbm/hr
{
  double amf =
      (1.f - zn_fVent) * zn_airNetI[0].af_amf + zn_fVent * zn_airNetI[1].af_amf;
  return amf;
} // ZNR::zn_AnAmf
//-----------------------------------------------------------------------------
...
// returns dry air mass flow rate (lbm/hr) into zone due to non-AirNet sources
{
  ...
  if (i.infELA > 0.f) {
    float amfELA =
        Top.tp_rhoDryOSh * 60.f *
        i.infELA // user input effective leakage area (ft2), hourly variable
        *
        sqrt( // flow is proportional to square root of
            zn_stackc *
                fabs(tzls - Top.tDbOSh) + //  stack coeff (see
                                          //  ZNR.zn_InfilSetup) * delta t
            zn_windc *
                Top.windSpeedSquaredSh); //  wind coeff (ditto) * wind speed ^ 2
...
double ZNR::zn_AirXMoistureBal( // air change rate and zone moisture balance
    ...
    double *
        pXLGain /*=nullptr*/) // if non-null, return condensation heat gain, Btu
...

{
  ...
  double dryAirMass =
      max(i.znVol * psyDenDryAir2(rho, wzls), .1); // dry air mass, lbm
  ...
  double mw = mwIG + mwInf + mwAN +
              mwX // total water vapor mass flow rate, lbm/hr
              + zn_ductLkI.af_Wmf() + zn_sysAirI.af_Wmf();

  // TODO: HPWH moisture removal? 2-16

  [[maybe_unused]] int wCase =
      0; // debug aid
  ...
  double dryAirMassEff =
      dryAirMass * i.zn_HIRatio; // effective dry air mass
  ...
  double XLGain =
      0.; // excess gain to space due to "condensation" or "evaporation", Btuh
  ...
  if (bFinal) { // set ZNR members
    ...
    zn_qlIz = (1.f - zn_fVent) * zn_airNetI[0].af_QLat(wzls) +
              (zn_fVent)*zn_airNetI[1].af_QLat(wzls) + zn_ductLkI.af_QLat(wzls);
...
#if defined(DEBUGDUMP)
    if (DbDo(dbdZM)) {
      DbPrintf("%s W:  mwIG=%0.3f  mwInf=%0.3f  mwAN=%0.3f  mwDuctLk=%0.3f  "
               "mwSys=%0.3f  mwSum=%0.3f\n"
               "     tdb=%0.2f  airX=%0.3f  hcAirX=%0.3f dryAirMass=%0.2f  "
               "XLGain=%0.2f  W=%0.6f  twb=%0.2f  rh=%0.4f\n",
               Name(), mwIG, mwInf, mwAN, zn_ductLkI.af_Wmf(),
               zn_sysAirI.af_Wmf(), mw, _tz, zn_airX, i.zn_hcAirX,
               zn_dryAirMass, znXLGain, wz, zn_twb, zn_relHum);
      DbPrintf("     rho=%0.4f  rho0ls=%0.4f  wzls=%0.6f  dryAirMassEff=%0.2f  "
               "qlHvac=%0.2f  qlIz=%0.2f\n",
                          ...
                          //   default = current tz
{
  ...
  zn_dryAirMass =
      max(i.znVol * psyDenDryAir2(zn_rho, wzls), .1); // dry air mass, lbm
  zn_dryAirMassEff =
      zn_dryAirMass; // effective dry air mass, lbm
...
double ZNR::zn_HumRat( // zone moisture ratio
    ...
    double _tz, double *pXLGain /*=nullptr*/)
...
// see also znW() re CNE zone moisture balance
{
  ...
  zn_dryAirMass = dryAirMass;
  zn_dryAirMassEff =
      dryAirMass; // effective dry air mass, lbm
                  //   zn_AirXMoistureBal() may adjust re short time steps
  ...
  double _wz = mw * Top.tp_subhrDur / (zn_dryAirMassEff * i.zn_HIRatio) +
               wzls * (1. - f);
  ...
  double wzClamp = bracket(double(PsyWmin), _wz, wSat);
  if (_wz != wzClamp) {
    XLGain = (_wz - wzClamp) * zn_dryAirMassEff * i.zn_HIRatio * PsyHCondWtr /
             Top.tp_subhrDur;
    _wz = wzClamp;
    wCase += 2; // limits applied
  }
...
// sets zn_comfPMV7730 and zn_comfPPD7730
{
  ...
  if (zn_pComf) {
    ...
    int ret = zn_pComf->CalcPMV_IP(tz, tr, wComf, i.znComfAirV, i.znComfMet,
                                   i.znComfClo);
    if (ret == 0) {
...
//-------------------------------------------------------------------------------
RC RSYS::rs_CkF() {
  static_assert(RSYSMODES ==
                rsmCOUNT - 1); // cndefn.h #define must be consistent
  ...
  if (rs_OAVType == C_RSYSOAVTYCH_NONE)
    rc |= disallowN("when rsOAVType=None", RSYS_OAVRELIEFZI, RSYS_OAVTDBINLET,
                    RSYS_OAVTDIFF, RSYS_OAVAVFDS, RSYS_OAVAVFMINF,
                    RSYS_OAVFANPWR, 0);
  else {
    ...
    if (IsVal(RSYS_OAVTDIFF) && rs_OAVTdiff < 2.f)
      oWarn("Dubious rsOAVTdiff (%0.2f F) may cause inadvertent vent heating."
            "\n    rsOAVTdiff is typically >= 5 F to allow for fan heat and "
            "duct gains",
            rs_OAVTdiff);
...
RC RSYS::rs_CkFHeating() {
  ...
  static constexpr int16_t ASHP_HtgFNs[]{RSYS_HSPF,         RSYS_CAP47,
                                         RSYS_COP47,        RSYS_CAP35,
                                         RSYS_COP35,        RSYS_CAP17,
                                         RSYS_COP17,        RSYS_ASHPLOCKOUTT,
                                         RSYS_CAPRAT1747,   RSYS_CAPRAT9547,
                                         RSYS_CAPRATCH,     RSYS_CDH,
                                         RSYS_DEFROSTMODEL, 0};

  // ASHPVC (VCHP2) active FNs
  static constexpr int16_t ASHPVC_HtgFNs[]{RSYS_LOADFMIN47,
                                           RSYS_LOADFMIN17,
                                           RSYS_LOADFMIN05,
                                           RSYS_COPMIN47,
                                           RSYS_COPMIN35,
  ...
  if (!rs_CanHeat()) {
    const char *whenTyNoHt = strtprintf(
        "when rsType=%s (heating not available)", getChoiTx(RSYS_TYPE));
    rc |= ignoreX(whenTyNoHt, RSYS_CAPH, RSYS_FANPWRH, RSYS_FXCAPHTARG,
                  RSYS_AFUE, RSYS_CAPNOMH, RSYS_TDDESH, RSYS_FEFFH,
                  RSYS_CHDHWSYSI, ASHP_HtgFNs, ASHPVC_HtgFNs);
  ...
  if (rs_IsFanCoil()) { // fancoil htg
    rc |= require(whenTy, RSYS_CAPH);
    rc |= ignoreX(whenTy, RSYS_AFUE, RSYS_CAPNOMH, RSYS_FEFFH, ASHP_HtgFNs,
                  ASHPVC_HtgFNs);
    rs_AFUE = 0.f;
  } else if (rs_IsWSHP()) {
    ...
    if (!IsSet(RSYS_CAPH) && !IsSet(RSYS_CAP95))
      rc |=
          oer("at least one of rsCapH and rsCapC must be specified %s", whenTy);
    ...
    if (IsSetCount(RSYS_CAPH, RSYS_CAP95, RSYS_CAPRATCH, 0) == 3 &&
      ...
      rc |= oer("rsCapH, rsCapC, and rsCapRatCH cannot all be specfied %s",
                whenTy);

    rc |= ignoreX(whenTy, RSYS_HSPF, RSYS_CAP47, RSYS_CAP35, RSYS_COP35,
                  RSYS_CAP17, RSYS_COP17, RSYS_ASHPLOCKOUTT, RSYS_CAPRAT1747,
                  RSYS_CAPRAT9547, RSYS_DEFROSTMODEL, ASHPVC_HtgFNs);
  } else if (rs_IsASHP()) {
    if (!IsSet(RSYS_CAP47) && !IsSet(RSYS_CAP95))
      rc |= oer("at least one of rsCap47 and rsCapC must be specified %s",
                whenTy);

    if (IsSetCount(RSYS_CAP47, RSYS_CAP95, RSYS_CAPRAT9547, 0) == 3 &&
        !(IsAusz(RSYS_CAP47) && IsAusz(RSYS_CAP95)))
      // cannot give all 3 values unless both caps are autosized
      rc |= oer("rsCap47, rsCapC, and rsCapRat9547 cannot all be specfied %s",
                whenTy);

    if (rs_IsASHPHydronic()) { // ASHPHydronic: air-to-water heat pump
      //   approximated with air-to-air model
      rc |= requireN(whenTy, RSYS_COP47, RSYS_COP17, 0);
      rc |= disallowN(whenTy, RSYS_HSPF, RSYS_CAPH, RSYS_AFUE,
                      RSYS_DEFROSTMODEL, 0);
      if (!IsSet(RSYS_FANPWRH))
        rs_fanPwrH = 0.f; // hydronic: no fan power
    } else {              // ASHP other than ASHPHydronic

      rc |= disallowN(whenTy, RSYS_CAPH, RSYS_AFUE, 0);

      if (rs_IsASHPPkgRoom()) {
        rc |= disallowN(whenTy, RSYS_HSPF, RSYS_CAP05, RSYS_COP05, RSYS_CAP17,
                        RSYS_COP17, RSYS_CAP35, RSYS_COP35, 0);
      ...
      } else { // ASHP (non-hydronic, non-pkgroom): only HSPF required
        ...
        if (IsAusz(RSYS_CAP47))
          rc |= disallowN("when rsCap47 is AUTOSIZE", RSYS_CAP05, RSYS_CAP17,
                          RSYS_CAP35, 0);
        else {
          if (IsSet(RSYS_CAP17))
        ...
        if (!rs_IsASHPVC()) {
          rc |= ignore("when rsType is not ASHPVC (VCHP2)", ASHPVC_HtgFNs);
          rs_loadFMin05 = rs_loadFMin17 = rs_loadFMin47 = 1.f;
        } else { // default loadFMins
    ...
    if (!IsAusz(RSYS_CAP47))
      // rs_cap47 not AUTOSIZEd (altho may be expression), use it as rs_capNomH
      // default
      //   see DefaultCapNomsIf()
      FldCopyIf(RSYS_CAP47, RSYS_CAPNOMH);
  } else if (rs_IsCHDHW()) { // combined heat and DHW
  ...
  } else { // not CHDHW or HP of any type
    rc |= disallowX(
        "when rsType is not ASHP, ASHPHydronic, ASHPPkgRoom, or ASHPVC (VCHP2)",
        ASHP_HtgFNs, ASHPVC_HtgFNs);
    ...
    if (!IsAusz(RSYS_CAPH))
      // rs_capH not AUTOSIZEd (altho may be expression), use it as rs_capNomH
      // default
...
RC RSYS::rs_CkFCooling() {
  ...
  static constexpr int16_t Clg_FNs[] = {
      RSYS_SEER,        RSYS_EER95,      RSYS_COP95,  RSYS_CAP82,
      RSYS_COP82,       RSYS_CAPRAT8295, RSYS_CAP115, RSYS_COP115,
      RSYS_CAPRAT11595, RSYS_FCHG,       RSYS_CDC,    0};

  // FNs meaningful only for variable capacity
  //   ignored for non-VC ASHP
  //   disallowed for others
  static constexpr int16_t VC_ClgFNs[] = {RSYS_LOADFMIN115,
                                          RSYS_LOADFMIN95,
                                          RSYS_LOADFMIN82,
                                          RSYS_COPMIN115,
                                          RSYS_COPMIN95,
  ...
  if (!rs_CanCool()) { // no cooling capability
    const char *whenNoCl = strtprintf("when rsType=%s (cooling not available)",
                                      getChoiTx(RSYS_TYPE));
    rc |= ignoreX(whenNoCl, RSYS_CAP95, RSYS_FANPWRC, RSYS_CAPNOMC,
                  RSYS_VFPERTON, Clg_FNs, VC_ClgFNs);
  ...
  } else if (rs_IsFanCoil()) { // fancoil cooling
    rc |= require(whenTy, RSYS_CAP95);
    rc |= ignoreX(whenTy, Clg_FNs, VC_ClgFNs);
  } else if (rs_IsWSHP()) { // note checks of RSYS_TDBOUT and RSYS_CAP95 in
                            // rs_CkFHeating()

    rc |= requireN(whenTy, RSYS_EER95, 0);

    rc |= ignoreX(whenTy, RSYS_SEER, VC_ClgFNs);
  } else
    rc |= requireN(whenTy, RSYS_SEER, 0);

  if (!IsAusz(RSYS_CAP95)) { // rs_cap95 not AUTOSIZEd (altho may be
                             // expression), use as rs_capNomC default
  ...
  } else { // rsCap95 is autosize

    rc |= disallowN("when rsCap95 is AUTOSIZE", RSYS_CAP82, RSYS_CAP115, 0);
  }
  ...
  if (rs_IsASHPVC()) {
    ...
    rc |= ignoreX("when rsType is not ASHPVC (VCHP2)", VC_ClgFNs);
...
//         else nz value
{
  ...
  if (!rs_IsFanCoil() && !rs_IsWSHP() &&
      !rs_IsVCClg()) { // check that SEER and EER are plausible
    if (rs_SEER <= rs_EER95)
  ...
  { // checks to prevent crazy extrapolation results
    // cap82 > cap95 typically; occasional cap82 < cap95 examples seen so use
    // 0.8
...
//    else !RCOK (user input out of range)
{
  ...
  } else { // not rsmCOOL, assume rsmHEAT
    if (!IsSet(RSYS_CDH))
      rs_CdH =
          rs_IsASHPHydronic() || rs_IsASHPPkgRoom() || rs_IsASHPVC() ||
                  rs_IsWSHP()
              ? 0.25f // hydronic / pkgRoom / WSHP: no HSPF source for default
                      // ASHPVC: relationship to HSPF not known
              : bracket(.05f, .25f - 0.2f * (rs_HSPF - 6.8f) / (10.f - 6.8f),
                        .25f);
    else
...
RC RSYS::rs_CkFAuxHeat() // check aux heat
{
  ...
  if (!rs_CanHaveAuxHeat()) {
    rc |= disallowN(
        "when rsType is not ASHP, ASHPHydronic, ASHPPkgRoom, or VCHP2",
    ...
    rc |= disallowN("when rsTypeAuxH = None", RSYS_CTRLAUXH, RSYS_CAPAUXH,
                    RSYS_FXCAPAUXHTARG, RSYS_AFUEAUXH, 0);
    rs_capAuxH = 0.f;
  } else if (!IsAusz(RSYS_CAPAUXH) && rs_capAuxH == 0.f) { // no auxiliary
    rs_typeAuxH = C_AUXHEATTY_NONE;
  } else { // save input value of rs_capAuxH
...
//-----------------------------------------------------------------------------
RC RSYS::rs_CkFRatio(int fn1,    // field 1
                     int fn2,    // field 2
...
//       mutua
{
  RC rc = IsSet(fn1) ? limitCheckRatio(fn1, fn2, vMin, vMax)
...
RC RSYS::rs_TopRSys1() // check RSYS, initial set up for run
{
  ...
  if (rs_IsVC()) { // VCHP default motor type is BPM (others are PSC from cult)
    if (!IsSet(RSYS_FAN + FAN_MOTTY))
      rs_fan.fn_motTy = C_MOTTYCH_BPM;
  }

  if (rs_CanCool()) { // cooling model air flow correlations have limited
                      // validity range
    ...
    if (rs_IsFanCoil()) { // nothing to check?

    } else { // compression cooling
      ...
      if (rs_IsWSHP()) {
      ...
      } else { // default/harmonize 95 F COP
        ...
        if (rs_IsPkgRoom()) { // pkg: derive SEER from EER
          ...
          else if (!rs_IsVCClg()) { // estimate missing EER from SEER
  ...
  if (rs_CanHeat()) {
    ...
    if (rs_IsHP()) {
      ...
      if (IsAusz(RSYS_CAPH) || IsAusz(RSYS_CAP47) ||
          IsAusz(RSYS_CAPAUXH)) {  // rs_capH for WSHP, rs_cap47 for ASHP
    ...
    } else if (IsAusz(RSYS_CAPH)) { // other checking?
...
RC RSYS::rs_TopRSys2() // final set up for run
{
  ...
  RLUP(DsR, ds) {
    ...
    if (ds->ownTi == ss) {          // if duct segment part of this system
    ...
    int isAusz)      // TRUE = autosize, FALSE = main simulation
{
  ...
  rs_isAuszH = rs_auszH.az_fazInit(&rs_capH, FALSE, this, RSYS_CAPH, isAusz,
                                   "RSYS[%s] capH");
  rs_isAuszC = rs_auszC.az_fazInit(&rs_cap95, FALSE, this, RSYS_CAP95, isAusz,
                                   "RSYS[%s] cap95");
...
//-----------------------------------------------------------------------------
RC RSYS::rs_RddInit(
    int isAusz) // init before each autosize design day and main sim
{
  isAusz;
  return RCOK;
} // RSYS::rs_RddInit
...
void RSYS::rs_RddiInit() // init before each autosize design day ITERATION
{
  ...
  {
    if (Top.tp_pass1A) { // pass1A: warmup with fixed air flow
      float cfmPerFt2 = rs_IsHP() ? 0.6f : 0.4f;
      rs_SetupCapH(rs_areaServed * cfmPerFt2); // rs_capH derived from AVF
    } else { // pass1B: rs_capH has latest value
      if (Top.tp_auszDsDayItr < 2)
        rs_capH =
            rs_auszH.az_a; // 1st iteration cap may be 0 (see rs_pass1AtoB)
      rs_SetupCapH();
    }
    ...
    } else { // pass1B: rs_cap95 has latest value
      if (Top.tp_auszDsDayItr < 2)
        rs_cap95 =
            rs_auszC.az_a; // 1st iteration cap may be 0 (see rs_pass1AtoB)
      rs_SetupCapC();
    }
  }
} // RSYS::rs_RddiInit
...
{
  RC rc = RCOK;
  if (auszMode == rsmHEAT) { // note: rs_fxCapHDay = 20 if no load
    float f = rs_fxCapHAsF / max(rs_fxCapHDay, .01f);
    if (Top.tp_pass1A) { // pass1A (warmup): system always has fixed size
      //   after 1st iter, change rs_capH at will (used only for convergence
      //   test) rs_capH re-inited in rs_RddiInit
      if (Top.tp_auszDsDayItr > 1)
        rs_capH *= f;
    } else { // pass1B: rs_capH sizes system
  ...
  } else if (auszMode == rsmCOOL) {     // note: rs_fxCapCDay = 20 if no load
    float f = rs_fxCapCAsF / max(rs_fxCapCDay, .01f);
    if (Top.tp_pass1A) { // pass1A (warmup): system always has fixed size
      //   after 1st iter, change rs_cap95 at will (used only for convergence
      //   test) rs_cap95 re-inited in rs_RddiInit
      if (Top.tp_auszDsDayItr > 1)
        rs_cap95 *= f;
    } else { // pass1B: rs_cap95 sizes system
      //   change in small steps (can be unstable)
...
//-----------------------------------------------------------------------------
RC RSYS::rs_pass1AtoB() // call at transition from autoSize pass 1 part A to
                        // part B for each design day

// called between iterations of a design day (between tp_SimDays()'s without
// reInit) at the change from const-temp size-finding open-ended models to the
// real models.
{
  ...
  // on return, caller will increase value to any larger converged part B value
  // (.b) then call RSYS::begP1b, next.
...
//-----------------------------------------------------------------------------------------------------------------------------
RC RSYS::rs_begP1b() // called b4 start of pass 1 part b iterations for des day,
                     // after value set to max part B value seen.
{
// reSetup models being autoSized, cuz AUSZ stuff and/or rs_pass1AtoB above may
// have changed values.
#if 0
...
//-----------------------------------------------------------------------------
RC RSYS::rs_endAutosize() // call at end successful autoSize for possible
                          // additional checks/messages
{
  return RCOK;
} // rs_endAutosize
//-----------------------------------------------------------------------------
void RSYS::rs_AuszFinal() // called at end of successful autosize (after all
                          // xx_endAutosize)
{

  RSYS *rsi = RSiB.GetAtSafe(ss);

  // member function sets _As, _AsNov, and input record members for each AUSZ's
  // 'x'.
  ...
  if (rs_isAuszH) {
    float capHBase =
        rs_auszH.ldPkAs1 / rs_fxCapHAsF; // max of all design days
                                         //  remove working cap factor

    rs_capH = capHBase * rs_fxCapHTarg; // apply user's oversize factor

    if (rs_IsASHP()) {
      if (IsAusz(RSYS_CAP47)) { // find cap47 that produces required output
        ...
        if (frDiff(capCheck, capExpected, 1.f) > .001f)
          oWarn(
              "ASHP heating capacity (%.1f) autosize mismatch (expected %.1f)",
              capCheck, capExpected);
#endif
      }
    }

    if (IsAusz(RSYS_CAPAUXH)) { // ASHP aux heat autosize cap = full load w/
                                // user oversize
  ...
  if (rs_IsASHP()) {
    if (rs_isAuszC) {
      if (IsAusz(RSYS_CAP47)) { // both autosized
        ASHPConsistentCaps(rs_cap95, rs_cap47, IsSet(RSYS_CAPRAT9547),
                           rs_capRat9547);
      }
    ...
    if (rs_isAuszC) {
      if (IsAusz(RSYS_CAPH)) { // both autosized
        WSHPPerf.whp_ConsistentCaps(rs_cap95, rs_capH, IsSet(RSYS_CAPRATCH),
                                    rs_capRatCH);
      }
...
//-----------------------------------------------------------------------------
float RSYS::rs_ClgCapForAMF(float amf) const // air mass flow rate, lbm/hr
...
//-----------------------------------------------------------------------------
float RSYS::rs_HtgCapForAMF(float amf) const // air mass flow rate, lbm/hr
...
//-----------------------------------------------------------------------------
float RSYS::rs_AMFForHtgCap(float capH) const // heating capacity
// returns air mass flow, lbm/hr
{
...
//-----------------------------------------------------------------------------
int RSYS::rs_CanHaveDucts(int iHC) const // 0=htg, 1=clg
// return nz iff
{
...
// sets rs_OAVAvfD and rs_OAVFanPwrD
{
  ...
  if (rs_OAVType ==
      C_RSYSOAVTYCH_VARFLOW) { // NightBreeze algorithm, reference =
    ...
    float afUnlim =
        float(1. / pow(1. + exp(653. - 8.152 * Wthr.d.wd_taDbAvg), 0.010654));
...

{
  ...
  if (okOAV > 0) { // at least 1 zone wants OAV
    ...
    double fSize = rs_amfReq[0] > 0. // fraction of total air that is avaailable
                       ...
                       : 0.;
    RLUPC(ZrB, zp,
          rs_IsZoneServed(zp)) { // set zn_rsAmfSup, zn_rsAmfRet, and zn_rsFSize
      zp->zn_SetRSYSAmf(fSize, 0);
      double mCp = zp->zn_rsAmfSup * Top.tp_airSH;
      double tzx = zp->zn_TAirCR(mCp * tSup, mCp, 0.);
      if (tzx >
          zp->zn_tzspC /* && RSYS can cool */) { // OAV cannot hold tz below
                                                 // cooling set point, disable
        okOAV = 0;
        break;
      }
    }
  ...
  if (okOAV <= 0) { // OAV not OK, revert any partial results
    RLUPC(ZrB, zp, rs_IsZoneServed(zp)) {
      ...
      ZnresB[zp->ss].curr.S.nShVentH =
          0; // vent heating not possible if no vent
    }
    rs_mode = rsmOFF;
    return -1;
  }
  ...
  RLUPC(ZrB, zp, rs_IsZoneServed(zp)) {
    ...
    if (ZnresB[zp->ss].curr.S.nShVentH) {
      ...
      int warnCount =
          ZnresB[zp->ss].zr_GetAllIntervalTotal(ZNRES_IVL_SUB_NSHVENTH);
...
//       1  OAV useful
{
  ...
  double amfOAV;
  if (tSup >= zn_tzsp) { // OAV causes heating, run full volume
#if defined(_DEBUG)
    if (tSup > 100.)
...
// redundant calls OK
{
  ...
  for (int iHC = 0; iHC < 2; iHC++)
    for (int iSR = 0; iSR < 2; iSR++) {
      int iDS = rs_Dsi(iSR, iHC);
      rs_ducts[iHC].ductLkXF[iSR] =
          iDS > 0 ? 1.f - min(DsR[iDS].ds_leakF, .99f) : 1.f;
    }
  ...
  if (avfH > 0.f) {
    ...
    if (rs_IsASHP()) {
      if (Top.tp_autoSizing && !(options & 1)) {
        ...
        rc |= rs_SetupASHP(); // set default capacities/efficiencies, derive
                              // constants
      ...
      rc |= rs_SetupWSHP(); // set default capacities/efficiencies, derive
                            // constants
                            //  multiple calls OK (e.g. during autosizing)
                            //  final call needed after autosize complete
      nomCap = rs_capH;
    ...
    rs_amfH =
        nomCap /
        (rs_tdDesH *
         Top.tp_airSH); // nominal full speed dry-air mass flow rate, lb/hr
...
// returns fan heat included in ratings, Btuh
{
  float fanHRtdPerTon =
      ...
      : rs_IsWSHP()
          ? 212.f  // WSHP: rated with 0 external static, assume .156 W/cfm
...
//   also can set rs_cap95 (if avfC > 0)
{
  ...
  rs_fanHeatC =
      avfC * rs_fanPwrC * BtuperWh; // full speed operating fan power, Btuh

  rs_fanHRtdC =
      rs_FanHRtdPerTon(capNomTons); // fan heat included in rated capacity, Btuh
...
// sets RSYS cooling members that do not vary during simulation
{
  ...
  float cap95nf = -(fabs(rs_cap95) +
                    rs_fanHRtdC); // coil (gross) total capacity at 95 F, Btuh
                                  // (< 0)
  ...
  if (rs_Is1Spd()) {
    ...
    float inpX =
        1.09f * rs_cap95 / rs_SEER - rs_fanHRtdC / 3.413f; // input power, W
...
RC RSYS::rs_BegHour() {
  ...
  if (Top.isBegDay) { // init capacity ratios for day
...
RC RSYS::rs_EndSubhr() {
  ...
  R.fhTot = R.fhParasitic =
      rs_parFuel * Top.tp_subhrDur; // assign fuel parasitics to heating
  if (rs_parElec > 0.f) { // assign electrical parasitics per current mode or
                          // last active mode
  ...
  if (rs_mode == rsmHEAT) {
    ...
    (rs_IsElecHeat() ? R.ehPrimary : R.fhPrimary) =
        rs_inPrimary * Top.tp_subhrDur;
    if (rs_IsFuelAuxH()) {
    ...
    for (int iM = 0; iM < 2; iM++) { // accumulate values to LOADMETERs
      //   [0] accums both heating and cooling
      //   [1] accums heating
      if (rs_pLoadMtr[iM]) // primary = coil loads
        rs_pLoadMtr[iM]->S.qHtg += R.qhPrimary;
      if (rs_pSrcSideLoadMtr[iM]) // source-side (= "outdoor coil" or
                                  // environment source)
        // + = from env: >0 during heating = coil heating - compressor
        // electricity
    ...
    for (int iM = 0; iM < 3; iM += 2) { // accumulate values to LOADMETERs
      //   [0] accums both heating and cooling
      //   [2] accums cooling
      if (rs_pLoadMtr[iM]) // primary = coil loads
      ...
      if (rs_pSrcSideLoadMtr[iM]) // source-side (= "outdoor coil" or
                                  // environment source)
        // + = from env: <0 when cooling = total cooling + compressor
        // electricity
        rs_pSrcSideLoadMtr[iM]->S.qClg += R.qcSen + R.qcLat - R.ecPrimary;
    }
  } else if (rs_mode == rsmOAV) { // sensible "output" = fan power
    R.evFan = eFan;
  ...
  // rsmHEAT/rsmCOOL/rsmOAV definitions must be consistent with member
  // sequences.
#define QZONECHK(m, oDif)                                                      \
  static_assert(&(((RSYSRES_IVL_SUB *)0)->m) -                                 \
                        &(((RSYSRES_IVL_SUB *)0)->qhZoneSen) ==                \
                    oDif,                                                      \
                "Bad seq " #m)
...
#undef QZONECHK
  ...
  if (rs_mode != rsmOFF) { // heat transfers from this RSYS to all zones
                           // (including duct losses)
    const ZNR *zp;
    int iMX = 2 * (rs_mode - 1);
    RLUPC(ZrB, zp, rs_IsZoneServed(zp)) {
...

{
  ...
  if (Top.isEndDay) { // last step of day
    int auszMode = rs_IsAutoSizing();
    if (auszMode != rsmOFF)
...
{
  // rs_asOut = rs_asIn;		// init to entering state (by caller)
  ...
  if (auszMode == rsmHEAT &&
      Top.tp_pass1A) { // autosize warmup: assume fixed temp rise
    rs_capHt = capHt =
        rs_asOut.as_CalcQSen2(rs_asRet.as_tdb + rs_tdDesH, rs_amf);
    rs_effHt = 1.f; // need nz value, else ASHP assumes compressor off
  } else if (rs_IsASHP()) {
    if (auszMode != rsmOFF) { // ASHP heat autosize (based on rs_capH)
      rs_effHt = 1.f;
      rs_capHt = capHt = rs_capH;
      rs_capAuxH = rs_capH; // same cap for aux during autosizing
                            //   used below if needed
    } else {                // ASHP simulation (not autosize)
      // run full model
    ...
    float capF, inpF;
    /*rc |=*/WSHPPerf.whp_HeatingFactors(capF, inpF, rs_tdbOut, rs_tdbCoilIn,
                                         airMassFlowF);
    rs_capHt = capHt = (rs_capH - rs_fanHRtdH) * capF; // gross heating capacity
    float inpX =
        ((rs_capH / rs_COP47) - rs_fanHRtdH) * inpF; // gross input power
    rs_effHt = rs_capHt / inpX * rs_fEffH;
  } else if (rs_IsCHDHW()) {
    if (rs_capHt == 0.f) { // do initial setup only once
      rs_CurCapHtCHDHW();
    }
    capHt = rs_capHt * rs_speedF;
    rs_effHt = 1.f;
  } else { // not heat pump of any type
  ...
  if (rs_CanHaveAuxHeat() && rs_speedF == 1.f) {
    ...
    float fanHeatAux;
    if (rs_ctrlAuxH ==
        C_AUXHEATCTRL_CYCLE) { // aux cycles: aux out = prim out + aux heat
      //     fan heat already in prim out
      rs_asOutAux = rs_asOut;
      fanHeatAux = 0.f;
    } else { // aux alternate or locked out: prim off
...
// return: fCondCap = current conditions factor for total gross capacity
{
  ...
  if (SHR > 0.9999f)
    fCondCap = // dry coil
        ...
        // + 0.f        * twbCoilIn
        - 0.000600600f * tdbOut - 0.000148900f * vfPerTon -
        ...
        - 52.561740000f / vfPerTon + 0.430751600f;
  else
    fCondCap = // wet coil
               // 0.f *       * tdbCoilIn
        +0.009645900f * twbCoilIn + 0.002536900f * tdbOut +
        0.000171500f * vfPerTon
        // + 0.f       * tdbCoilIn* tdbOut
        // + 0.f       * tdbCoilIn* vfPerTon
        - 0.000095900f * twbCoilIn * tdbOut +
        0.000008180f * twbCoilIn * vfPerTon - 0.000007550f * tdbOut * vfPerTon +
        0.000105700f * twbCoilIn * twbCoilIn - 53.542300000f / vfPerTon +
        0.381567150f;
...
// return: fCondCap = current conditions factor for total gross capacity
{
  ...
  rs_fCondCap =
      CoolingCapF1Spd(rs_SHR, tdbOut, rs_tdbCoilIn, rs_twbCoilIn, rs_vfPerTon);
  return rs_fCondCap;
...
// return: fCondInp = current conditions factor for compressor input
{
  ...
  if (SHR > 0.9999f) {
    fBase = 0.0046103f * tdbCoilIn // dry coil
                                   // + 0.f     * twbCoilIn
            + 0.0125598f * tdbOut - 0.000512f * vfPerTon -
            0.0000357f * tdbCoilIn * tdbOut + 0.0000105f * tdbCoilIn * vfPerTon;
    // + 0.f     * twbCoilIn * tdbOut
    // + 0.f     * twbCoilIn * vfPerTon
    // + 0.f     * tdbOut * vfPerTon
    // + 0.f     * twbCoilIn * twbCoilIn
    ...
    fBase = // 0.f	 * tdbCoilIn
        -0.0202256f * twbCoilIn + 0.0236703f * tdbOut -
        0.0006638f * vfPerTon
        // + 0.f     * tdbCoilIn* tdbOut
        // + 0.f     * tdbCoilIn* vfPerTon
        - 0.0001841f * twbCoilIn * tdbOut + 0.0000214f * twbCoilIn * vfPerTon -
        0.00000812f * tdbOut * vfPerTon + 0.0002971f * twbCoilIn * twbCoilIn -
        27.95672f / vfPerTon;
...
// returns input power modifier
{
  ...
  float fInpSEER;
  float fInpEER = CoolingInpF1Spd(rs_SHR, tdbOut, rs_tdbCoilIn, rs_twbCoilIn,
                                  rs_vfPerTon, fInpSEER);
  ...
  rs_EERt = tdbOut < 82.f ? rs_SEERnf
            : tdbOut < 95.f
                ? rs_SEERnf + (tdbOut - 82.f) * (rs_EERnf - rs_SEERnf) / 13.f
                : rs_EERnf;
...
//-----------------------------------------------------------------------------
void RSYS::rs_CoolingEnteringAirFactorsVS( // adjustments for entering (indoor)
                                           // air state
    float &capF,                           // returned: capacity factor
    float &inpF) const // returned: compressor input power factor
{
  float capFN = CoolingCapF1Spd(rs_SHR, rs_tdbOut, rs_tdbCoilIn, rs_twbCoilIn,
                                rs_vfPerTon);
  float capFD = CoolingCapF1Spd(0.7f, rs_tdbOut, 80.f, 67.f, 400.f);
  capF = capFN / capFD;

  float fSink;
  float inpFN = CoolingInpF1Spd(rs_SHR, rs_tdbOut, rs_tdbCoilIn, rs_twbCoilIn,
                                rs_vfPerTon, fSink);
  float inpFD = CoolingInpF1Spd(0.7f, rs_tdbOut, 80.f, 67.f, 400.f, fSink);

  inpF = inpFN / inpFD;

} // RSYS::rs_CoolingEnteringAirFactorsVS
//-----------------------------------------------------------------------------
...
// derives RSYS outlet conditions (before supply distribution losses) for
// cooling mode
...
x		printf("\nhit");
  ...
  if (rs_IsFanCoil()) {
    ...
    //  total, sensible
    if (auszMode == rsmCOOL &&
        Top.tp_pass1A) { // autosizing warmup: assume coil can produce design
                         // delta-T
      ...
      /* rc |= */ WSHPPerf.whp_CoolingFactors(capF, capSenF, inpF, rs_tdbOut,
                                              rs_tdbCoilIn, rs_twbCoilIn,
                                              airMassFlowF);
      rs_capTotCt =
          rs_capnfX * capF; // total gross capacity at current conditions, Btuh
      rs_capSenCt = rs_SHRtarget * rs_capTotCt *
                    capSenF; // SHR set using air-to-air approach
    ...
    } else if (rs_Is1Spd()) { // 1 spd, not autosizing warmup, use full model
      rs_CoolingCapF1Spd();   // use 1 spd model for both 1 spd
      rs_capTotCt =
          rs_capnfX *
          rs_fCondCap; // total gross capacity at current conditions, Btuh
      rs_capSenCt =
          rs_SHR *
          rs_capTotCt; // sensible gross capacity at current conditions, Btuh

      rs_CoolingEff1Spd(); // derive rs_EERt

      rs_effCt = rs_SHR * rs_fEffC * rs_EERt /
                 BtuperWh; // compressor efficiency (COP)
      ...
      /* RC rc = */ rs_GetPerfBtwxt(rs_pRgiClg, rs_tdbOut, capClg, inpClg,
                                    capClgMin, inpClgMin);

      capClgMin = bracket(capClg, capClgMin, 0.05f * capClg);
      inpClgMin = bracket(-capClgMin / 10.f, inpClgMin, inpClg);

      rs_speedFMin = capClg >= 0.f
                         ? 1.f
                         : capClgMin / capClg; // rs_speedFMin = min/max ratio
      ...
      rs_capTotCt // total coil capacity (= gross) at current conditions and
                  // speed, Btuh
          = (capClg - rs_fanHRtdC) * rs_fChg * rs_fCondCap * rs_speedF;
      rs_capSenCt = rs_SHR * rs_capTotCt; // sensible coil capacity at current
                                          // conditions and speed, Btuh

      float inpX =
          (inpClg - rs_fanHRtdC) *
          fCondInp; // full speed input power at current conditions w/o fan
      if (rs_speedF < 1.f) {
        //  capTotMinCt = (capClgMin - rs_fanHRtdC) * rs_fChg * rs_fCondCap *
        //  rs_speedFMin;
        float inpXMin =
            (inpClgMin - rs_fanHRtdC * rs_speedFMin) *
            fCondInp; // min speed input power at current conditions w/o fan

        inpX = inpClgMin + (inpX - inpXMin) * (rs_speedF - rs_speedFMin) /
                               (1.f - rs_speedFMin);
      }
...
x	rs_asOut = asSav;
  ...
  if (ret == 1) { // as_AddQSenLat() applied humidity limit
    //   update capacities
  ...
  rs_capSenNetFS =
      rs_capSenCt / rs_speedF + rs_fanHeatC; // net full speed sensible capacity
...
{
  pwrIn = (fabs(rs_capSenCt / rs_effCt) + rs_fanHeatC) / (BtuperWh * 1000.f);
...
// testing aid
{
  ...
  fprintf(
      f,
      "cfm,IDB (F),IWB (F),ODB (F),CapTot (kBtuh),CapSen (kBtuh),Pwr (kW)\n");
  ...
  for (int iAF = 0; fAF[iAF] > 0.f; iAF++) {
    ...
    for (int idbI = 0; tdbI[idbI] > 0.f; idbI++) {
      for (int iwbI = 0; twbI[iwbI] > 0.f; iwbI++) {
        ...
        for (int idbO = 0; tdbO[idbO] > 0.f; idbO++) { // outdoor conditions
          rs_asOut = rs_asIn;
          rs_tdbOut = tdbO[idbO];
          ...
          fprintf(f, "%.f,%.f,%.f,%.f,%.2f,%.2f,%.2f\n", cfm, tdbI[idbI],
                  rs_twbIn, rs_tdbOut, capTot, capSen, pwrIn);
        }
      }
    }
  }
...
//-------------------------------------------------------------------------------
RC RSYS::rs_ExportCorrelationValues() // write CSV file containing values from
                                      // RSYS correlations
// testing aid
{
  static float fAF[] = {.550f, .700f, .875f, 1.f, 1.125f, 0.f};
  ...
  for (int iAF = 0; fAF[iAF] > 0.f; iAF++) {
    ...
    for (int idbO = 0; tdbO[idbO] > 0.f; idbO++) { // outdoor conditions
      ...
      for (int idbI = 0; tdbI[idbI] > 0.f; idbI++) { // entering dry bulb
        rs_tdbCoilIn = tdbI[idbI];
        char sDbI[20];
        sprintf(sDbI, "%.f", rs_tdbCoilIn);
        for (int iwbI = 0; twbI[iwbI] > 0.f;
             iwbI++) { // entering wet bulb conditions
          ...
          fprintf(f, "%s,%s,%s,%.f,%.3f,%.3f,%.3f,%.3f,%.3f\n", sVf, sDbO, sDbI,
                  rs_twbCoilIn, rh, rs_SHR, rs_fCondCap, rs_fCondEER, fCondInp);
        }
      }
    }
  }
...
RC RSYS::rs_PerfDataASHP() {
  ...
  fprintf(f, "%s,All values include rating fan power,,%s,CSE %s\n",
          rs_desc.CStrIfNotBlank(nameX), Top.runDateTime.CStr(),
          Top.tp_progVersion.CStr());

  fprintf(f,
          "HSPF=%0.2f, HSPF CSE=%0.3f,,HSPF MP=%0.3f,,HSPF ESL=%0.3f,,HSPF "
          "E+=%0.3f\n",
          rs_HSPF, rs_HSPFCheckASHP(0), rs_HSPFCheckASHP(1),
          rs_HSPFCheckASHP(2), rs_HSPFCheckASHP(3));

  fprintf(f, "tODB,cap CSE,inp CSE,COP CSE,cap MP,inp MP,COP MP,cap ESL,inp "
             "ESL,COP ESL,cap E+,inp E+,COP E+\n");
  ...
  const float hspfs[] = {6.5f,  7.0f,  7.5f,  8.0f, 8.5f,  9.0f, 9.5f, 10.0f,
                         10.5f, 11.0f, 11.5f, 12.f, 12.5f, 13.f, -1.};
...
// all values include rating fan power
{
  ...
  fprintf(
      f,
      "%.f, %.f, %.f, %.3f, %.f, %.f, %.3f, %.f, %.f, %.3f, %.f, %.f, %.3f\n",
      tdbOut, cap, inp, COP, capMP, inpMP, COPMP, capESL, inpESL, COPESL, capEP,
      inpEP, COPEP);
...
// returns total heating capacity (compressor + capDefrostHt), Btuh
{
  ...
  COP = rs_PerfASHP2(ashpModel, tdbOut, fanHAdj, capHt, inpHt, capDfHt,
                     capHtMin, inpHtMin, capDfHtMin);
...
float RSYS::rs_PerfASHP2( // ASHP performance
    ...
    float
        &capHtMin, // returned: compressor-only min speed heating capacity, Btuh
...
// returns compressor-only full-speed COP (as adjusted by COPAdjF)
{
  ...
  if (ashpModel == 0) {
    int iDefrost = rs_HasDefrost() && tdbOut > 17.f &&
                   tdbOut < 45.f; // 1 if defrost, else 0
    if (rs_IsASHPVC()) {          // retrieve min/max cap and input at tdbOut
                         //   rs_pRgiHtg[ 0] includes 5, 17, 35, and 47 F points
                         //   values include fan heat/power
      /* rc = */ rs_GetPerfBtwxt(rs_pRgiHtg[0], tdbOut, capHt, inpHt, capHtMin,
                                 inpHtMin);
#if defined(_DEBUG)
      if (capHt < capHtMin)
        printf("\ncapHt < capHtMin");
      ...
      if (iDefrost && bDoDefrost) { // C_RSYSDEFROSTMODELCH_REVCYCLEAUX
        ...
        /* rc = */ rs_GetPerfBtwxt(rs_pRgiHtg[1], tdbOut, capHtSS, inpSink,
                                   capHtSSMin, inpSink);
    ...
    if (ashpModel == 1) { // Micropas curves from Ken Nittler 5-2013)
      ...
      fCap =
          0.302 + 0.0121 * tdbOut + 0.0000759 * tdbOut2 - 0.000000384 * tdbOut3;
    ...
    } else if (ashpModel == 2) { // ESL curves from Juan-Carlos Baltazar (ESL,
                                 // Texas A&M), 5-2013
      // From DOE-2.1E?
      ...
      // EIR (curve SDL-C56)
      fEIR = 2.03914613 - 0.03906753 * tdbOut + 0.00045617 * tdbOut2 -
             0.00000203 * tdbOut3;
      ...
      //  however, they discovered after the fact that RESNET procedure is based
      //  on
      //    rated HSPF (with fans)
      float HSPFX = 1. / ((1. / rs_HSPF) - .01095);
      float EIR47X =
          .582 * (1. / (HSPFX / 3.413)); // compressor-only EIR @ 47 F

      // capNet?
      inpHt = capHt * fEIR * EIR47X + fanHAdj; // input w/o fan
    } else if (ashpModel ==
               3) { // EnergyPlus model with Cutler/NREL coefficients

      // Data from tables 12 and 13 of NREL/TP-5500-56354 (Cutler et al.)
      // January 2013
      ...
      static const double epCap[6][6] = {
          {3.68637657, -0.098352478, 0.000956357, 0.005838141, -0.0000127,
           -0.000131702},
          {3.998418659, -0.108728222, 0.001056818, 0.007512314, -0.0000139,
           -0.000164716},
          {3.466810106, -0.091476056, 0.000901205, 0.004163355, -0.00000919,
           -0.000110829},
          {0.566333415, -0.000744164, -0.0000103, 0.009414634, 0.0000506,
           -0.00000675},
          {0.335690634, 0.002405123, -0.0000464, 0.013498735, 0.0000499,
           -0.00000725},
          {0.306358843, 0.005376987, -0.0000579, 0.011645092, 0.0000591,
           -0.0000203}};
      // EIR coefficients
      static const double epEIR[6][6] = {
          {-3.437356399, 0.136656369, -0.001049231, -0.0079378, 0.000185435,
           -0.0001441},
          {-4.282911381, 0.181023691, -0.001357391, -0.026310378, 0.000333282,
           -0.000197405},
          {-3.557757517, 0.112737397, -0.000731381, 0.013184877, 0.000132645,
           -0.000338716},
          {0.718398423, 0.003498178, 0.000142202, -0.005724331, 0.00014085,
           -0.000215321},
          {0.36338171, 0.013523725, 0.000258872, -0.009450269, 0.000439519,
           -0.000653723},
          {0.981100941, -0.005158493, 0.000243416, -0.005274352, 0.000230742,
           -0.000336954}};
      ...
      fCap = epCap[iHS][0] + epCap[iHS][1] * tdbEnt + epCap[iHS][2] * tdbEnt2 +
             epCap[iHS][3] * tdbOut + epCap[iHS][4] * tdbOut2 +
             epCap[iHS][5] * tdbX;
      capHt = fCap * rs_cap47;

      fEIR = epEIR[iHS][0] + epEIR[iHS][1] * tdbEnt + epEIR[iHS][2] * tdbEnt2 +
             epEIR[iHS][3] * tdbOut + epEIR[iHS][4] * tdbOut2 +
             epEIR[iHS][5] * tdbX;
...
// redundant calls OK
{
  ...
  if (!IsSet(RSYS_CAP95))
    rs_cap95 =
        ASHPCap95FromCap47(rs_cap47, IsSet(RSYS_CAPRAT9547), rs_capRat9547);
  else if (!IsSet(RSYS_CAP47))
    rs_cap47 =
        ASHPCap47FromCap95(rs_cap95, IsSet(RSYS_CAPRAT9547), rs_capRat9547);
  ...
  rs_fanHRtdH =
      rs_IsASHPHydronic() ? 0.f : rs_FanHRtdPerTon(rs_cap47 / 12000.f);
  ...
  if (!IsSet(RSYS_COP47))
    rs_COP47 =
        0.3038073f * rs_HSPF - 1.984475f * rs_cap17 / rs_cap47 + 2.360116f;
  if (!IsSet(RSYS_COP17))
    rs_COP17 =
        0.2359355f * rs_HSPF + 1.205568f * rs_cap17 / rs_cap47 - 0.1660746;
#elif ASHP_COPREG == 2
  // Revised 5-31-2013
  if (!IsSet(RSYS_COP47))
  ...
  if (!IsSet(RSYS_COP17))
    rs_COP17 =
        rs_HSPF < 8 ? 0.5183f * rs_HSPF - 1.7244f : 0.2186f * rs_HSPF + 0.6734f;
#elif ASHP_COPREG == 6
  if (rs_IsPkgRoom()) {
    rs_COP17 = 0.6870f * rs_COP47;
    // rs_cap17 set above
  } else if (!rs_IsASHPHydronic()) { // COP47: default as per ASHP_COPREG == 2
...
// returns ratio cap17 / cap47
{
  ...
  //  includes HSPF < 7.5
  float capRat = IsSet(RSYS_CAPRAT1747) ? rs_capRat1747
                 : rs_IsPkgRoom()       ? .6184f // per Abram Conant, 6-4-2020
                 : rs_IsASHPHydronic()
                     ? .63f // plausible result when HSPF not known
...
// redundant calls OK
{
  ...
  if (!IsSet(RSYS_CAP95))
    rs_cap95 =
        WSHPPerf.whp_CapCFromCapH(rs_capH, IsSet(RSYS_CAPRATCH), rs_capRatCH);
  else if (!IsSet(RSYS_CAPH))
    rs_capH =
        WSHPPerf.whp_CapHFromCapC(rs_cap95, IsSet(RSYS_CAPRATCH), rs_capRatCH);
...
// returns RCOK iff CHDHW config is valid
{
  ...
  if (!rc) {
    ...
    rs_fanHRtdH =
        blowerEfficacy * rs_pCHDHW->hvt_GetRatedBlowerAVF() * BtuperWh;
  }
...
float RSYS::rs_CapDfHtCurSpeedF() const {
  float capDfHt = rs_speedF == 1.f
                      ? rs_capDfHt
                      : rs_capDfHtMin + (rs_capDfHt - rs_capDfHtMin) *
                                            (rs_speedF - rs_speedFMin) /
                                            (1.f - rs_speedFMin);
  return capDfHt;
...
float RSYS::rs_InpHtCurSpeedF() const {
  float inpHt = rs_speedF == 1.f
                    ? rs_inpHt
                    : rs_inpHtMin + (rs_inpHt - rs_inpHtMin) *
                                        (rs_speedF - rs_speedFMin) /
                                        (1.f - rs_speedFMin);
  return inpHt;
...
RC RSYS::rs_SetupBtwxt( // init/populate btwxt for heating runtime interpolation
    ...
    Btwxt::RegularGridInterpolator *
        &pRgi, // returned: heap ptr to Btwxt interpolator object
...

{
  ...
  Btwxt::GridAxis dbtRange(gridODB, Btwxt::InterpolationMethod::linear,
...
float RSYS::rs_CapEffASHP(    // performance at current conditions
    float tdbOut /*=-999.f*/, // outdoor dry-bulb temp, F
                              //   default = rs_tdbOut (weather file value or
                              //   expression)
...
// returns rs_capHt, Btuh = total heating capacity including fan and defrost if
// any
{
  if (tdbOut < -998.f)
    tdbOut = rs_tdbOut;
  ...
  if (tdbOut < rs_ASHPLockOutT) {
    ...
    rs_effHt =
        rs_PerfASHP2(ashpModel, tdbOut, fanHRtd, rs_capHt, rs_inpHt, rs_capDfHt,
...
// main simulation variant of rs_CapEffASHP() (see above)
{
  if (rs_tdbOut < rs_ASHPLockOutT) {
    ...
    rs_effHt =
        rs_PerfASHP2(0, rs_tdbOut, rs_fanHRtdH, rs_capHt, rs_inpHt, rs_capDfHt,
...
// returns RCOK iff COP17 found
{
  ...
  int ret = secant(rs_CallCalcHSPF, this, rs_HSPF, .001, COP17, hspf, // x1, f1
                   COP17 + dCOP17);                                   // x2 (f2)
  if (ret != 0)
...
// returns HSPF calculated from RSYS properties
{
  ...
  static double fHrReg4[] =
      //   62      57    52 ...
      {0.132, 0.111, 0.103, 0.093, 0.100, 0.109, 0.126, 0.087,
       0.055, 0.036, 0.026, 0.013, 0.006, 0.002, 0.001, 0.};
  ...
  rs_CapEffASHP(47.f,
                ashpModel + 0x100); // debug aid (check values with fan power)
  ...
  for (int iBin = 0; nHrReg4[iBin] > 0; iBin++) {
    ...
    rs_CapEffASHP(tDbBin, ashpModel + 0x100, 0.f, 0.f,
                  1.f); // derive capacity and efficiency for bin temp
    ...
    // other factors
    double X = rs_capHt > 0.f ? min(BL / rs_capHt, 1.)
                              : double(BL > 0.); // 1. iff load else 0.
    double PLF = 1. - cd * (1. - X);

    double Eh = rs_effHt > 0. ? rs_capHt / rs_effHt // input, Btuh
                              : 0.;
    ...
    if (bDoPrint)
      DbPrintf("%4.f %6.f %6.f %6.3f %6.f %6.f %6.f\n", tDbBin, outBin,
               rs_capHt, rs_effHt, ehBin, RHBin, inpBin);
  }
...
#else
  x float roCap = capHt <= 40000.f    ? 5000.f x
                  : capHt <= 110000.f ? 10000.f x
                                      : 20000.f;
...
// returns total heating output at rs_tdbOut, Btuh
{
  ...
  int ashpModel = 0;
  float capHt, inpHt, capDfHt, capHtMin, inpHtMin, capDfHtMin;
  rs_PerfASHP2(ashpModel, rs_tdbOut, rs_fanHRtdH, capHt, inpHt, capDfHt,
               capHtMin, inpHtMin, capDfHtMin);
...

{
  ...
  float tF;
  float d;
  if (rs_HasDefrost() && tdbOut > 17.f &&
      tdbOut < 45.f) { // defrost regime: cap based on 17 - 35 slope
    //   pkg ASHP does not defrost
    tF = tdbM17 / (35.f - 17.f);
    float r35 = 0.9f * (r17 + 0.6f * (1.f - r17)); // cap35/cap47
    d = (r17 + tF * (r35 - r17));
  } else { // non-defrost: cap based on 17 - 47 slope
    tF = tdbM17 / (47.f - 17.f);
    d = (r17 + tF * (1.f - r17));
  }
  ...
  double f1 = DBL_MIN;
  int ret = secant(rs_CallCalcCapHt, this, dsnLoad, 20.f, cap47, f1, // x1, f1
                   cap47 + 100.);                                    // x2 (f2)
  if (ret != 0) {
    rc = err(ERR, "RSYS '%s': Cap47 for design load fail (return code=%d)",
             Name(), ret);
    cap47 = cap47Est;
  }
...
// rs_capNomH and rs_capNomC are reporting only
{
  ...
  if (!IsVal(RSYS_CAPNOMH))
    rs_capNomH =
        rs_IsASHP() ? ifNotNaN(rs_cap47, 18000.f) : ifNotNaN(rs_capH, 18000.f);
                         ...
                         //  1: limited clear for speed retry
{
  ...
  // all modes
  rs_loadF = rs_PLR = rs_runF = rs_speedF = rs_runFAux = rs_PLF =
      rs_capSenNetFS = 0.f;
  rs_outSen = rs_outLat = rs_outFan = rs_outAux = rs_outDefrost = rs_outSenTot =
...
//  rs_twbIn = corresponding wet bulb temp, F
{
  ...
  if (rs_mode == rsmOAV)
    ...
    RLUPC(ZrB, zp, rs_IsZoneServed(zp)) { // determine return air state
      //   mix zone states weighted by prior flow
      //   use area as proxy if prior flow not known
      double amfX = rs_mode == rs_modeLs ? zp->zn_rsAmfRetLs : zp->i.znArea;
  ...
  rs_asIn = iDS > 0 ? DsR[iDS].ds_CalcFL(rs_asRet, rs_amf) : rs_asRet;

#if defined(_DEBUG)
  if (rs_mode != rsmOAV && (rs_asIn.as_tdb < 30. || rs_asIn.as_tdb > 110.f)) {
    if (!Top.isWarmup)
  ...
  if (IsSet(RSYS_RHINTEST)) {
    rs_asIn.as_SetWFromRh(
        rs_rhInTest); // testing aid: set w from rh (tdb unchanged)
    rs_rhIn = rs_rhInTest;
  } else
...
// autosized) 			0: mode not available
{
  ...
  if (rs_mode == rsmOFF ||
      speedF != speedFWas) // if was off or speed has changed
...
// autosized) 			0: no amf available, rs_amf = 0
{
  ...
  int rsAvail = 3;
  int auszMode = rs_IsAutoSizing();
  if (auszMode != rsmOFF) { // autosizing underway
    if (auszMode != rsMode)
...
// returns room register air mass flow rate, lbm/hr (after leakage if any)
{
  ...
  } else {                    // supply duct (if any) to get supply state
                                    ...
                                    //    return: state at RSYS outlet
{
  int iHC = rs_DsHC(); // returns nz iff cooling
  float DSE = iHC ? rs_DSEC : rs_DSEH;
  if (DSE > 0.f) { // distribution system efficiency
    //   1-DSE is discarded
    if (DSE < 1.f)
      as.as_MixF(-(1.f - DSE) / DSE, rs_asIn);
    // else leave as unchanged (no duct losses)
  } else { // supply duct (if any)
    int iDS = rs_Dsi(0);
    if (iDS > 0) // if supply duct present
    {
      AIRSTATE asSup(as);
...
double RSYS::rs_ZoneAirRequest( // air quantity needed by zone
    double
        znSupReq, // dry-air mass flow rate required at supply register, lbm/hr
...
//
{
  ...
  if (bAux &&
      rs_ctrlAuxH != C_AUXHEATCTRL_CYCLE) { // check that aux will be helpful
    ...
    if (rs_asSupAux.as_tdb <= rs_asSup.as_tdb) {
      orWarn("Aux heat supply temperature (%0.1f F) <= primary supply "
             "temperature (%0.1f F)."
  ...
  ZNR *zp;
  if (!bAux) { // aux not available or not needed
    if (rs_IsVCMode(rs_mode) && fSize == 1. &&
        !rs_IsAutoSizing()) { // variable capacity with excess capacity
    ...
    RLUPC(ZrB, zp, rs_IsZoneServed(zp))
    zp->zn_SetRSYSAmf(fSize, 0);
  } else if (rs_amfReq[1] > rs_amf) { // full aux insufficient
  ...
  } else if (rs_effHt == 0.f ||
             rs_ctrlAuxH == C_AUXHEATCTRL_LO) { // compressor is unavailable
    ...
    int ret = secant(
        [](void *pO, double &tSup) { // returns 1/reqAMF, 1.e10 iff reqAMF = 0
#if defined(RSYSITERCOUNT)
          rsysIterCount++;
#endif
...
#endif
    double qAux;
    if (rs_ctrlAuxH == C_AUXHEATCTRL_ALT) { // aux alternates with primary
      rs_runFAux = (rs_asOut.as_tdb - asOutNoAux.as_tdb) /
                   (rs_asOutAux.as_tdb - asOutNoAux.as_tdb);
#if defined(_DEBUG)
      if (rs_runFAux < 0.f)
        printf("\nrs_runFAux < 0");
      if (rs_runFAux > 1.f)
    ...
    } else { // aux cycles: determine added heat rqd in addition to prim
...
//         if rs_fxCap[ 0] == 1, pegged at rs_speedFMin (must cycle below min)
{
  ...
  float speedFEst = rs_speedF + (1.f - rs_speedF) * (1.f - fxCapMinSpd) /
                                    (fxCapFullSpd - fxCapMinSpd);
  ...
  printf("\nStart speedFMin=%0.4f  fxCapMinSpd=%0.4f  fxCapFullSpd=%0.4f  "
         "speedFEst=%0.4f",
         rs_speedFMin, fxCapMinSpd, fxCapFullSpd, speedFEst);
#endif

  // search for speedF that produces fxCap = 1.
  //   this RSYS is left in the corresponding state via
  //   rc_TotalAirRequestForSpeed() calls (secant results not directly returned)
  int ret = secant(
      [](void *pO, double &speedF) {
        return ((RSYS *)pO)->rs_FxCapForSpeedF(speedF);
      },
      this, 1., .0005,     // target=rs_fxCap[0]=1 within .0005
      speedF, f1,          // x1, f1
...
//    else RCBAD, rs_fxCap[ 0] = 0
{
  ...
  if (speedF != rs_speedF) {
    ...
    if (!rs_IsSupplyAirTempUseful(
            rs_mode, rs_asSup.as_tdb,
            tSupUseful)) { // adjust rs_speedF to produce useful supply air temp
...
// returns required rs_fxCap[ 0] (= rs_amf / amfReq), >1 = excess capacity
{
  ...
  printf("\n    speedF=%0.4f  f=%0.4f tSup=%0.3f rs_amf=%0.3f rs_amfReq=%0.3f "
         "(speedFLastCall=%0.4f rsSpeedFWas=%0.4f)%s",
         speedF, rs_fxCap[0], rs_asSup.as_tdb, rs_amf, rs_amfReq[0],
         speedFLastCall, rsSpeedFWas, bDup ? " Dup" : "");
  if (bDup) {
...
//-----------------------------------------------------------------------------
RC RSYS::rs_FindSpeedFForTSup( // find speedF that will deliver specified supply
                               // temp
    double tSup) {
  RC rc = RCOK;
  ...
  //   this RSYS is left in the corresponding state via
  //   rc_TotalAirRequestForSpeed() calls (secant results not directly returned)
  [[maybe_unused]] int ret = secant(
      [](void *pO, double &speedF) {
        return ((RSYS *)pO)->rs_TSupForSpeedF(speedF);
      },
      this, tSup, .005,   // target=tSup within .005 degF
      speedFLo, f1,       // x1, f1
...
//-----------------------------------------------------------------------------
double RSYS::rs_TSupForSpeedF(double &speedF) {
#if defined(_DEBUG)
  double speedFWas = speedF;
  // float rsSpeedFWas = rs_speedF;
#endif
...
#if defined(_DEBUG)
  if (speedF != speedFWas)
    printf("\nrs_TSupForSpeedF limit: speedFWas=%0.4f  speedF=%0.4f", speedFWas,
           speedF);
#endif
...
// rs_speedF known
{
  ...
  if (rs_mode == rsmCOOL) {
    rs_outSen =
        rs_runF *
        rs_capSenCt; // average output w/o fan (= gross cooling), Btuh (< 0)
    rs_outLat = rs_runF * rs_capLatCt;
    rs_outFan = rs_inFan =
        rs_runF * rs_speedF * rs_fanHeatC; // all fan heat to air
    runFFan = rs_runF;
    rs_outSenTot =
        rs_outSen + rs_outFan; // net cooling output, Btuh (generally < 0)

    if (rs_IsFanCoil()) { // rs_inPrimary = 0

    } else if (!rs_IsVCClg() || rs_speedF == 1.f ||
               rs_speedF <= rs_speedFMin) { // cycling possible
      //   not variable capacity
      //   or variable capacity below min speed
      rs_PLF = 1.f - rs_CdC * (1.f - rs_runF);
      rs_inPrimary = fabs(
          rs_outSen / max(.01f, rs_effCt * rs_PLF)); // compressor power, Btuh
    } else {
      ...
      rs_PLF = 1.f;
      rs_inPrimary =
          fabs(rs_outSen / max(.01f, rs_effCt)); // compressor power, Btuh
    }

    if (rs_pMtrElec) {
      rs_pMtrElec->H.clg +=
          rs_inPrimary * Top.tp_subhrDur; // compressor energy for step, Btu
      rs_pMtrElec->H.fanC += rs_inFan * Top.tp_subhrDur;
#if defined(_DEBUG)
      if (!rs_IsFanCoil() && rs_pMtrElec->H.clg < .01f &&
          rs_pMtrElec->H.fanC > .01f)
        printf("\nFanC > 0");
#endif
    }
  } else if (rs_mode == rsmHEAT) {
    if (rs_IsHP()) {       // rs_runFAux known at this point
      ...
      {                    // rs_inPrimary = 0.;
        ...
        rs_runF = rs_PLF = 0.f;
        rs_capHt = rs_capHtMin = rs_capDfHt = rs_capDfHtMin =
            0.f; // clear values for reports
                 // aux may be on, handled below
      } else {
        float fFanHeatPrim; // run fraction for both fan and primary
        if (rs_runFAux > 0.f &&
            rs_ctrlAuxH ==
                C_AUXHEATCTRL_ALT) { // aux/primary alternate: primary runs when
                                     // aux does not
        ...
        double outTot =
            rs_runF * rs_CapHtCurSpeedF(); // total primary output (incl fan and
                                           // defrost), Btuh

        rs_outAux = rs_runFAux * rs_capAuxH;

        rs_outFan = min(outTot + rs_outAux,
                        runFFan * rs_speedF *
...
#endif
        if (!rs_IsASHPVC() ||
            rs_speedF == 1.f) { // single speed or variable speed at max
          ...
          rs_COPHtAdj =
              rs_effHt * rs_PLF; // full load efficiency modified by PLF
          rs_inPrimary = rs_outSen / rs_COPHtAdj;
          ASSERT(rs_speedF == 1.f);
        } else if (rs_speedF <=
                   rs_speedFMin) { // variable capacity: cycling at min speed
          ...
          rs_inPrimary = rs_outSen / rs_COPHtAdj;
        } else { // variable capacity: intermediate speed
...
x				rs_inPrimary = rs_outSen / (rs_effHt * rs_PLF);
      ...
      float tRet =
          rs_tCoilEW - rs_outSen * Top.tp_subhrDur / (vol * waterRhoCp);
      ...
      else { // not fancoil, not CHDHW, not AHSP
  ...
  if (runFFan > 0.f) { // duct conduction and leakage to adjacent zones
    for (int iSR = 0; iSR < 2; iSR++) {
      int iDS = rs_Dsi(iSR);
      if (iDS > 0)
...
// accumulates zone .zn_qIzShXAnSh's, assumed pre-0'd.
{
  ...
  zp->zn_qIzSh =
      zp->zn_qIzXAnSh + zp->zn_AnAmfCpT(0) - zp->zn_AnAmfCp(0) * zp->tzls;
...
// called before zone balances
{
  ...
  RLUP(ZrB, zp) // for zp = ZNR record 1 to .n
  {
    if (subhrly) {
      zp->zn_ieMassls =
          zp->zn_ieMass; // re determination of mass internal energy change
      zp->aMassSh = zp->zn_hcATMsSh = zp->zn_hrATMsSh = zp->zn_hcAMsSh =
          zp->zn_hrAMsSh = zp->zn_qCondQS = zp->zn_qCondMS = zp->zn_ieMass = 0.;
  ...
  {
    KIVA *ki;
    RLUP(KvR, ki) { ki->kv_Step(dur); }

    // Loop over surfaces to assign weighted values and accummulate to zone
    // balance
    XSRAT *xr;
    RLUP(XsB, xr) { rc |= xr->xr_ApplyKivaResults(); }
  }

  // Loop over surfaces to accummulate to zone balance
  XSRAT *xr;
  RLUP(XsB, xr) { rc |= xr->xr_KivaZoneAccum(); }
...
RC XSURF::xs_SubhrQS() // subhour quick surface
{
  ...
  if (xs_IsASHWAT())
    ...
    if (xs_sbcO.sb_zi) { // interzone quick surface: handle adjacent zone
...
RC XSURF::xs_EndSubhr() // end-of-subhr calcs for surface
{
  ...
  if (xs_IsASHWAT()) {
    ...
    // accum to zone
    //  zn_ieMass and zn_qCondMS are experimental value re energy balance
    //  checking unused? as of 12-21-10
    z.zn_ieMass += ms.ms_InternalEnergy();
    double qSrfA =
        xs_area * xs_sbcO.sb_qSrf; // conduction at outside surface, Btuh
    z.zn_qCondMS += qSrfA;
    if (sfAdjZi) {
      ZNR &zx = ZrB[sfAdjZi];
      zx.zn_qCondMS -= qSrfA;
    ...
    if (Top.isEndHour) { // last step of hour: store layer boundary temps for
                         // probes
...
// end-subhour after-exprs/reports stuff for loads: set 'prior interval'
// variables etc if done sooner, probes come out wrong.
{
  RC rc = RCOK;
...
#endif
  tzlsDelta =
      tz - tzls; // last subhour t delta: may be used to extrapolate next tz
  tzls = tz;     // last subhr zone air temp for next subHour
  wzlsDelta =
      wz - wzls; // last subhour w delta: may be used to extrapolate next wz
  wzls = wz;     // last subhr zone hum rat for next subHour
  znXLGainLs = znXLGain; // last subhour excess latent gain: next subhour
                         // sensible condensation gain. 5-97.
...
// set 'prior interval' variables etc: if done sooner, probes come out wrong.
{
  ...
  tzlh = tzls; // air temp from final step of hour (NOT average here).
               // needed? used in main eqn in loadsHourBeg, but .tzls cd be used
               // directly 1-92.
...
x	// ?? This is *NOT* the full CEC current algorithm. Where is the 1 degF hysteresis?
  ...
  // default zone shade closure (former shutter fraction): whether movable
  // window shades open or closed for next hour. note this code responds to net
  // cooling, whereas CALRES responded to ANY cooling (zrh->nHrCool, not now
  // implemented). subhourly defaulting desirable (rob's opinion); must
  // duplicate loadsHourBeg sg code in loadsSubhr w znSC-change trigger.
  if (!znSCF) // not if expression given by user
  {
    ZNRES_IVL_SUB *zrh =
        &ZnresB.p[ss].curr.H; // point zone current subhour results in resrat
    i.znSC = (float)(zrh->qsMech <
                     -ABOUT0); // 1.0 if cooling (more than heating) in previous
                               // hour, else 0. -ABOUT0: no cooling if near 0 so
                               // not random when hvac off, 8-92.
  }
#endif
  return RCOK;
} // ZNR::zn_AfterHour
