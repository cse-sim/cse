...
// call *after* lagged values used
{
	...
	// vent fractions = actual mCp / possible mCp
	//   outdoor airflow used to hold zone temp to zn_tzspD
	zn_fVentPrf = 0.f;		// preferred: what this zone wants in isolation
							//   (not limited by e.g. C_VENTAVAILVC_WHOLEBLDG)
	zn_fVent = 0.f;			// outcome as limited
	zn_tzVent = 0.f;		// zone temp with full vent, F
...
// COMPUTE INTERZONE TRANSFERS and base (infil only) airnet
	...
	ULI ventAvail = Top.tp_GetVentAvail();	// overall vent availability
											//    C_VENTAVAILVC_WHOLEBLDG
											//    C_VENTAVAILVC_ZONAL
#if 1 || defined( VENTREV)

	rc |= Top.tp_DoVent(ventAvail);
	float tpFVent = Top.tp_fVent;
	...
	if (ventAvail == C_VENTAVAILVC_WHOLEBLDG || ventAvail == C_VENTAVAILVC_ZONAL)
	{
		...
		{	// vent might be useful for at least 1 zone
			...
			RLUP(ZrB, zp)
			{
				if (!zp->zn_IsUZ())
				{
					int vRet = zp->zn_FVentPreferred();
#if 0
					float fvp = zp->zn_fVentPrf;
					int vRet2 = zp->zn_FVentPreferredOld();
					if (vRet != vRet2 || fvp != zp->zn_fVentPrf)
						printf("\nFVentPreferred Mismatch");
#endif
...
//-----------------------------------------------------------------------------
bool ZNR::zn_IsVentAvail() const	// determine vent availability
// returns true iff zone venting is possible (but not necessarily helpful)
{
	return !zn_IsUZ()			// unconditioned zones cannot have controlled vent
		&& !zn_HasTerminal()	// terminal zones cannot vent (TODO: why not?)
		&& zn_anVentEffect		// there are IZXRATs connected to zone
		&& zn_tzspD > 0.f;		// zone has a specified desired temp
}	// ZNR::zn_IsVentAvail()
//-----------------------------------------------------------------------------
...
{
#if 0 && defined( _DEBUG)
	// works iff airnet vent-on calcs have been done
	float tSupVent = 0.f;
	double anAmfCp = zn_AnAmfCp(1) - zn_AnAmfCp(0);
	if (fabs(anAmfCp) > .01)
	{
		double anAmfCpT = zn_AnAmfCpT(1) - zn_AnAmfCpT(0);
		tSupVent = float(anAmfCpT / anAmfCp);
		if (fabs(tSupVent - Top.tDbOSh) > .01)
			printf("\nVent supply temp mismatch");
	}
#endif
...
// sets zn_ventUt: -1: don't vent; 0: don't care; 1: vent may be helpful
...
#endif
	{	if (tz < zn_tzspD)
			ventUt = vForbid;		// zone temp below TD (any vent would hurt)
		else if (tz > zn_tzspD)
...
//-----------------------------------------------------------------------------
int ZNR::zn_FVentPreferred()		// find zone's preferred vent fraction
...
//          1: useful
{
	...
	int ret = 0;
	// zn_fVentPrf = 0.f;  // in zn_BegSubhr2
	// zn_tzVent = 0.f;	// in zn_BegSubhr2
	...
	{	double anAmfCp = zn_AnAmfCp(1) - zn_AnAmfCp(0);
		...
		{	double amfDT = anAmfCpT - anAmfCp * zn_tzspD;
			...
			{	double qVent = zn_QAirCR( zn_tzspD); // q required to keep tz at zn_tzspD
				...
				if (zn_fVentPrf > 0.f)
				{
					float fV = zn_fVentPrf;
					double anAmfCpVent = fV * (zn_AnAmfCp(1) - zn_AnAmfCp(0));
					double anAmfCpTVent = fV * (zn_AnAmfCpT(1) - zn_AnAmfCpT(0));

					double tzx = zn_TAirCR(anAmfCpTVent, anAmfCpVent);
					if (fabs(tzx - zn_tzspD) > .01)
						printf("\nTD mismatch");
				}
#endif
			}
	...
	return ret;
}	// ZNR::zn_FVentPreferred
//-----------------------------------------------------------------------------
#if 0
int ZNR::zn_FVentPreferredOld()			// find zone's preferred vent fraction
// assume tz is at floating temp
// sets zn_fVentPrf = best vent fraction for this zone in isolation (0 - 1)
// returns -1: vent is harmful
//          0: no effect / don't care
//          1: useful
{
	...
	return ret;
}	// ZNR::zn_FVentPreferredOld
#endif
//-----------------------------------------------------------------------------
RC ZNR::zn_DoVentIf(			// determine zone conditions with possible vent
	const float fVentZn)	// target vent fraction, 0 - 1
							//   based on global availability and zone preference

// call with tz at floating temp w/o vent

// return
//   zn_fVent > 0 iff vent enabled
//   tz = zone temp modified by vent if any
// 
...
// returns RCOK or ...
{
	...
	{	// float temp
		...
		if (fVentZn > 0.f)
		{
			zn_anAmfCpVent = fVentZn * (zn_AnAmfCp(1) - zn_AnAmfCp(0));
			zn_anAmfCpTVent = fVentZn * (zn_AnAmfCpT(1) - zn_AnAmfCpT(0));

			zn_pz0 = fVentZn * zn_pz0W[1] + (1.f - fVentZn) * zn_pz0W[0];	// zone pressure
...
#if 0 && defined( _DEBUG)
			double qVentSh = zn_anAmfCpTVent - zn_anAmfCpVent * tz;
			if (qVentSh > 0.)
				printf("\n%s qVentSh > 0", name);
#endif

			if (!bUZ && fVentZn < 1.f && fVentZn == zn_fVentPrf)
...
#endif
		zn_fVent = fVentZn;		// final vent fraction
	}
	return rc;
...
RC ZNR::zn_CondixCR2()		// zone conditions, part 2
{
	...
	zn_qVentSh = zn_anAmfCpTVent - zn_anAmfCpVent * tz;
#if 0 && defined( _DEBUG)
	if (zn_qVentSh > 0.)
	{
		printf("\n%s: qVentSh (%0.3f) > 0, zn_fVent = %0.3f", name, zn_qVentSh, zn_fVent);
		if (zn_qVentSh > 1000.)
			printf("  Big!");
	}
#endif

	zn_qIzSh = zn_AnAmfCpT(0) + zn_sysDepAirIls.af_AmfCpT()
		- (zn_AnAmfCp(0) + zn_sysDepAirIls.af_AmfCp()) * tz
		+ zn_qIzXAnSh + zn_qVentSh;
...
//         else nz value
{
	...
	if (!rs_IsFanCoil() && !rs_IsWSHP() && !rs_IsVCClg())
...
//-----------------------------------------------------------------------------
RC RSYS::rs_SetRunConstantsASHP()	// finalize constant heating data for simulation
...
//============================================================================
RC TOPRAT::tp_DoVent(		// ventilation modeling
	const ULI ventAvail)	// global ventilation availability
							//   C_VENTAVAILVC_WHOLEBLDG: all zones control vent in parallel
							//   C_VENTAVAILVC_ZONAL: zone-by-zone vent control
							//   C_VENTAVAILVC_RSYSOAV: use outdoor air vent via RSYS

// returns: RCOK if success
{
...
#if 0 && defined( _DEBUG)
	bool bReportVent = jDay == 178 /* && !isWarmup */;
#endif

	RC rc = RCOK;

	tp_fVent = 0.f;	// consensus whole building vent fraction (if not RSYSOAV)
					//     = fraction of full vent flow to use
					//       determined by zone that wants the least vent
					//  or -1 for zonal vent control
	...
	case C_VENTAVAILVC_WHOLEBLDG:
	case C_VENTAVAILVC_ZONAL:
	{
		// Initial assessment of vent usefulness by zone
		//   WHY: if vent not useful for any zone, "vent-on" airnet calcs can be skipped
		int nVentUseful = 0;	// ventilation usefulness
								//   >0: vent potentially useful for 1 or more zones
								//   =0: vent not useful
								//   <0: vent forbidden by conditions in 1 or more zones
		...
		RLUP(ZrB, zp)
		{
			int nVU1 = zp->zn_AssessVentUtility();	// returns -9999, 0, 1
#if 0 && defined(_DEBUG)
			if (bReportVent)
			{	// printf("\n%s Hr=%d, Zone '%s': VU=%d", dateStr, iHr, zp->name, nVU1);
				if (nVU1 < 0)
					zp->zn_AssessVentUtility();		// call again for debug
			}	
#endif	
			if (nVU1 > 0 || ventAvail == C_VENTAVAILVC_WHOLEBLDG)
				nVentUseful += nVU1;	// count zones where vent may be useful
										//   if WHOLEBLDG, include big <0 value to forbid
										// 
			// if (nVentUseful < 0) NO: zn_AssessVentUtility() must set zn_ventUt for all zones
			//		break; 
		}
		...
		{
			if (tp_pAirNet)
				tp_pAirNet->an_ClearResults(1);
		}
		else
		{	// vent might be useful for at least 1 zone
			loadsIzxSh2();		// AirNet "vents open"
			tp_fVent = 1.f;		// try full vent and limit
			RLUPC(ZrB, zp, zp->zn_IsVentAvail() /* && zp->zn_ventUt >= 0 */ )
			{
				int vRet = zp->zn_FVentPreferred();
				if (ventAvail == C_VENTAVAILVC_WHOLEBLDG)
				{	// whole building control
					...
					if (vRet < 0)
					{
						tp_fVent = 0.f;	// vent is harmful for this zone: off for all
						break;
					}
					else if (vRet > 0)
					{
						if (zp->zn_fVentPrf < tp_fVent)
							tp_fVent = zp->zn_fVentPrf;
						bAllNoVent = 0;
					}
					// else vRet == 0
					//   this zone does not care, do nothing
		...
		if (bAllNoVent)
			tp_fVent = 0.f;		// no zone can vent

#if 0 && defined( _DEBUG)
		if (bReportVent && !isWarmup)
			printf("\n%s Hr=%d: NVU=%d  ANV=%d  TFvnt=%0.3f", dateStr, iHr, nVentUseful, bAllNoVent, tp_fVent);
#endif
		break;
	 }

	case C_VENTAVAILVC_RSYSOAV:
		// attempt OAV for each system
		// if successful, rs_mode = (zones)->zn_hcMode = rsmOAV (prevents other RSYS modeling)
		// fVent = 0 if here
		RSYS* rs;
	...
	RLUP(ZrB, zp)
	{
		float fVentZn = ventAvail == C_VENTAVAILVC_ZONAL ? zp->zn_fVentPrf : tp_fVent;

		rc |= zp->zn_DoVentIf( fVentZn);
