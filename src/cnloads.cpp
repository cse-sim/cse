	...
	int isAusz)		// TRUE = autosize, FALSE = main simulation
{
	...
	// autosizing init
	// set flag nz iff autosizing now underway
...
// returns fan power assumed to be included in net ratings, W/cfm
{
	...
	{	// pre-2024 assumptions for non-ASHPVC types
...
//-----------------------------------------------------------------------------
float RSYS::rs_FanHeatRatedAtRatedSpeed(		// fan heat at rating conditions
	int iHC,			// 0=htg, 1=clg
...
}	// RSYS::rs_FanHeatRated
//-----------------------------------------------------------------------------
float RSYS::rs_FanHeatRatedAtSpeedF(
	int iHC,		// 0=htg, 1=clg
	float capRef,	// reference capacity (typically rs_cap47 or rs_cap95)
	float speedF) const
{
...
float RSYS::rs_FanHeatAtSpeedF(
	int iHC,		// 0=htg, 1=clg
	float capRef,	// reference capacity (typically rs_cap47 or rs_cap95)
	float fanHeatRef,	// fan heat at capRef, Btuh
	float speedF) const
{
	if (rs_IsASHPVC())
	{
...
#if defined( _DEBUG)
		if (!rs_pPMACCESS[iHC])
			oer("rs_FanHeatRatedAtSpeedF: Null rs_PMACCESS[ %d]", iHC);
		else
#endif
...

{
	...
	else if (rs_IsASHP())
	{
		...
		{	// ASHP simulation (not autosize)
			// run full model
			rs_capHt = rs_CapEffASHP2();	// sets rs_capHt and rs_EffHt
		}
	}
	else if (rs_IsWSHP())
	{
	...
	{
		if (rs_capHtFS == 0.f)
		{	// do initial setup only once per step
			rs_CurCapHtCHDHW();
		}
		rs_capHt = rs_capHtFS * rs_speedF;
		rs_effHt = 1.f;
	}
	else
...
float RSYS::rs_PerfASHP2(		// ASHP performance
	...
	float fanHAdj,		// fan power adjustment, Btuh
						// removed from capacity and input (before COP calc)
	float& capHt,		// returned: gross (compressor-only) full speed heating capacity
	float& inpHt,		// returned: gross (compressor-only) full speed input power, Btuh
...
// redundant calls OK
{
	...
	rs_fanHRtdH = rs_IsASHPHydronic()
		? 0.f
		: rs_FanHeatRated(0, rs_cap47, PMSPEED::RATED);
...
// redundant calls OK
{
	...
	rs_fanHRtdH = rs_FanHeatRated(0, rs_capH, PMSPEED::RATED);
...
// sets rs_tCoilEW, rs_capHtFS, and rs_speedFMin
{
...
// returns total heating output at rs_tdbOut, Btuh
{
	...
	int ashpModel = 0;
	float capHt, inpHt, capDfHt;
	rs_PerfASHP2(ashpModel, rs_tdbOut, 1.f, rs_fanHRtdH,
		capHt, inpHt, capDfHt);
	...
	//			0: no amf available, rs_amf = 0
{
	...
	if (rs_IsCHDHW() && rs_mode == rsmHEAT)
	{	// CHDHW heating
		if (rs_capHtFS == 0.f)
			rs_CurCapHtCHDHW();
		rs_capHt = speedF * rs_capHtFS;
		float avf;
		float fanPwr;	// unused
