...
- `record-index-exceptions.yaml` allows one to override where a record index's main header link is **not** located in the `doc/src/records` by the file corresponding to the record's name (lowercase) with a header of that same name resulting in an in-file hash link of the same name (lowercase). In other words, we would normally expect to find a record named "SOMETHING" in file `doc/src/records/something.md` where, opening that file, we would expect a header of `# SOMETHING` (Markdown). If that is **not** the case, this file is where you say differently. The format of the file is:
